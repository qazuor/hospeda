# V39 · Tirada de voz — parte B (S3 + S4)

Genera **la pista de audio de T3 y T4**. La imagen se descarta: sólo se usa la voz.

Montaje y hoja de corte: [`montaje.md`](montaje.md).

---

Segunda mitad del guion, ver [`voz1.md`](voz1.md) para la explicación completa de por
qué son dos tiradas. Cubre el tramo T3–T4 (9,5 a 20,0 s del video).

**Aunque T3 y T4 llevan su propio lip sync**, el audio final del video sale
enteramente de esta tirada — igual que en el resto de la serie.
...
48-52% a SHORT pause.
52-80% he speaks the SECOND sentence.
80-100% SILENCE until the end.

DO NOT SPREAD THE SPEECH ACROSS THE CLIP. The delivery is at a normal conversational
pace and finishes at 80%. Do not slow it down and do not stretch it to fill the 12
seconds.

FIRST sentence: [Spanish] A partir de ahí, coordinás los detalles y llegás a un acuerdo
directamente con esa persona.

SECOND sentence: [Spanish] La reserva, el pago y el trato quedan entre ustedes dos.
...
- **Que el timbre sea idéntico al de `voz1.md`** — se generan por separado, así que
  conviene escucharlas una después de la otra antes de dar por buena la narración
  completa.
- **Que "dos", la última palabra, quede completa y clara.**
