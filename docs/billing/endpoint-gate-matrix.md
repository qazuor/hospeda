...
| `POST /api/v1/protected/accommodations/import-from-url` | `accommodation/protected/import-from-url.ts` | gate+limit | `ai_accommodation_import`, `max_ai_accommodation_import_per_month` | wired | SPEC-222 T-020. Endpoint access is authenticated-user only (BETA-153) — no accommodation permission is required, mirroring `host-onboarding/start`, so a plain USER can import their first listing BEFORE the USER -> HOST promotion (which happens later, on the mini-form submit). This endpoint is stateless (scrape + draft return, persists nothing); it is NOT entitlement-gated. The AI entitlement + monthly quota apply ONLY to Strategy B (AI-assisted extraction for sparse generic pages), enforced LAZILY inside the injected `aiExtract` port and degrade-clean (structured-only partial + informational notice, never 403) when the plan lacks `ai_accommodation_import` or the monthly quota is spent. Successful AI calls metered via recordAiUsage. Per-user 10/h sliding-window rate limit (HOSPEDA_IMPORT_RATE_LIMIT_RPH) → 429. |
| `GET /api/v1/protected/accommodations/import-from-url/status` | `accommodation/protected/import-from-url-status.ts` | none | - | n/a | HOS-50 T-011. Auth-only status poll for an in-flight async Apify run started by the 202 branch of the import-from-url POST above; no entitlement/limit gate of its own — the same authenticated-user-only access as the POST route applies (BETA-153; no accommodation permission required), and this route only reads the run state the client already holds. Not billing-metered (no AI/entitlement spend happens here — Strategy B AI extraction is unavailable on this path, see the route's module doc). |
...
| `GET /api/v1/protected/host-onboarding/precheck` | `host-onboarding/protected/precheck.ts` | none | - | n/a | BETA-197. Read-only precheck feeding the web "publicar nueva" dialog decision (draftCount × hasQuota matrix via `deriveOnboardingDecision`). Same funnel-exception access as `/start` — any authenticated user, no `publish_accommodations` permission required. It only reads `accommodationService.count`/`.list`; it does not mutate anything, so it is not itself limit-gated — `enforceAccommodationLimit()` still runs on the actual create/onboarding-start routes. |
| `GET /api/v1/protected/publish/precheck/{vertical}` | `publish/protected/precheck.ts` | none | - | n/a | HOS-1156 D-7. The vertical-parameterised generalisation of the row above, serving accommodation, gastronomy and experiences from one matrix. Same funnel-exception access — any authenticated user, no permission required — for the same reason: a tourist who has never published still needs to be told they may `create_direct`. Read-only, so it is not itself limit-gated; the real caps stay on the create paths (`enforceAccommodationLimit` / `enforceGastronomyLimit` / `enforceExperienceLimit`). It READS a commerce cap via `resolveCommerceVerticalCap` — the same function `commerceVerticalEntitlementMiddleware` calls — because that middleware takes its vertical at construction time and so cannot serve a route whose vertical arrives as a path param; mounting the wrong one would be silent, since an absent limit key reads as unlimited. Fails OPEN to `create_direct` by design (the create path fails closed). |
...
| `POST /api/v1/protected/accommodations/{id}/publish` | `accommodation/protected/publish.ts` | none | - | n/a | Publish (DRAFT → ACTIVE) is ungated; billing eligibility (first-publish trial vs subscription) is enforced inside `publish()` via checkEligibility, not a route gate; ownership enforced via route factory (HOS-110) |
...
| `GET /api/v1/protected/accommodations/{id}/whatsapp` | `accommodation/protected/getWhatsApp.ts` | none | `can_contact_whatsapp_display`, `can_contact_whatsapp_direct` | n/a | HOS-19: VIEWER-gated read. Auth required; the number is returned ONLY when the CALLER holds `can_contact_whatsapp_display` (fail-closed — omits the number, never 403s), and the `wa.me` direct link only with `can_contact_whatsapp_direct`. Gate is inline (`hasEntitlement`) not `requireEntitlement` middleware, so the web block degrades to an upsell instead of erroring. Separate from the shared-cached public payload for cache safety. |
...
| `PUT /api/v1/protected/accommodations/{id}/faqs/reorder` | `accommodation/protected/reorderFaqs.ts` | gate | `edit_accommodation_info` | wired | HOS-393. requireEntitlement(EDIT_ACCOMMODATION_INFO) — same gate as the sibling FAQ mutation routes; service layer enforces UPDATE_OWN + ownership via `_canUpdate`. Registered BEFORE `/{id}/faqs/{faqId}` so "reorder" is not resolved as a faqId UUID param. Mirrors gastronomy/experience protected FAQ reorder (PUT); the admin tier uses PATCH for the equivalent route (preexisting cross-tier inconsistency, not addressed here). |
...
| `POST /api/v1/protected/accommodations/{id}/media/featured` | `accommodation/protected/addFeaturedMedia.ts` | gate | `edit_accommodation_info` | wired | Upload straight to COVER (HOS-803). requireEntitlement(EDIT_ACCOMMODATION_INFO); ownership via `_canUpdate` in the service (UPDATE_ANY, or UPDATE_OWN + ownership). DELIBERATELY EXEMPT from the plan gallery cap: unlike the sibling `addMedia` this route never answers LIMIT_REACHED for a full gallery — that refusal WAS the bug, because a cover is not a gallery item (HOS-791) and the owner at the cap could therefore never replace the one photo that does not occupy a slot. The exemption is earned, not asserted: the swap is quota-neutral by construction — the row is created already featured and the cover it replaces is soft-deleted in the SAME transaction, so one row enters the featured slot and one leaves the table and the visible gallery cannot move. Decision is `gate`, not `gate+limit`, because no cap is enforced here; `max_photos_per_accommodation` IS read (server-side, via getRemainingLimit — never from the request body) for exactly one purpose: a plan whose allowance is 0 includes no photos at all and so grants no cover either. The per-ENTITY cap `getGalleryCap('accommodation')` is untouched and still applies, since it measures the gallery and the gallery does not grow. |
...
| `PATCH /api/v1/protected/accommodations/{id}/media/{mediaId}` | `accommodation/protected/updateMedia.ts` | gate | `edit_accommodation_info` | wired | H-125 / G7 smoke. requireEntitlement(EDIT_ACCOMMODATION_INFO) — same gate as the sibling gallery mutations. Corrects the text of an already-uploaded photo (`caption`, `description`, `alt`, `attribution`); before this the only way to fix a wrong alt was deleting the photo and re-uploading it, which burned a second Cloudinary asset and lost the row's ordering. Cannot touch `url`, `publicId`, `moderationState`, `state`, `isFeatured` or `sortOrder`. |
...
| `GET /api/v1/protected/accommodations/{id}/occupancy` | `accommodation/protected/getOccupancy.ts` | none | - | n/a | HOS-43 Phase 1. Owner read of own occupancy calendar; auth + ownership (or ACCOMMODATION_UPDATE_ANY) in handler; reads are not entitlement-gated. |
| `POST /api/v1/protected/accommodations/{id}/occupancy` | `accommodation/protected/addOccupancy.ts` | gate | `can_use_calendar` | wired | HOS-43 Phase 1. requireEntitlement(CAN_USE_CALENDAR) middleware + ACCOMMODATION_OCCUPANCY_MANAGE permission + ownership. Marks a single day occupied (source=MANUAL). |
| `PATCH /api/v1/protected/accommodations/{id}/occupancy/batch` | `accommodation/protected/batchOccupancy.ts` | gate | `can_use_calendar` | wired | HOS-43 Phase 1. requireEntitlement(CAN_USE_CALENDAR) + ACCOMMODATION_OCCUPANCY_MANAGE + ownership. Idempotent block/unblock of a set of dates (MANUAL rows only). |
| `PATCH /api/v1/protected/accommodations/{id}/occupancy/event` | `accommodation/protected/updateOccupancyEvent.ts` | gate | `can_use_calendar` | wired | HOS-175 Phase 3. requireEntitlement(CAN_USE_CALENDAR) + ACCOMMODATION_OCCUPANCY_MANAGE + ownership. Atomically moves a MANUAL event from [oldStartDate..oldEndDate] to [newStartDate..newEndDate] (both inclusive) and/or edits its note, in one transaction (delete old range + upsert new range, MANUAL rows only). |
| `DELETE /api/v1/protected/accommodations/{id}/occupancy/{date}` | `accommodation/protected/removeOccupancy.ts` | gate | `can_use_calendar` | wired | HOS-43 Phase 1. requireEntitlement(CAN_USE_CALENDAR) + ACCOMMODATION_OCCUPANCY_MANAGE + ownership. Unblocks a single MANUAL day. |
| `POST /api/v1/protected/accommodations/{id}/calendar-sync/connect-google` | `accommodation/protected/calendarConnectGoogle.ts` | gate | `can_sync_external_calendar` | wired | HOS-157 Phase 2. requireEntitlement(CAN_SYNC_EXTERNAL_CALENDAR) + ACCOMMODATION_OCCUPANCY_MANAGE + ownership (inline via assertOccupancyManageAccess). Returns the Google OAuth authorize URL. |
| `GET /api/v1/protected/accommodations/calendar-sync/google/callback` | `accommodation/protected/calendarGoogleCallback.ts` | none | - | n/a | HOS-157 Phase 2. OAuth return leg (fixed redirect URI, no :id). **createPublicRoute (skipAuth)** so a session that lapses mid-consent still redirects gracefully instead of a raw 401; secured by the one-time, server-bound CSRF state matched against a connect-time HttpOnly SameSite=Lax double-submit cookie (state issued only to the authenticated owner who passed the connect gate); no entitlement gate. |
| `POST /api/v1/protected/accommodations/{id}/calendar-sync/connect-ical` | `accommodation/protected/calendarConnectIcal.ts` | gate | `can_sync_external_calendar` | wired | HOS-162 Phase 3. requireEntitlement(CAN_SYNC_EXTERNAL_CALENDAR) + ACCOMMODATION_OCCUPANCY_MANAGE + ownership (inline via assertOccupancyManageAccess). Airbnb/Booking/generic iCal feed connect — probes the feed live (fetchAndParseIcsFeed) before persisting; 400 VALIDATION_ERROR on an unreadable feed, nothing saved. Triggers an immediate first sync after persisting (a failed first sync is swallowed; the connection stays saved and the 6-hourly cron retries). |
| `POST /api/v1/protected/accommodations/{id}/calendar-sync/sync` | `accommodation/protected/calendarSync.ts` | gate | `can_sync_external_calendar` | wired | HOS-157 Phase 2, widened HOS-162 Phase 3. requireEntitlement(CAN_SYNC_EXTERNAL_CALENDAR) + ACCOMMODATION_OCCUPANCY_MANAGE + ownership. On-demand sync; dispatches by the optional `provider` body field (default `google`, backward-compatible) to the Google or iCal sync service. Operational failures return status=error, not 5xx. |
| `GET /api/v1/protected/accommodations/{id}/calendar-sync/status` | `accommodation/protected/calendarSyncStatus.ts` | none | - | n/a | HOS-157 Phase 2, widened HOS-162 Phase 3. Read of connection sync health (safe projection) — now returns `{ connections: [...] }`, one row per EVER-connected provider (google/airbnb/booking/other). Ownership only, no entitlement — parity with the occupancy read route so a downgraded host can still see/clean up. |
| `DELETE /api/v1/protected/accommodations/{id}/calendar-sync/{provider}` | `accommodation/protected/calendarDisconnect.ts` | none | - | n/a | HOS-157 Phase 2, widened HOS-162 Phase 3. Soft-disconnect (isActive=false). `:provider` now accepts google/airbnb/booking/other. ACCOMMODATION_OCCUPANCY_MANAGE + ownership; deliberately NO entitlement gate so a downgraded host can remove a stale connection. |
| `GET /api/v1/protected/accommodations/{id}/qr-sheet` | `accommodation/protected/qrSheet.ts` | none | - | n/a | HOS-982: owner-only printable A4 PDF carrying a large QR that resolves to the listing's public page — designed to be taped to a door or left on a counter. **UNGATED ON PURPOSE (owner decision, 2026-09-07)**, and this row exists so that decision lives somewhere a reviewer looks rather than only in the route's own docblock. The neighbouring brochure requires `download_listing_pdf`; this sheet requires nothing but a published ficha, because a QR on a door brings people TO the platform — restricting it costs us, not the subscriber. Do not "harmonise" the two: one is a premium marketing asset handed to a customer, the other is an acquisition channel we want on every door. Frozen by `test/static-guards/listing-qr-sheet-no-entitlement-gate.guard.test.ts`, which fails if `requireEntitlement` or `EntitlementKey` ever appears in any of the three route files. What IS checked: authentication, then ownership (`ownerId === actor.id` \|\| `ACCOMMODATION_UPDATE_ANY`) answering 404 with the same wording as `getById.ts` — a 403 would confirm the id exists — then publication, `lifecycleState === ACTIVE && visibility === PUBLIC`, because a code printed for a listing with no public page 404s on every scan, permanently, on paper. Rate-limited to 20/min: minting writes a `qr_codes` row on the first call. |
...
| `POST /api/v1/protected/billing/subscriptions/{localId}/replace-payment-method` | `billing/replace-payment-method.ts` | none | - | n/a | HOS-348 Part B: mints a real MercadoPago preapproval to replace a `past_due` subscription's failing card. This moves money (a live provider preapproval, not a read) but is deliberately NOT entitlement-gated — a past-due customer's plan has already lapsed, and this route's whole purpose is letting them fix the ONE thing (their card) that is blocking every other gate, so it cannot itself require an entitlement to reach it. Auth + ownership only: the row is looked up by `{localId}` and rejects with 404 (never 403 — a 403 would confirm the id exists) both when it does not exist AND when `row.customerId !== billingCustomerId`, so only the subscription's own owner can ever act on it; a well-formed request against someone else's id gets no signal beyond "not found". Also rejects 422 for a row that is not `past_due`, or whose `billingInterval` is annual (Part B scope is monthly only). Deliberately exempt from `pastDueGraceMiddleware` (`GRACE_EXEMPT_PATH_SUFFIXES` in `past-due-grace.middleware.ts`) — it IS the recovery path the grace gate exists to funnel a past-due customer toward, so it must stay reachable even after the grace window has fully expired. |
...
| `POST /api/v1/protected/host-trades/{slug}/usages` | `host-trade/protected/usages.ts` | none | - | n/a | HOS-376 T-030, the QR path. Gated by the `HOST_TRADE_VIEW` permission only — the directory itself is a free acquisition hook, not a paid perk (owner decision, engram #8776), so a billing entitlement here would gate the wrong thing. The permission is what keeps the weakly-verified host-declared branch safe: a passer-by who scans the sticker on a van is not a host (spec §6.5) |
| `GET /api/v1/protected/host-trades/usages/pending` | `host-trade/protected/usages.ts` | none | - | n/a | HOS-376 T-030. Auth-only on purpose, one step looser than the declaration above: the rows are already scoped to `hostUserId = actor.id`, so a permission would only decide whether a host may read his own inbox — and it would lock out someone whose directory perk lapsed while confirmations were still waiting on them |
| `GET /api/v1/protected/host-trades/usages/pending-count` | `host-trade/protected/usages.ts` | none | - | n/a | HOS-376 T-030. Same auth-only reasoning as the list it counts; feeds the navigation badge |
| `GET /api/v1/protected/host-trades/usages` | `host-trade/protected/usages.ts` | none | - | n/a | HOS-376 T-046, the host's own record. Auth-only for the same reason as the inbox: the rows are already scoped to `hostUserId = actor.id`, so a permission could only decide whether a host may read his own history — and it would lock out someone whose directory perk lapsed while confirmed usages stayed on file. Deliberately NOT a widened `/usages/pending`: that path answers "what awaits your action" and its definition (declared by a provider, still PENDING) is shared with the navigation badge, whereas this one includes the host's OWN declarations and the CONFIRMED rows the review action depends on. Collapsing them would make the badge count a history. Not declared in spec §7.5 — added because the host otherwise had no read of his own usages at all, which left the QR declaration flow ending nowhere |
| `POST /api/v1/protected/host-trades/mine/usages` | `host-trade/protected/mine-usages.ts` | none | - | n/a | HOS-376 T-031. Auth + row ownership, the same shape as `/mine` (HOS-278 AC-7): an approved provider is an ordinary account holding no `HOST_TRADE_*` permission, so requiring one would lock them out of their own listing. The path carries no id, so "a provider cannot declare from another's listing" is structural, not a check |
| `GET /api/v1/protected/host-trades/mine/usages` | `host-trade/protected/mine-usages.ts` | none | - | n/a | HOS-376 T-031. Same auth + ownership shape; scoped server-side to the caller's own listing |
| `GET /api/v1/protected/host-trades/mine/linked-hosts` | `host-trade/protected/mine-usages.ts` | none | - | n/a | HOS-376 T-031, the declaration selector. Same auth + ownership shape. Scoped to hosts with a CONFIRMED usage on the caller's listing — it exposes nobody the provider has not already served — and the payload carries a display name but never an email |
| `GET /api/v1/protected/host-trades/mine/qr` | `host-trade/protected/mine-qr.ts` | none | - | n/a | HOS-376 T-032. Same auth + ownership shape. The QR is derived from the caller's own slug on every call — nothing stored, nothing to expire — so there is no state an entitlement could gate |
| `GET /api/v1/protected/host-trades/mine/reviews` | `host-trade/protected/mine-reviews.ts` | none | - | n/a | HOS-376 T-050, spec §7.5. Same auth + ownership shape as the rest of `/mine/*`: an approved provider holds no `HOST_TRADE_*` permission (HOS-278 AC-7), and the path carries no id, so reading another's is structural rather than a check. NOT a duplicate of `GET /{id}/reviews`: that one nulls any reply which is not APPROVED, which is right for a host browsing the directory and wrong for the reply's own author — it would tell a provider that the answer he just wrote does not exist. Here the reply carries its `moderationState` and its rejection reason. The REVIEW side stays filtered to APPROVED, because one still in moderation is not published and handing it to its subject would surface a complaint that may yet be rejected |
| `POST /api/v1/protected/host-trades/usages/{id}/confirm` | `host-trade/protected/usage-transitions.ts` | none | - | n/a | HOS-376 T-033. Auth-only, and deliberately role-blind: `declaredBy` on the row decides who the counterpart is, so one endpoint serves both sides. An account that is both a host and a provider has no single role to branch on. Every foreign path answers 404, never 403, so the endpoint is not an oracle for which usage ids exist |
| `POST /api/v1/protected/host-trades/usages/{id}/reject` | `host-trade/protected/usage-transitions.ts` | none | - | n/a | HOS-376 T-033. Same role-blind, auth-only shape as the confirm |
| `POST /api/v1/protected/host-trades/usages/{id}/reject/undo` | `host-trade/protected/usage-transitions.ts` | none | - | n/a | HOS-376 T-033. Restricted to the account that rejected it, not merely the counterpart — otherwise the rejected party could reverse a refusal aimed at them |
| `POST /api/v1/protected/host-trades/{id}/reviews` | `host-trade/protected/reviews.ts` | none | - | n/a | HOS-376 T-034. Gated by the `HOST_TRADE_REVIEW_CREATE` permission only — same reasoning as the declaration above: the directory is a free acquisition hook, not a paid perk (owner decision, engram #8776), so a billing entitlement would gate the wrong thing. The other three eligibility gates of spec §6.3 (a CONFIRMED usage for the pair, not your own listing, a provider still listed) are facts about rows and live in the service |
| `PATCH /api/v1/protected/host-trades/reviews/{id}` | `host-trade/protected/reviews.ts` | none | - | n/a | HOS-376 T-034. Auth + row ownership, one step looser than creating: the row is already scoped to `hostUserId = actor.id`, so a permission could only decide whether a host may edit something he wrote himself — and it would lock out a host whose directory perk lapsed while his review stayed published. A review that is not the caller's answers 404, never 403, so the endpoint is not an oracle for which review ids exist |
| `GET /api/v1/protected/host-trades/{id}/my-review` | `host-trade/protected/reviews.ts` | none | - | n/a | HOS-376 T-034. Same auth + ownership shape as the edit. Answers `{ review: null }` rather than 404 when the host has written none — absence is an ordinary state here, and it is what the provider card reads to choose between offering "write one" and "edit yours" |
| `POST /api/v1/protected/host-trades/reviews/{id}/reply` | `host-trade/protected/replies.ts` | none | - | n/a | HOS-376 T-035. Auth + row ownership, no permission and no entitlement: HOS-278 AC-7 again — an approved provider is an ordinary account holding no `HOST_TRADE_*` permission, and requiring one would silence him on the one page carrying a complaint about his own business. Ownership is re-derived from the listing server-side, so a listing that changed hands does not leave its former owner able to answer. A review that is not on the caller's listing answers 404, never 403 |
| `PATCH /api/v1/protected/host-trades/replies/{id}` | `host-trade/protected/replies.ts` | none | - | n/a | HOS-376 T-035. Same auth + ownership shape. Editing returns the reply to moderation and discards the previous decision, which was made about text that no longer exists (AC-23) |
| `GET /api/v1/protected/host-trades/{id}/reviews` | `host-trade/protected/directory-reviews.ts` | none | - | n/a | HOS-376 T-036, the directory's own review listing. Gated by `HOST_TRADE_VIEW` only — these rows ARE the directory, which is a free acquisition hook rather than a paid perk (owner decision, engram #8776). The endpoint declares no moderation filter, so a caller cannot ask for anything but approved reviews; the service forces that anyway. A provider reading his OWN reviews holds no `HOST_TRADE_*` permission and does not come through here — that is `/mine/reviews` (spec §7.5) |
| `GET /api/v1/protected/host-trades/{slug}` | `host-trade/protected/get-by-slug.ts` | none | - | n/a | HOS-376 T-045, the read the QR landing page runs on. Gated by `HOST_TRADE_VIEW` only, matching the declaration POST it precedes — the directory is a free acquisition hook, not a paid perk (owner decision, engram #8776), and a visitor who is not a host has no more business reading a provider off a scanned sticker than declaring a usage with one. It exists because the destination-scoped list cannot answer the page's two questions: a QR carries no destination, and in that list a revoked listing is indistinguishable from one in somebody else's destination — both read as absent. So 404 and 422 PROVIDER_REVOKED are two statuses here. A declaration suspension is deliberately NOT reported: `declarationSuspended*` is excluded from the public read shape because a suspension freezes a listing rather than passing public judgement on a person, so it surfaces on the POST (403 DECLARATION_SUSPENDED), never on a generic read |
| **PARTNER — PROTECTED** | | | | | |
| `GET /api/v1/protected/partners/mine` | `partners/protected/mine.ts` | none | - | n/a | HOS-278 D3: a partner's OWN listing. Auth-only by design — no PARTNER_* permission and no entitlement. An approved partner is an ordinary account, and PARTNER_VIEW_ALL is an admin perk; gating on it would lock them out of their own ficha (AC-7). Scoped server-side to `ownerUserId = actor.id`, so there is nothing an elevated permission would unlock |
| `PATCH /api/v1/protected/partners/mine` | `partners/protected/mine.ts` | none | - | n/a | HOS-278 D3. Same auth-only reasoning as the GET. Identity and commercial fields are absent from the request schema and therefore stripped, not rejected; contact and social links apply immediately (merged JSONB), while logo/description/website are parked for admin review rather than published |
| `GET /api/v1/protected/partners/mine/mentions` | `partners/protected/mine-mentions.ts` | none | - | n/a | HOS-377: a partner's OWN mentions log. Auth-only, gated by ownership of `partners.ownerUserId` exactly like `GET /mine` (HOS-278 AC-7) — an approved partner holds no PARTNER_* permission, so requiring one would lock them out of their own log. Fails closed to an empty list rather than 403, which would confirm a partner exists. Admin-only fields are stripped by the service, not by this route |
| `GET /api/v1/protected/partners/mine/stats` | `partners/protected/mine-stats.ts` | none | - | n/a | HOS-1063 A-4: a partner's OWN in-platform numbers. Auth-only for the same reason as `GET /mine` and `/mine/mentions` — an approved partner holds no PARTNER_* permission. The `gate` half is deliberately NOT copied from the host-side `daily-series.ts`, which demands VIEW_BASIC_STATS: `loadEntitlements` resolves against the ACCOMMODATION subscription and a partner subscription lives in a different product domain, so an entitlement gate here would refuse every partner who is not also a paying host, and admit a host who is not a partner (spec §5.6, NG-8). No id in the path, so "a partner cannot read another's" is structural. Fails closed to an "unavailable" payload, never 403 (which would confirm a partner exists) and never 404 |
...
| `GET /api/v1/protected/events/{id}` | `event/protected/getById.ts` | none | - | n/a | Author's own event for the web editor; ownership on `authorId`, EVENT_VIEW_ALL bypasses (HOS-374) |
...
| `POST /api/v1/protected/events/{id}/moderate` | `event/protected/moderate.ts` | none | - | n/a | Trusted-editor self-approve — authorship + EVENT_PUBLISH_OWN, target state APPROVED only, checked in the service; a foreign event is masked to 404 (HOS-1037) |
...
| `GET /api/v1/protected/events/{id}/media` | `event/protected/getMedia.ts` | none | - | n/a | List the gallery. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkEventCanEditMedia`, which delegates to `checkCanUpdateEvent` — editing an event's photos IS editing the event, so the gate is deliberately identical (EVENT_UPDATE, or EVENT_UPDATE_OWN for the author while the platform has not approved the event). NOT entitlement-gated: posts and events carry no per-plan photo allowance — the accommodation `max_photos_per_accommodation` LimitKey has no editorial counterpart. This endpoint creates no gallery slot. |
| `POST /api/v1/protected/events/{id}/media` | `event/protected/addMedia.ts` | none | - | n/a | Register an uploaded URL as a new row. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkEventCanEditMedia`, which delegates to `checkCanUpdateEvent` — editing an event's photos IS editing the event, so the gate is deliberately identical (EVENT_UPDATE, or EVENT_UPDATE_OWN for the author while the platform has not approved the event). NOT entitlement-gated (no editorial photo allowance exists). Like the commerce equivalent, this URL-receiver endpoint does not re-check any cap before inserting — the gallery cap lives on the UPLOAD endpoints via `routes/media/gallery-count.ts`. |
| `DELETE /api/v1/protected/events/{id}/media/{mediaId}` | `event/protected/removeMedia.ts` | none | - | n/a | Delete the binary then drop the row. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkEventCanEditMedia`, which delegates to `checkCanUpdateEvent` — editing an event's photos IS editing the event, so the gate is deliberately identical (EVENT_UPDATE, or EVENT_UPDATE_OWN for the author while the platform has not approved the event). NOT entitlement-gated; removing one's own photo is never a paid capability. |
| `PATCH /api/v1/protected/events/{id}/media/reorder` | `event/protected/reorderMedia.ts` | none | - | n/a | Reorder the gallery. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkEventCanEditMedia`, which delegates to `checkCanUpdateEvent` — editing an event's photos IS editing the event, so the gate is deliberately identical (EVENT_UPDATE, or EVENT_UPDATE_OWN for the author while the platform has not approved the event). NOT entitlement-gated; creates no gallery slot. |
| `PUT /api/v1/protected/events/{id}/media/{mediaId}/featured` | `event/protected/setFeaturedMedia.ts` | none | - | n/a | Set the featured photo. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkEventCanEditMedia`, which delegates to `checkCanUpdateEvent` — editing an event's photos IS editing the event, so the gate is deliberately identical (EVENT_UPDATE, or EVENT_UPDATE_OWN for the author while the platform has not approved the event). NOT entitlement-gated; creates no gallery slot. |
| `PATCH /api/v1/protected/events/{id}/media/{mediaId}` | `event/protected/updateMedia.ts` | none | - | n/a | HOS-1036: corrects the text of an already-uploaded photo (`caption`, `description`, `alt`, `attribution`); cannot touch `url`, `publicId`, `moderationState`, `state`, `isFeatured` or `sortOrder`. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkEventCanEditMedia`, which delegates to `checkCanUpdateEvent` — same gate as the sibling gallery mutations above. NOT entitlement-gated: posts and events carry no per-plan photo allowance, and correcting a photo's text consumes no gallery slot. |
...
| `GET /api/v1/protected/posts/{id}` | `post/protected/getById.ts` | none | - | n/a | Author's own post for the web editor; ownership on `authorId`, POST_VIEW_ALL bypasses (HOS-374) |
...
| `POST /api/v1/protected/posts/{id}/moderate` | `post/protected/moderate.ts` | none | - | n/a | Trusted-editor self-approve — authorship + POST_PUBLISH_OWN, target state APPROVED only, checked in the service; a foreign post is masked to 404 (HOS-1037) |
...
| `GET /api/v1/protected/posts/{id}/media` | `post/protected/getMedia.ts` | none | - | n/a | List the gallery. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkPostCanEditMedia`, which delegates to `checkCanUpdatePost` — editing a post's photos IS editing the post, so the gate is deliberately identical (POST_UPDATE, or POST_UPDATE_OWN for the author while the platform has not approved the post). NOT entitlement-gated: posts and events carry no per-plan photo allowance — the accommodation `max_photos_per_accommodation` LimitKey has no editorial counterpart. This endpoint creates no gallery slot. |
| `POST /api/v1/protected/posts/{id}/media` | `post/protected/addMedia.ts` | none | - | n/a | Register an uploaded URL as a new row. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkPostCanEditMedia`, which delegates to `checkCanUpdatePost` — editing a post's photos IS editing the post, so the gate is deliberately identical (POST_UPDATE, or POST_UPDATE_OWN for the author while the platform has not approved the post). NOT entitlement-gated (no editorial photo allowance exists). Like the commerce equivalent, this URL-receiver endpoint does not re-check any cap before inserting — the gallery cap lives on the UPLOAD endpoints via `routes/media/gallery-count.ts`. |
| `DELETE /api/v1/protected/posts/{id}/media/{mediaId}` | `post/protected/removeMedia.ts` | none | - | n/a | Delete the binary then drop the row. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkPostCanEditMedia`, which delegates to `checkCanUpdatePost` — editing a post's photos IS editing the post, so the gate is deliberately identical (POST_UPDATE, or POST_UPDATE_OWN for the author while the platform has not approved the post). NOT entitlement-gated; removing one's own photo is never a paid capability. |
| `PATCH /api/v1/protected/posts/{id}/media/reorder` | `post/protected/reorderMedia.ts` | none | - | n/a | Reorder the gallery. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkPostCanEditMedia`, which delegates to `checkCanUpdatePost` — editing a post's photos IS editing the post, so the gate is deliberately identical (POST_UPDATE, or POST_UPDATE_OWN for the author while the platform has not approved the post). NOT entitlement-gated; creates no gallery slot. |
| `PUT /api/v1/protected/posts/{id}/media/{mediaId}/featured` | `post/protected/setFeaturedMedia.ts` | none | - | n/a | Set the featured photo. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkPostCanEditMedia`, which delegates to `checkCanUpdatePost` — editing a post's photos IS editing the post, so the gate is deliberately identical (POST_UPDATE, or POST_UPDATE_OWN for the author while the platform has not approved the post). NOT entitlement-gated; creates no gallery slot. |
| `PATCH /api/v1/protected/posts/{id}/media/{mediaId}` | `post/protected/updateMedia.ts` | none | - | n/a | HOS-1036: corrects the text of an already-uploaded photo (`caption`, `description`, `alt`, `attribution`); cannot touch `url`, `publicId`, `moderationState`, `state`, `isFeatured` or `sortOrder`. HOS-390: relational content media. PermissionEnum-gated inside the service via `checkPostCanEditMedia`, which delegates to `checkCanUpdatePost` — same gate as the sibling gallery mutations above. NOT entitlement-gated: posts and events carry no per-plan photo allowance, and correcting a photo's text consumes no gallery slot. |
| **COMMENT — PROTECTED** | | | | | |
...
| `POST /api/v1/admin/accommodations/{id}/media/featured` | `accommodation/admin/addFeaturedMedia.ts` | none | - | n/a | Upload straight to COVER (HOS-803) — admin mirror of the protected route, and the one the admin panel actually calls (its hooks address /api/v1/admin/...). Admin write; PermissionEnum-gated via `_canUpdate` in the service (ACCOMMODATION_UPDATE_ANY, or UPDATE_OWN + ownership). Plan allowance resolved ONLY for owner-actors and bypassed for staff, exactly as the sibling `addMedia` does — an admin acting on a host's listing is a support action, not a consumption of that host's plan. Like the protected twin it never answers LIMIT_REACHED for a full gallery: the cover it replaces is soft-deleted in the same transaction, so the swap is quota-neutral by construction and there is no cap to spend. The per-entity `getGalleryCap` still applies. |
...
| `PATCH /api/v1/admin/accommodations/{id}/media/{mediaId}` | `accommodation/admin/updateMedia.ts` | none | - | n/a | Admin write; PermissionEnum-gated; text metadata only (caption/description/alt/attribution), nullable per field, empty body is VALIDATION_ERROR. No entitlement gate unlike the protected twin's EDIT_ACCOMMODATION_INFO — staff bypass entitlements (INV-6) and correcting misleading alt text is moderation, not a billable feature (HOS-388) |
...
| `GET /api/v1/admin/accommodations/{id}/occupancy` | `accommodation/admin/getOccupancy.ts` | none | - | n/a | HOS-43 Phase 1. Admin read-only occupancy view; PermissionEnum-gated (ACCOMMODATION_OCCUPANCY_VIEW); staff bypass entitlements so no billing gate. |
...
| `GET /api/v1/admin/events/{id}/media` | `event/admin/getMedia.ts` | none | - | n/a | List the gallery. HOS-390: relational content media. Admin write; PermissionEnum-gated (EVENT_UPDATE), enforced both by the route and by `checkEventCanEditMedia` in the service. NOT entitlement-gated: posts and events carry no per-plan photo allowance — the accommodation `max_photos_per_accommodation` LimitKey has no editorial counterpart. This endpoint creates no gallery slot. |
| `POST /api/v1/admin/events/{id}/media` | `event/admin/addMedia.ts` | none | - | n/a | Register an uploaded URL as a new row. HOS-390: relational content media. Admin write; PermissionEnum-gated (EVENT_UPDATE), enforced both by the route and by `checkEventCanEditMedia` in the service. NOT entitlement-gated (no editorial photo allowance exists). Like the commerce equivalent, this URL-receiver endpoint does not re-check any cap before inserting — the gallery cap lives on the UPLOAD endpoints via `routes/media/gallery-count.ts`. |
| `DELETE /api/v1/admin/events/{id}/media/{mediaId}` | `event/admin/removeMedia.ts` | none | - | n/a | Delete the binary then drop the row. HOS-390: relational content media. Admin write; PermissionEnum-gated (EVENT_UPDATE), enforced both by the route and by `checkEventCanEditMedia` in the service. NOT entitlement-gated; removing one's own photo is never a paid capability. |
| `PATCH /api/v1/admin/events/{id}/media/reorder` | `event/admin/reorderMedia.ts` | none | - | n/a | Reorder the gallery. HOS-390: relational content media. Admin write; PermissionEnum-gated (EVENT_UPDATE), enforced both by the route and by `checkEventCanEditMedia` in the service. NOT entitlement-gated; creates no gallery slot. |
| `PUT /api/v1/admin/events/{id}/media/{mediaId}/featured` | `event/admin/setFeaturedMedia.ts` | none | - | n/a | Set the featured photo. HOS-390: relational content media. Admin write; PermissionEnum-gated (EVENT_UPDATE), enforced both by the route and by `checkEventCanEditMedia` in the service. NOT entitlement-gated; creates no gallery slot. |
| `PATCH /api/v1/admin/events/{id}/media/{mediaId}` | `event/admin/updateMedia.ts` | none | - | n/a | HOS-1036: corrects the text of an already-uploaded photo (`caption`, `description`, `alt`, `attribution`); cannot touch `url`, `publicId`, `moderationState`, `state`, `isFeatured` or `sortOrder`. HOS-390: relational content media. Admin write; PermissionEnum-gated (EVENT_UPDATE), enforced both by the route and by `checkEventCanEditMedia` in the service. No entitlement gate — staff bypass entitlements (INV-6) and correcting misleading alt text is moderation, not a billable feature, mirroring the accommodation admin twin's reasoning (HOS-388). |
...
| `GET /api/v1/admin/host-trades/usages` | `host-trade/admin/usages.ts` | none | - | n/a | HOS-376 T-038, the abuse-triage screen. Staff bypass entitlements (INV-6), so the gate is `HOST_TRADE_USAGE_VIEW_ALL` only. The `creationChannel` filter is the sharp one: `EMAIL_LOOKUP` is the only channel a provider can drive without the host being present (R-5) |
| `POST /api/v1/admin/host-trades/{id}/declaration-suspension` | `host-trade/admin/usages.ts` | none | - | n/a | HOS-376 T-038. `HOST_TRADE_USAGE_MANAGE`. One endpoint for both halves of the decision, the body deciding which, so a screen cannot end up with a suspend button that silently lifts. Suspending requires a reason and stamps the acting admin — the automatic threshold suspension leaves that id NULL on purpose, and the two must stay distinguishable |
...
| `GET /api/v1/admin/posts/{id}/media` | `post/admin/getMedia.ts` | none | - | n/a | List the gallery. HOS-390: relational content media. Admin write; PermissionEnum-gated (POST_UPDATE), enforced both by the route and by `checkPostCanEditMedia` in the service. NOT entitlement-gated: posts and events carry no per-plan photo allowance — the accommodation `max_photos_per_accommodation` LimitKey has no editorial counterpart. This endpoint creates no gallery slot. |
| `POST /api/v1/admin/posts/{id}/media` | `post/admin/addMedia.ts` | none | - | n/a | Register an uploaded URL as a new row. HOS-390: relational content media. Admin write; PermissionEnum-gated (POST_UPDATE), enforced both by the route and by `checkPostCanEditMedia` in the service. NOT entitlement-gated (no editorial photo allowance exists). Like the commerce equivalent, this URL-receiver endpoint does not re-check any cap before inserting — the gallery cap lives on the UPLOAD endpoints via `routes/media/gallery-count.ts`. |
| `DELETE /api/v1/admin/posts/{id}/media/{mediaId}` | `post/admin/removeMedia.ts` | none | - | n/a | Delete the binary then drop the row. HOS-390: relational content media. Admin write; PermissionEnum-gated (POST_UPDATE), enforced both by the route and by `checkPostCanEditMedia` in the service. NOT entitlement-gated; removing one's own photo is never a paid capability. |
| `PATCH /api/v1/admin/posts/{id}/media/reorder` | `post/admin/reorderMedia.ts` | none | - | n/a | Reorder the gallery. HOS-390: relational content media. Admin write; PermissionEnum-gated (POST_UPDATE), enforced both by the route and by `checkPostCanEditMedia` in the service. NOT entitlement-gated; creates no gallery slot. |
| `PUT /api/v1/admin/posts/{id}/media/{mediaId}/featured` | `post/admin/setFeaturedMedia.ts` | none | - | n/a | Set the featured photo. HOS-390: relational content media. Admin write; PermissionEnum-gated (POST_UPDATE), enforced both by the route and by `checkPostCanEditMedia` in the service. NOT entitlement-gated; creates no gallery slot. |
| `PATCH /api/v1/admin/posts/{id}/media/{mediaId}` | `post/admin/updateMedia.ts` | none | - | n/a | HOS-1036: corrects the text of an already-uploaded photo (`caption`, `description`, `alt`, `attribution`); cannot touch `url`, `publicId`, `moderationState`, `state`, `isFeatured` or `sortOrder`. HOS-390: relational content media. Admin write; PermissionEnum-gated (POST_UPDATE), enforced both by the route and by `checkPostCanEditMedia` in the service. No entitlement gate — staff bypass entitlements (INV-6) and correcting misleading alt text is moderation, not a billable feature, mirroring the accommodation admin twin's reasoning (HOS-388). |
| **POST-TAG — ADMIN** | | | | | |
...
| `PUT /api/v1/admin/users/{id}/trusted-editor` | `user/admin/permissions.ts` | none | - | n/a | Admin write of the atomic trusted-editor bundle (HOS-374 OQ-1); PermissionEnum-gated (`PERMISSION_ASSIGN` + `PERMISSION_REVOKE` — one path, one middleware, so the declared gate is the enforced one). Staff-only surface — entitlements never apply |
...
| `POST /api/v1/admin/billing/subscriptions/{subscriptionId}/grant-courtesy` | `billing/admin/subscription-courtesy.ts` | none | - | n/a | Admin write; PermissionEnum.BILLING_MANAGE gated; gifts N free billing cycles by pausing the MP preapproval (HOS-180). NOT_FOUND→404 (including someone else's subscription — a 403 would confirm the id exists), ineligible/already-gifted/too-close-to-next-charge→422, MP refusal→502, billing unconfigured→503 |
| `POST /api/v1/admin/billing/subscriptions/grant-comp` | `billing/admin/subscription-comp.ts` | none | - | n/a | Admin write; PermissionEnum.BILLING_MANAGE gated; grants a permanently-complimentary (status=comp) subscription — the sibling of grant-courtesy, and since HOS-1171 the ONLY path that produces one (no promo code grants a comp). No entitlement gate: it CONFERS entitlements rather than consuming them, exactly like grant-courtesy. Retires every subscription of the customer that could still be charged and hard-cancels its MP preapproval first; unknown customer/plan→404, non-accommodation plan→422, customer already comped→409, MP refusal or unreachable→502 |
...
| `GET /api/v1/admin/billing/reconciliation/divergences` | `billing/admin/payment-reconciliation.ts` | none | - | n/a | HOS-765 admin read; PermissionEnum.BILLING_RECONCILIATION_MANAGE gated (its own permission, SUPER_ADMIN-only, NOT BILLING_MANAGE); staff bypass entitlements so no billing gate. Read-only: sweeps MercadoPago for divergences and proposes candidates, never links |
| `POST /api/v1/admin/billing/reconciliation/force-link` | `billing/admin/payment-reconciliation.ts` | none | - | n/a | HOS-765 admin mutation; PermissionEnum.BILLING_RECONCILIATION_MANAGE gated; binds billing_subscriptions.mp_subscription_id to an operator-named preapproval and writes a BILLING_MUTATION audit entry |
| `POST /api/v1/admin/billing/reconciliation/backfill-payment` | `billing/admin/payment-reconciliation.ts` | none | - | n/a | HOS-765 admin mutation; PermissionEnum.BILLING_RECONCILIATION_MANAGE gated; writes a billing_payments row for a settled MercadoPago charge and a BILLING_MUTATION audit entry. Idempotent on the MercadoPago payment id |
| `GET /api/v1/admin/billing/reconciliation/orphan-queue` | `billing/admin/orphan-payment-queue.ts` | none | - | n/a | HOS-1001 admin read; PermissionEnum.BILLING_RECONCILIATION_MANAGE gated (same permission as the sibling rescue verbs it feeds). Read-only and purely local — reads billing_orphan_payments, costs no MercadoPago calls, so unlike `/divergences` it carries an ordinary admin rate limit and may be polled |
| `POST /api/v1/admin/billing/reconciliation/orphan-queue/resolve` | `billing/admin/orphan-payment-queue.ts` | none | - | n/a | HOS-1001 admin mutation; PermissionEnum.BILLING_RECONCILIATION_MANAGE gated; closes one billing_orphan_payments row as resolved/dismissed with a required operator note and writes a BILLING_MUTATION audit entry. Moves no money and never reopens a row; the UPDATE is guarded on status=unresolved so a second operator gets 409 rather than overwriting the first one's note |
...
| `PATCH /api/v1/protected/gastronomies/{id}` | `gastronomy/protected/patch.ts` | gate | `edit_gastronomy_info` | wired | HOS-1074: `commerceVerticalEntitlementMiddleware('gastronomy')` + `requireEntitlement(EDIT_GASTRONOMY_INFO)`, the gastronomy mirror of `accommodation/protected/patch.ts`. The loader MUST precede the gate: the global entitlement middleware leaves the ACCOMMODATION set in the context, which never carries a commerce key, so a gate mounted alone refuses everyone. Ownership and the per-section COMMERCE_*_EDIT_OWN checks still run inside `updateOwn` — the permission says WHO may touch the row, the entitlement says whether their PLAN includes editing. Granted on all three gastronomy tiers, and floored from config so an owner mid-funnel (no subscription yet, DRAFT listing) is never locked out. |
...
| `GET /api/v1/protected/gastronomies/{id}/media` | `gastronomy/protected/getMedia.ts` | none | - | n/a | Read own gallery. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkGastronomyCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check. Gallery cap: `getGalleryCap(entityType)` is enforced server-side on the UPLOAD endpoints via `routes/media/gallery-count.ts` (which counts this vertical's relational rows), not here — this endpoint creates no new gallery slot. |
| `POST /api/v1/protected/gastronomies/{id}/media` | `gastronomy/protected/addMedia.ts` | none | - | n/a | Register an uploaded URL as a new row. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkGastronomyCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check. Gallery cap: `getGalleryCap(entityType)` IS enforced server-side, but on the UPLOAD endpoints (`protected/media/upload-entity`, `admin/media/upload`, `admin/media/import-stock`) via `routes/media/gallery-count.ts`, which counts this vertical's relational rows. This URL-receiver endpoint does NOT re-check it before inserting, so a caller that skips the upload step (posting an already-hosted URL, or the same URL repeatedly) can exceed the cap — there is no unique constraint on `url` either. Tracked separately. The accommodation equivalent additionally enforces a PLAN cap (`max_photos_per_accommodation`); commerce has no counterpart because its plan ships entitlements:[], so there is no LimitKey to check. |
| `POST /api/v1/protected/gastronomies/{{id}}/media/featured` | `gastronomy/protected/addFeaturedMedia.ts` | none | - | n/a | Upload straight to COVER (HOS-803). PermissionEnum-gated inside the service via `checkGastronomyCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check and no plan cap exists to exempt it from — the HOS-803 exemption that matters on the accommodation side has no counterpart here. The per-entity cap `getGalleryCap('gastronomy')` still applies to the gallery, and this route cannot move it: the row is created already featured and the cover it replaces is soft-deleted in the SAME transaction, so one row enters the featured slot and one leaves the table. That is what lets it skip the cap the sibling `addMedia` is subject to — the swap is quota-neutral by construction, not by exception. |
| `DELETE /api/v1/protected/gastronomies/{id}/media/{mediaId}` | `gastronomy/protected/removeMedia.ts` | none | - | n/a | Delete the binary then drop the row. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkGastronomyCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check. Gallery cap: `getGalleryCap(entityType)` is enforced server-side on the UPLOAD endpoints via `routes/media/gallery-count.ts` (which counts this vertical's relational rows), not here — this endpoint creates no new gallery slot. |
| `PATCH /api/v1/protected/gastronomies/{id}/media/reorder` | `gastronomy/protected/reorderMedia.ts` | none | - | n/a | Reorder the gallery. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkGastronomyCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check. Gallery cap: `getGalleryCap(entityType)` is enforced server-side on the UPLOAD endpoints via `routes/media/gallery-count.ts` (which counts this vertical's relational rows), not here — this endpoint creates no new gallery slot. |
| `PUT /api/v1/protected/gastronomies/{id}/media/{mediaId}/featured` | `gastronomy/protected/setFeaturedMedia.ts` | none | - | n/a | Set the featured photo. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkGastronomyCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check. Gallery cap: `getGalleryCap(entityType)` is enforced server-side on the UPLOAD endpoints via `routes/media/gallery-count.ts` (which counts this vertical's relational rows), not here — this endpoint creates no new gallery slot. |
| `PATCH /api/v1/protected/gastronomies/{id}/media/{mediaId}` | `gastronomy/protected/updateMedia.ts` | none | - | n/a | HOS-1036: corrects the text of an already-uploaded photo (`caption`, `description`, `alt`, `attribution`); cannot touch `url`, `publicId`, `moderationState`, `state`, `isFeatured` or `sortOrder`. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkGastronomyCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff), same gate as the sibling gallery mutations above. NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check, and correcting a photo's text consumes no gallery slot. |
| `GET /api/v1/protected/gastronomies/{id}/brochure` | `gastronomy/protected/brochure.ts` | gate | `download_listing_pdf` | wired | HOS-1058: owner-only printable A4 PDF of the listing's PUBLIC page (cover photo, hours, contact, services, QR back to the online ficha). `commerceVerticalEntitlementMiddleware('gastronomy')` loads the gastronomy-domain entitlements (the global entitlement middleware only ever carries the ACCOMMODATION set) and `requireEntitlement(DOWNLOAD_LISTING_PDF)` gates on it — granted by the premium gastronomy plan, not by `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (that map is the floor every tier gets). Ownership re-checked explicitly in the handler (`ownerId === actor.id \|\| COMMERCE_VIEW_ALL`), same 404 wording as `getById.ts` — a 403 would confirm the id exists. A listing that is not `VisibilityEnum.PUBLIC` also answers 404: there is no public ficha to print, and the QR would point at a dead page. |
| `GET /api/v1/protected/gastronomies/{id}/qr-sheet` | `gastronomy/protected/qrSheet.ts` | none | - | n/a | HOS-982: owner-only printable A4 PDF carrying a large QR that resolves to the listing's public page — designed to be taped to a door or left on a counter. **UNGATED ON PURPOSE (owner decision, 2026-09-07)**, and this row exists so that decision lives somewhere a reviewer looks rather than only in the route's own docblock. The neighbouring brochure requires `download_listing_pdf`; this sheet requires nothing but a published ficha, because a QR on a door brings people TO the platform — restricting it costs us, not the subscriber. Do not "harmonise" the two: one is a premium marketing asset handed to a customer, the other is an acquisition channel we want on every door. Frozen by `test/static-guards/listing-qr-sheet-no-entitlement-gate.guard.test.ts`, which fails if `requireEntitlement` or `EntitlementKey` ever appears in any of the three route files. What IS checked: authentication, then ownership (`ownerId === actor.id` \|\| `COMMERCE_VIEW_ALL`) answering 404 with the same wording as `getById.ts` — a 403 would confirm the id exists — then publication, `lifecycleState === ACTIVE && visibility === PUBLIC`, because a code printed for a listing with no public page 404s on every scan, permanently, on paper. Rate-limited to 20/min: minting writes a `qr_codes` row on the first call. |
| `GET /api/v1/protected/gastronomies/mine/views` | `gastronomy/protected/viewStats.ts` | gate | `view_basic_stats` | wired | HOS-734: basic view-count stats for the owner's own gastronomy listings, mirroring `views/protected/accommodations-me.ts`. `commerceVerticalEntitlementMiddleware('gastronomy')` loads the gastronomy-domain entitlements and `requireEntitlement(VIEW_BASIC_STATS)` gates on it — granted from `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (the floor every tier gets, unlike `download_listing_pdf` above which is premium-only). Ownership scoping has no `PermissionEnum` gate — `EntityViewService.getStatsForOwnCommerceListings` resolves owned listing IDs via `findIdsByOwnerId(actor.id)`, matching `listMine.ts`'s existing ownership-only convention for this vertical. |
| `GET /api/v1/protected/gastronomies/mine/views/daily-series` | `gastronomy/protected/viewStatsDailySeries.ts` | gate | `view_basic_stats` | wired | HOS-734: gap-filled daily view-count series for the owner's own gastronomy listings, mirroring `views/protected/daily-series.ts`. Same gate as `viewStats.ts` directly above (same entitlement, same ordering requirement), so both widgets are consistently accessible on the same plan tier. |
| `GET /api/v1/protected/gastronomies/{id}/menu` | `gastronomy/protected/getMenu.ts` | none | - | n/a | HOS-895: reads the listing's carta (sections + dishes) together with the uploaded photo/PDF and the external link. Deliberately NOT gated on `manage_gastronomy_menu`, unlike the PUT below: that key gates BUILDING a structured carta, not looking at one. Two ordinary cases need this open — a `-basico` owner opens the same editor page and must see the two ungated fallbacks with only the structured half refused, and an owner whose subscription lapsed after typing forty dishes is still reading their own data. Ownership enforced in the handler (`ownerId === actor.id \|\| COMMERCE_VIEW_ALL`), answering 404 with the same wording as `getById.ts` — a 403 would confirm the id exists. |
| `PUT /api/v1/protected/gastronomies/{id}/menu` | `gastronomy/protected/putMenu.ts` | gate | `manage_gastronomy_menu` | wired | HOS-895: replaces the whole carta in one transaction (an empty `sections` array deletes it, leaving the photo/PDF and the link untouched). `commerceVerticalEntitlementMiddleware('gastronomy')` loads the gastronomy-domain entitlements — the global entitlement middleware only ever carries the ACCOMMODATION set — and `requireEntitlement(MANAGE_GASTRONOMY_MENU)` gates on it. A TIER differentiator granted by the `gastronomy-pro` and `gastronomy-premium` plan ROWS, deliberately NOT by `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (that map is the floor every tier gets, so a paid capability there would be handed to `-basico` too). Ownership is enforced inside `replaceGastronomyMenu` via the same `COMMERCE_EDIT_OWN` / `COMMERCE_EDIT_ALL` gate the FAQ and media writes use. **HOS-1043**: `menuPayloadCarriesTranslations` inspects the SAME body and, when any section/dish names `nameI18n`/`descriptionI18n`, additionally requires `MULTILINGUAL_GASTRONOMY_MENU` (`gastronomy-premium` alone) — the same payload-conditional shape `menu_item_photos` uses one row down, but checked in THIS handler rather than a separate upload route, since a translation has no bytes of its own to upload. A `-pro` owner keeps writing an untranslated carta undisturbed. |
| `POST /api/v1/protected/gastronomies/{id}/menu-file` | `gastronomy/protected/uploadMenuFile.ts` | gate | `manage_gastronomy_menu` | wired | HOS-895 PR2 (owner decision, 2026-09-02, reversing PR1's ungated shipment): uploads a photo or PDF of the printed menu and writes `menu_file_url`/`menu_file_public_id`/`menu_file_kind` in the SAME request that stores the asset (HOS-372's rule). `commerceVerticalEntitlementMiddleware('gastronomy')` loads the gastronomy-domain entitlements before `requireEntitlement(MANAGE_GASTRONOMY_MENU)` gates on them — same pair and same order as the PUT above. The attachment is now `-pro`/`-premium` only, matching the structured carta; `menuUrl` (the external link) is the one fallback still free on every tier. Ownership checked BEFORE the body is parsed, answering 404, so a non-owner cannot spend the Cloudinary quota. Also carries a per-user sliding-window rate limit (10/min), ahead of the entitlement gate. |
| `DELETE /api/v1/protected/gastronomies/{id}/menu-file` | `gastronomy/protected/deleteMenuFile.ts` | gate | `manage_gastronomy_menu` | wired | HOS-895 PR2: same gate and same reason as the POST above — clears the three `menu_file_*` columns and deletes the stored asset via `menu_file_public_id`. A `-basico` owner can no longer clear an attachment uploaded before this gate existed; the public page already stops rendering it for them regardless (`resolveOwnerGrantsGastronomyMenuManagement`), and their withdraw path is the same downgrade that hid it. Ownership enforced in the handler, answering 404. The provider deletion is best-effort and runs AFTER the columns are cleared. |
| `POST /api/v1/protected/gastronomies/{id}/menu-item-photo` | `gastronomy/protected/uploadMenuItemPhoto.ts` | gate | `menu_item_photos` | wired | HOS-1045: uploads ONE dish photo and returns its delivery URL plus the Cloudinary public id. `commerceVerticalEntitlementMiddleware('gastronomy')` loads the gastronomy-domain entitlements before `requireEntitlement(MENU_ITEM_PHOTOS)` gates on them — same pair and same order as the two menu-file rows above, and for the same reason (the global entitlement middleware only ever carries the ACCOMMODATION set). A TIER differentiator granted by the `gastronomy-premium` plan ROW alone, deliberately NOT by `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (that map is the floor every tier gets) and deliberately NARROWER than `manage_gastronomy_menu`: `-pro` may type a carta, only `-premium` may put pictures on its dishes. **`gate`, not `gate+limit`** — HOS-1045 ships NO limit key and no `enforceXxxLimit()` call: access is the whole gate, because the limit engine resolves an unknown key as *unlimited* across five layers without raising, so a cap declared but not wired end to end behaves like no cap while reading in the catalogue like a promise. `MAX_PHOTOS_PER_ACCOMMODATION` is accommodation machinery and is not reused. Only `MENU_ITEM_PHOTOS` is required, not `MANAGE_GASTRONOMY_MENU` alongside it, so the route does not die the day a plan grants photos without the carta. Ownership checked BEFORE the body is parsed, answering 404, so a non-owner cannot spend the Cloudinary quota. Carries a per-user sliding-window rate limit (40/min — larger than the menu-file's 10 because photographing a twenty-dish carta is a genuine burst), ahead of the entitlement gate. NO row is written here: the dish it belongs to gets a fresh id on the next `PUT .../menu`, so the association travels in that document and this route persists nothing. |
| `GET /api/v1/protected/gastronomies/{id}/menu-qr` | `gastronomy/protected/menuQr.ts` | gate | `menu_qr_scan_metrics` | wired | HOS-1044: returns the venue's `purpose = MENU` QR (minting it via `QrCodeService.getOrCreateForEntity` on the FIRST call, reusing the same row on every later one — idempotent per `(entityType, entityId, purpose)`) plus its rendered SVG. `commerceVerticalEntitlementMiddleware('gastronomy')` loads the gastronomy-domain entitlements before `requireEntitlement(MENU_QR_SCAN_METRICS)` gates on them — same pair and same order as every other commerce route (HOS-1074: the global entitlement middleware only ever carries the ACCOMMODATION set). A TIER differentiator granted by the `gastronomy-premium` plan ROW alone, deliberately NOT by `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (that map is the floor every tier gets). Minting happens ONLY behind this gate, never on the public `/carta/` page read — the public page would otherwise let any visitor of a non-premium venue create a live `qr_codes` row as the side effect of a `GET`. Ownership checked via `getById` + explicit `ownerId === actor.id \|\| COMMERCE_VIEW_ALL`, answering 404 with the same wording as `getById.ts`/`brochure.ts` — a 403 would confirm the id exists. |
| `GET /api/v1/protected/gastronomies/{id}/menu-qr/scans` | `gastronomy/protected/menuQrScans.ts` | gate | `menu_qr_scan_metrics` | wired | HOS-1044 §6.4: aggregate read of `qr_code_scans` for the venue's `purpose = MENU` code — total, a gap-filled daily series, and breakdowns by `deviceType`/`os`/`browserLanguage` (a `NULL` derivation groups under the explicit `'unknown'` key, never dropped) — over a rolling window (`?window=7d\|30d`, default `30d`). Same gate pair and same order as `menuQr.ts` directly above (HOS-1074). Counting is by `qrCodeId`, never by the entity: a code's target can be repointed (a slug rename) and the scan history must survive that split. **Never mints** — the opposite of `menuQr.ts`'s own rule: a venue with no `MENU` code yet gets the all-zero aggregate (`buildEmptyQrCodeScanStats`) rather than a fresh row created as the side effect of opening the panel. Ownership checked the same way as `menuQr.ts` (404, never 403, on a foreign listing). |
| `GET /api/v1/protected/gastronomies/{id}/daily-specials` | `gastronomy/protected/getDailySpecials.ts` | none | - | n/a | HOS-1041: the owner's view of the menú del día. Deliberately NOT gated on `manage_gastronomy_daily_special`, unlike the PUT below and for the same reason `getMenu.ts` is open next to `putMenu.ts`: the key gates PUBLISHING a daily special, not looking at one. A `-basico` owner opens the same editor page and must see the panel with only the save refused, and an owner whose subscription lapsed after scheduling next week's plates is still reading their own rows. This read is also NOT filtered by the validity window — it returns scheduled and already-elapsed specials on purpose, because the editor must show the row the owner just saved for tomorrow; the public read (`public/getBySlug.ts`) is the one that filters to today. Ownership enforced in the handler (`ownerId === actor.id \|\| COMMERCE_VIEW_ALL`), answering 404 with the same wording as `getById.ts` — a 403 would confirm the id exists. |
| `PUT /api/v1/protected/gastronomies/{id}/daily-specials` | `gastronomy/protected/putDailySpecials.ts` | gate | `manage_gastronomy_daily_special` | wired | HOS-1041: replaces the whole menú del día in one transaction (an empty `specials` array takes it down — the manual escape hatch beside the automatic expiry). `commerceVerticalEntitlementMiddleware('gastronomy')` loads the gastronomy-domain entitlements — the global entitlement middleware only ever carries the ACCOMMODATION set — and `requireEntitlement(MANAGE_GASTRONOMY_DAILY_SPECIAL)` gates on it. A TIER differentiator granted by the `gastronomy-pro` and `gastronomy-premium` plan ROWS, deliberately NOT by `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (that map is the floor every tier gets, so a paid capability there would reach `-basico` too). A SEPARATE key from `manage_gastronomy_menu`: the carta is the year's menu, this is today's, and merging them would make it impossible to price them apart later without migrating live subscriptions. No limit key — the payload cap (`GASTRONOMY_DAILY_SPECIALS_MAX`) is a schema ceiling on one request, not a plan quota. Ownership is enforced inside `replaceGastronomyDailySpecials` via the same `COMMERCE_EDIT_OWN` / `COMMERCE_EDIT_ALL` gate the carta, FAQ and media writes use. |
| `GET /api/v1/protected/gastronomies/{id}/events` | `gastronomy/protected/getEvents.ts` | none | - | n/a | HOS-1042: reads the venue's OWN agenda — live music night, happy hour, dinner show — in display order, including entries the owner switched off. Deliberately NOT gated on `manage_gastronomy_events`, unlike the PUT below, and for ONE of `getMenu.ts`'s two reasons rather than both: there is no free fallback shape here, so a `-basico` owner simply gets an empty list, but an owner whose subscription lapsed after typing a season still owns those rows and refusing to show them back would be the platform holding their data hostage. Ownership enforced in the handler (`ownerId === actor.id \|\| COMMERCE_VIEW_ALL`), answering 404 with the same wording as `getById.ts` — a 403 would confirm the id exists. |
| `PUT /api/v1/protected/gastronomies/{id}/events` | `gastronomy/protected/putEvents.ts` | gate | `manage_gastronomy_events` | wired | HOS-1042: replaces the whole agenda in one transaction (an empty `events` array takes it down). Each entry is either dated (`recurrence: 'once'` + `date`) or weekly (`recurrence: 'weekly'` + `weekday` 0-6, Sunday-based); the two are mutually exclusive and a payload declaring neither or both is rejected by the schema refinement, since the nullable column pair cannot catch it. `commerceVerticalEntitlementMiddleware('gastronomy')` loads the gastronomy-domain entitlements — the global entitlement middleware only ever carries the ACCOMMODATION set — and `requireEntitlement(MANAGE_GASTRONOMY_EVENTS)` gates on it. A TIER differentiator granted by the `gastronomy-pro` and `gastronomy-premium` plan ROWS, deliberately NOT by `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (that map is the floor every tier gets, so a paid capability there would be handed to `-basico` too). Ownership is enforced inside `replaceGastronomyEvents` via the same `COMMERCE_EDIT_OWN` / `COMMERCE_EDIT_ALL` gate the FAQ, media and menu writes use. Gastronomy-only: an experience IS an event with a date, so it has no second agenda. |
| **COMMERCE — PROTECTED (self-checkout, HOS-166)** | | | | | |
| `POST /api/v1/protected/commerce/listings/gastronomy` | `commerce/protected/create.ts` | gate+limit | `publish_gastronomy`, `max_gastronomies` | wired | HOS-166 + HOS-687 + HOS-688: owner self-service listing creation. Auth-only — NO `requiredPermissions` and no COMMERCE_* check in the service's `_canCreate`, because this route is how an account BECOMES a commerce owner: it grants COMMERCE_OWNER inside the create transaction (`createForOwner`). Requiring COMMERCE_CREATE made the role unreachable. The ADMIN create route still requires COMMERCE_CREATE. Forces ownerId=actor.id, creates PRIVATE/DRAFT. LIMIT-gated per owner and per vertical (HOS-688): `commerceVerticalEntitlementMiddleware('gastronomy')` loads the gastronomy-domain limits (the global entitlement middleware loads the ACCOMMODATION domain's, which never carries this key) and `enforceGastronomyLimit()` reads them. ENTITLEMENT-gated since HOS-1074: `requireEntitlement(PUBLISH_GASTRONOMY)` now sits between the loader and the limit check, closing the one gap this stack had against accommodation's `requireEntitlement(PUBLISH_ACCOMMODATIONS)` + `enforceAccommodationLimit()` pair. The gate precedes the limit check so the counter is never queried for a caller who lacks the feature entirely. Diverges from `enforceAccommodationLimit` on one point: a count failure answers 503 rather than calling next(), because this is the ONLY gate on the create path and waving it through gives the product away silently. |
| `POST /api/v1/protected/commerce/listings/experience` | `commerce/protected/create.ts` | gate+limit | `publish_experience`, `max_experiences` | wired | HOS-166 + HOS-687 + HOS-688: same handler file as the gastronomy create above, experience vertical. Same gating (auth-only, COMMERCE_OWNER granted on create, ownerId forced, PRIVATE/DRAFT). Limit-gated on its OWN key — this route only ever loads and reads `max_experiences`, which is what makes the two caps count independently instead of sharing a pool (AC-13). HOS-1074: also entitlement-gated on `publish_experience`, so the two verticals are independent in BOTH halves — a gastronomy-only subscription grants nothing here. |
| `POST /api/v1/protected/commerce/listings/{entityType}/{entityId}/start-subscription` | `commerce/protected/start-subscription.ts` | none | - | n/a | HOS-166: owner-scoped commerce checkout (mirrors billing/start-paid). Auth + COMMERCE_EDIT_OWN + ownership check (actor.id===listing.ownerId else 403, AC-2); completeness 422; 409 if already subscribed. NOT entitlement-gated — visibility runs through the link table + reconciler, not the entitlement engine. Intentionally exempt from trial/pastDueGrace middlewares (cross-domain isolation, ADR-035). HOS-688: no longer binary. The route now forks three ways on the OWNER's subscription for this vertical — start one / attach with NO checkout (`appliedEffect: 'attached'`) / refuse with LIMIT_REACHED at the cap. The middle branch is what stops a second MercadoPago preapproval being created for a listing the owner's plan already covers (AC-14). Enforced in the handler rather than by a middleware, because the answer is a 201 with a different body, not a 403. |
| `POST /api/v1/protected/commerce/subscriptions/{entityType}/change-plan` | `commerce/protected/change-plan.ts` | none | - | n/a | HOS-1119: owner-scoped tier change for a commerce vertical, the counterpart to the checkout tier picker. NOT entitlement-gated, and for the same reason as its sibling `start-subscription.ts` above: what a commerce tier buys is resolved through the plan row and the link-table reconciler, not through the entitlement engine, so there is no key to require here. Auth + `COMMERCE_EDIT_OWN` come from the router middleware, which runs BEFORE `idempotencyKeyMiddleware` on purpose — reversing them would let an unauthenticated caller burn idempotency keys. Ownership is checked in the handler (the caller's own subscription for that vertical), not by a permission. Deliberately its OWN route rather than `billing/plan-change.ts`: that handler picks the first live subscription with no domain predicate, so for an account that is both host and restaurateur a gastronomy tier change would land on whichever row came back first. Both directions since HOS-1122 — the sentence here used to read "upgrades only, because the downgrade path runs through accommodation-only restriction code whose unknown limit keys resolve as unlimited", and both halves of that reason have since been removed: the cron dispatches a scheduled downgrade on its target plan's own product domain, and `applyDowngradeRestrictions` refuses a slug from any other one. A cheaper target is now scheduled for period end; only an equally-priced one still answers 422. |
| `GET /api/v1/protected/commerce/subscriptions/{entityType}/downgrade-preview` | `commerce/protected/downgrade-preview.ts` | none | - | n/a | HOS-1122: read-only preview of which listings a cheaper tier would stop covering — nothing is scheduled, charged or mutated. `none` for two independent reasons, either of which would be enough. First, it is the same reason its sibling `change-plan.ts` above carries: what a commerce tier buys is resolved through the plan row and the link-table reconciler, not through the entitlement engine, so there is no key to require. Second, it mutates nothing, which is the precedent `host-onboarding/protected/precheck.ts` sets — a read that informs a decision is not itself gated; the gate belongs on the route that acts. Auth + `COMMERCE_EDIT_OWN` come from the router middleware, and the subscription is resolved from `ctx.billingCustomerId` through `findOwnerVerticalSubscription`, never from a caller-supplied id, so there is no IDOR surface for a permission to have to cover. NOT limit-gated either: it READS a cap to describe it, and enforcing one on a read would refuse to tell an owner what they are about to lose. One deliberate non-default: a tier whose listing cap cannot be resolved answers 422 rather than an empty preview, because "nothing is over the cap" is exactly what an unresolved cap looks like from the outside (HOS-1078). |
| `GET /api/v1/protected/commerce/subscriptions/{entityType}/trial-verdict` | `commerce/protected/trial-verdict.ts` | none | - | n/a | HOS-1184: read-only answer to "what would publishing in this vertical do right now" — one of `trial_available` / `has_active_sub` / `payment_required`. `none` for the same two independent reasons its sibling `downgrade-preview.ts` above carries: what a commerce tier buys is resolved through the plan row and the link-table reconciler rather than the entitlement engine, so there is no key to require; and it mutates nothing, which is the `host-onboarding/protected/precheck.ts` precedent — a read that informs a decision is not itself gated, the gate belongs on the route that acts (here `start-subscription.ts`, whose own branch re-checks eligibility before granting). Auth + `COMMERCE_EDIT_OWN` come from the router middleware, and the verdict is resolved from `ctx.billingCustomerId`, never from a caller-supplied id, so there is no IDOR surface. NOT limit-gated: it describes what publishing would cost, and refusing to answer at the cap would hide the checkout from exactly the owner who needs to see it. One deliberate non-default, and it is the opposite of the sibling's: a caller with NO billing customer answers 200 with a verdict, not the 404 `downgrade-preview` gives. A preview of a subscription that does not exist has nothing to describe; an owner with no billing history has a perfectly definite verdict — they are the one owner guaranteed not to have spent their trial, and a 404 would render the brand-new owner as "Publicar y pagar". |
| `DELETE /api/v1/protected/commerce/listings/{vertical}/{id}` | `commerce/protected/delete-draft.ts` | none | - | n/a | HOS-1156 T-015 / AC-14: the owner discards one of their OWN DRAFT listings, which is the free way past a full plan the publish precheck panel offers next to the paid ones. Auth-only and no `requiredPermissions`, same posture as the owner-create routes above and for the same HOS-687 reason: `COMMERCE_OWNER` is granted BY creating a listing, so gating the owner's own draft behind a commerce permission locks out the accounts this flow exists for. Ownership is checked in the service (`softDeleteOwnDraft`), which also refuses anything that is not `lifecycleState: DRAFT`. Deliberately NOT limit-gated: it FREES a slot rather than consuming one. A row that does not exist, belongs to somebody else, or is already deleted all answer 404 — never 403, which would confirm the id exists. |
| **GASTRONOMY — ADMIN** | | | | | |
...
| `GET /api/v1/admin/gastronomies/{id}/media` | `gastronomy/admin/getMedia.ts` | none | - | n/a | Read own gallery. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkGastronomyCanEditMedia` in the service. |
| `POST /api/v1/admin/gastronomies/{id}/media` | `gastronomy/admin/addMedia.ts` | none | - | n/a | Register an uploaded URL as a new row. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkGastronomyCanEditMedia` in the service. Same missing cap re-check as the protected add path: the cap lives on the upload endpoints, not here. |
| `POST /api/v1/admin/gastronomies/{{id}}/media/featured` | `gastronomy/admin/addFeaturedMedia.ts` | none | - | n/a | Upload straight to COVER (HOS-803). Admin write; PermissionEnum-gated inside the service via `checkGastronomyCanEditMedia` — COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check and no plan cap exists to exempt it from — the HOS-803 exemption that matters on the accommodation side has no counterpart here. The per-entity cap `getGalleryCap('gastronomy')` still applies to the gallery, and this route cannot move it: the row is created already featured and the cover it replaces is soft-deleted in the SAME transaction, so one row enters the featured slot and one leaves the table. That is what lets it skip the cap the sibling `addMedia` is subject to — the swap is quota-neutral by construction, not by exception. |
| `DELETE /api/v1/admin/gastronomies/{id}/media/{mediaId}` | `gastronomy/admin/removeMedia.ts` | none | - | n/a | Delete the binary then drop the row. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkGastronomyCanEditMedia` in the service. |
| `PATCH /api/v1/admin/gastronomies/{id}/media/reorder` | `gastronomy/admin/reorderMedia.ts` | none | - | n/a | Reorder the gallery. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkGastronomyCanEditMedia` in the service. |
| `PUT /api/v1/admin/gastronomies/{id}/media/{mediaId}/featured` | `gastronomy/admin/setFeaturedMedia.ts` | none | - | n/a | Set the featured photo. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkGastronomyCanEditMedia` in the service. |
| `PATCH /api/v1/admin/gastronomies/{id}/media/{mediaId}` | `gastronomy/admin/updateMedia.ts` | none | - | n/a | HOS-1036: corrects the text of an already-uploaded photo (`caption`, `description`, `alt`, `attribution`); cannot touch `url`, `publicId`, `moderationState`, `state`, `isFeatured` or `sortOrder`. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkGastronomyCanEditMedia` in the service. No entitlement gate — staff bypass entitlements (INV-6) and correcting misleading alt text is moderation, not a billable feature, mirroring the accommodation admin twin's reasoning (HOS-388). |
...
| `PATCH /api/v1/protected/experiences/{id}` | `experience/protected/patch.ts` | gate | `edit_experience_info` | wired | HOS-1074: `commerceVerticalEntitlementMiddleware('experience')` + `requireEntitlement(EDIT_EXPERIENCE_INFO)`. See the gastronomy PATCH row above for the ordering invariant and for why the permission check and the entitlement gate both stay. Gated on its OWN vertical's key, so a gastronomy-only subscription grants nothing here. HOS-1049 adds a second, narrower FIELD gate inside the handler: a body naming `meetingPointDirections` also requires `manage_experience_directions` (`experience-pro` and up). It is not a second `requireEntitlement` in `options.middlewares` because every other field on this body is free on `-basico`, and a route-wide gate would lock those providers out of their own listing. |
...
| `GET /api/v1/protected/experiences/{id}/media` | `experience/protected/getMedia.ts` | none | - | n/a | Read own gallery. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkExperienceCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check. Gallery cap: `getGalleryCap(entityType)` is enforced server-side on the UPLOAD endpoints via `routes/media/gallery-count.ts` (which counts this vertical's relational rows), not here — this endpoint creates no new gallery slot. |
| `POST /api/v1/protected/experiences/{id}/media` | `experience/protected/addMedia.ts` | none | - | n/a | Register an uploaded URL as a new row. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkExperienceCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check. Gallery cap: `getGalleryCap(entityType)` IS enforced server-side, but on the UPLOAD endpoints (`protected/media/upload-entity`, `admin/media/upload`, `admin/media/import-stock`) via `routes/media/gallery-count.ts`, which counts this vertical's relational rows. This URL-receiver endpoint does NOT re-check it before inserting, so a caller that skips the upload step (posting an already-hosted URL, or the same URL repeatedly) can exceed the cap — there is no unique constraint on `url` either. Tracked separately. The accommodation equivalent additionally enforces a PLAN cap (`max_photos_per_accommodation`); commerce has no counterpart because its plan ships entitlements:[], so there is no LimitKey to check. |
| `POST /api/v1/protected/experiences/{{id}}/media/featured` | `experience/protected/addFeaturedMedia.ts` | none | - | n/a | Upload straight to COVER (HOS-803). PermissionEnum-gated inside the service via `checkExperienceCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check and no plan cap exists to exempt it from — the HOS-803 exemption that matters on the accommodation side has no counterpart here. The per-entity cap `getGalleryCap('experience')` still applies to the gallery, and this route cannot move it: the row is created already featured and the cover it replaces is soft-deleted in the SAME transaction, so one row enters the featured slot and one leaves the table. That is what lets it skip the cap the sibling `addMedia` is subject to — the swap is quota-neutral by construction, not by exception. |
| `DELETE /api/v1/protected/experiences/{id}/media/{mediaId}` | `experience/protected/removeMedia.ts` | none | - | n/a | Delete the binary then drop the row. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkExperienceCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check. Gallery cap: `getGalleryCap(entityType)` is enforced server-side on the UPLOAD endpoints via `routes/media/gallery-count.ts` (which counts this vertical's relational rows), not here — this endpoint creates no new gallery slot. |
| `PATCH /api/v1/protected/experiences/{id}/media/reorder` | `experience/protected/reorderMedia.ts` | none | - | n/a | Reorder the gallery. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkExperienceCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check. Gallery cap: `getGalleryCap(entityType)` is enforced server-side on the UPLOAD endpoints via `routes/media/gallery-count.ts` (which counts this vertical's relational rows), not here — this endpoint creates no new gallery slot. |
| `PUT /api/v1/protected/experiences/{id}/media/{mediaId}/featured` | `experience/protected/setFeaturedMedia.ts` | none | - | n/a | Set the featured photo. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkExperienceCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check. Gallery cap: `getGalleryCap(entityType)` is enforced server-side on the UPLOAD endpoints via `routes/media/gallery-count.ts` (which counts this vertical's relational rows), not here — this endpoint creates no new gallery slot. |
| `PATCH /api/v1/protected/experiences/{id}/media/{mediaId}` | `experience/protected/updateMedia.ts` | none | - | n/a | HOS-1036: corrects the text of an already-uploaded photo (`caption`, `description`, `alt`, `attribution`); cannot touch `url`, `publicId`, `moderationState`, `state`, `isFeatured` or `sortOrder`. HOS-372: relational commerce media. PermissionEnum-gated inside the service via `checkExperienceCanEditMedia` — COMMERCE_EDIT_OWN (listing owner) or COMMERCE_EDIT_ALL (staff), same gate as the sibling gallery mutations above. NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check, and correcting a photo's text consumes no gallery slot. |
| `GET /api/v1/protected/experiences/{id}/brochure` | `experience/protected/brochure.ts` | gate | `download_listing_pdf` | wired | HOS-1058: owner-only printable A4 PDF of the listing's PUBLIC page (cover photo, meeting point, hours, contact, services, QR back to the online ficha). `commerceVerticalEntitlementMiddleware('experience')` loads the experience-domain entitlements and `requireEntitlement(DOWNLOAD_LISTING_PDF)` gates on it — granted by the premium experience plan, gated separately from gastronomy's (R-1: no shared "commerce" plan grants it once). Ownership re-checked explicitly in the handler (`ownerId === actor.id \|\| COMMERCE_VIEW_ALL`), same 404 wording as `getById.ts` — a 403 would confirm the id exists. A listing that is not `VisibilityEnum.PUBLIC` also answers 404: there is no public ficha to print, and the QR would point at a dead page. |
| `GET /api/v1/protected/experiences/{id}/qr-sheet` | `experience/protected/qrSheet.ts` | none | - | n/a | HOS-982: owner-only printable A4 PDF carrying a large QR that resolves to the listing's public page — designed to be taped to a door or left on a counter. **UNGATED ON PURPOSE (owner decision, 2026-09-07)**, and this row exists so that decision lives somewhere a reviewer looks rather than only in the route's own docblock. The neighbouring brochure requires `download_listing_pdf`; this sheet requires nothing but a published ficha, because a QR on a door brings people TO the platform — restricting it costs us, not the subscriber. Do not "harmonise" the two: one is a premium marketing asset handed to a customer, the other is an acquisition channel we want on every door. Frozen by `test/static-guards/listing-qr-sheet-no-entitlement-gate.guard.test.ts`, which fails if `requireEntitlement` or `EntitlementKey` ever appears in any of the three route files. What IS checked: authentication, then ownership (`ownerId === actor.id` \|\| `COMMERCE_VIEW_ALL`) answering 404 with the same wording as `getById.ts` — a 403 would confirm the id exists — then publication, `lifecycleState === ACTIVE && visibility === PUBLIC`, because a code printed for a listing with no public page 404s on every scan, permanently, on paper. Rate-limited to 20/min: minting writes a `qr_codes` row on the first call. |
| `POST /api/v1/protected/experiences/{id}/certificates` | `experience/protected/certificates.ts` | gate | `issue_experience_certificate` | wired | HOS-1057: the certificate a provider issues to whoever did the experience. `commerceVerticalEntitlementMiddleware('experience')` loads the experience-domain entitlements (the global entitlement middleware only ever carries the ACCOMMODATION set) and `requireEntitlement(ISSUE_EXPERIENCE_CERTIFICATE)` gates on it — granted by `experience-pro` and upwards, NOT by `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (that map is the floor every tier gets, and `experience-basico` is the only sellable experience tier). Ownership re-checked explicitly in the handler (`ownerId === actor.id \| COMMERCE_VIEW_ALL`), same 404 wording as `getById.ts` — a 403 would confirm the id exists. The recipient's name is provider-typed free text, bounded and control-character-free by `ExperienceCertificateRecipientNameSchema`. |
| `GET /api/v1/protected/experiences/{id}/certificates` | `experience/protected/certificates.ts` | gate | `issue_experience_certificate` | wired | HOS-1057: the certificate a provider issues to whoever did the experience. `commerceVerticalEntitlementMiddleware('experience')` loads the experience-domain entitlements (the global entitlement middleware only ever carries the ACCOMMODATION set) and `requireEntitlement(ISSUE_EXPERIENCE_CERTIFICATE)` gates on it — granted by `experience-pro` and upwards, NOT by `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (that map is the floor every tier gets, and `experience-basico` is the only sellable experience tier). Ownership re-checked explicitly in the handler (`ownerId === actor.id \| COMMERCE_VIEW_ALL`), same 404 wording as `getById.ts` — a 403 would confirm the id exists. Reads back only the caller's own listing's certificates; there is no public certificate URL (see `services/experience-certificate/certificate-response.ts` for that decision). |
| `GET /api/v1/protected/experiences/{id}/certificates/{certificateId}/pdf` | `experience/protected/certificates.ts` | gate | `issue_experience_certificate` | wired | HOS-1057: the certificate a provider issues to whoever did the experience. `commerceVerticalEntitlementMiddleware('experience')` loads the experience-domain entitlements (the global entitlement middleware only ever carries the ACCOMMODATION set) and `requireEntitlement(ISSUE_EXPERIENCE_CERTIFICATE)` gates on it — granted by `experience-pro` and upwards, NOT by `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (that map is the floor every tier gets, and `experience-basico` is the only sellable experience tier). Ownership re-checked explicitly in the handler (`ownerId === actor.id \| COMMERCE_VIEW_ALL`), same 404 wording as `getById.ts` — a 403 would confirm the id exists. A listing that is not `VisibilityEnum.PUBLIC` also answers 404: the sheet carries a QR back to the public ficha, and printing one for an unpublished listing puts a permanent 404 on paper. |
| `GET /api/v1/protected/experiences/mine/views` | `experience/protected/viewStats.ts` | gate | `view_basic_stats` | wired | HOS-734: basic view-count stats for the owner's own experience listings, mirroring `views/protected/accommodations-me.ts`. `commerceVerticalEntitlementMiddleware('experience')` loads the experience-domain entitlements and `requireEntitlement(VIEW_BASIC_STATS)` gates on it — granted from `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (the floor every tier gets, unlike `download_listing_pdf` above which is premium-only). Ownership scoping has no `PermissionEnum` gate — `EntityViewService.getStatsForOwnCommerceListings` resolves owned listing IDs via `findIdsByOwnerId(actor.id)`, matching `listMine.ts`'s existing ownership-only convention for this vertical. |
| `GET /api/v1/protected/experiences/mine/views/daily-series` | `experience/protected/viewStatsDailySeries.ts` | gate | `view_basic_stats` | wired | HOS-734: gap-filled daily view-count series for the owner's own experience listings, mirroring `views/protected/daily-series.ts`. Same gate as `viewStats.ts` directly above (same entitlement, same ordering requirement), so both widgets are consistently accessible on the same plan tier. |
| **EXPERIENCES — ADMIN** | | | | | |
...
| `GET /api/v1/admin/experiences/{id}/media` | `experience/admin/getMedia.ts` | none | - | n/a | Read own gallery. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkExperienceCanEditMedia` in the service. |
| `POST /api/v1/admin/experiences/{id}/media` | `experience/admin/addMedia.ts` | none | - | n/a | Register an uploaded URL as a new row. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkExperienceCanEditMedia` in the service. Same missing cap re-check as the protected add path: the cap lives on the upload endpoints, not here. |
| `POST /api/v1/admin/experiences/{{id}}/media/featured` | `experience/admin/addFeaturedMedia.ts` | none | - | n/a | Upload straight to COVER (HOS-803). Admin write; PermissionEnum-gated inside the service via `checkExperienceCanEditMedia` — COMMERCE_EDIT_ALL (staff). NOT entitlement-gated: commerce is a separate billing domain whose plan ships entitlements:[] (ADR-035), so there is no key to check and no plan cap exists to exempt it from — the HOS-803 exemption that matters on the accommodation side has no counterpart here. The per-entity cap `getGalleryCap('experience')` still applies to the gallery, and this route cannot move it: the row is created already featured and the cover it replaces is soft-deleted in the SAME transaction, so one row enters the featured slot and one leaves the table. That is what lets it skip the cap the sibling `addMedia` is subject to — the swap is quota-neutral by construction, not by exception. |
| `DELETE /api/v1/admin/experiences/{id}/media/{mediaId}` | `experience/admin/removeMedia.ts` | none | - | n/a | Delete the binary then drop the row. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkExperienceCanEditMedia` in the service. |
| `PATCH /api/v1/admin/experiences/{id}/media/reorder` | `experience/admin/reorderMedia.ts` | none | - | n/a | Reorder the gallery. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkExperienceCanEditMedia` in the service. |
| `PUT /api/v1/admin/experiences/{id}/media/{mediaId}/featured` | `experience/admin/setFeaturedMedia.ts` | none | - | n/a | Set the featured photo. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkExperienceCanEditMedia` in the service. |
| `PATCH /api/v1/admin/experiences/{id}/media/{mediaId}` | `experience/admin/updateMedia.ts` | none | - | n/a | HOS-1036: corrects the text of an already-uploaded photo (`caption`, `description`, `alt`, `attribution`); cannot touch `url`, `publicId`, `moderationState`, `state`, `isFeatured` or `sortOrder`. HOS-372: relational commerce media. Admin write; PermissionEnum-gated (COMMERCE_EDIT_ALL), enforced both by the route and by `checkExperienceCanEditMedia` in the service. No entitlement gate — staff bypass entitlements (INV-6) and correcting misleading alt text is moderation, not a billable feature, mirroring the accommodation admin twin's reasoning (HOS-388). |
...
| `POST /api/v1/admin/alliance/leads/{id}/approve-and-provision-partner` | `alliance/admin/approve-and-provision-partner.ts` | none | - | n/a | HOS-278 §6.5 OQ-1: admin combined approve+provision for `partner` leads only, kept SEPARATE from mark-handled so approving still provisions nothing. Idempotent via `lead.provisioned_partner_id`. No entitlement gate — the actor is staff (INV-6) and the partner created is dormant (DRAFT + pending), invisible publicly until it pays. PermissionEnum-gated (ALLIANCE_LEAD_MANAGE) |
...
| `POST /api/v1/protected/alliance/leads/{id}/claim` | `alliance/protected/claim.ts` | none | - | n/a | HOS-278 AC-4: redeems the single-use token from a claim invitation email. Auth-only — the token plus the caller's own email address ARE the authorization; a permission would gate the wrong thing (the person confirming is an ordinary user, not staff). Every rejection answers the same 404 so the endpoint is not an oracle for which lead ids exist. Same `pastDueGraceMiddleware` prefix exemption as the tier above. |
...
| `GET /api/v1/admin/partners/{partnerId}/mentions` | `partners/admin/mentions/list.ts` | none | - | n/a | Admin read of one partner's manual-promotion log; PermissionEnum-gated (PARTNER_MANAGE) (HOS-377). Includes the admin-only `internalNote`, which the partner-facing route strips |
| `POST /api/v1/admin/partners/{partnerId}/mentions` | `partners/admin/mentions/create.ts` | none | - | n/a | Admin write; PermissionEnum-gated (PARTNER_MANAGE) (HOS-377). Logs N channels in one transaction under a server-generated `batchId` and notifies the partner once |
| `PATCH /api/v1/admin/partners/{partnerId}/mentions/{id}` | `partners/admin/mentions/update.ts` | none | - | n/a | Admin correction; PermissionEnum-gated (PARTNER_MANAGE) (HOS-377). The owning partner and the batch are not patchable, and the service checks the mention really belongs to the partner in the path |
| `DELETE /api/v1/admin/partners/{partnerId}/mentions/{id}` | `partners/admin/mentions/delete.ts` | none | - | n/a | Admin soft-delete only; PermissionEnum-gated (PARTNER_MANAGE) (HOS-377). A mention announced by email must stay on the row, marked removed, rather than vanish |
