...
| `POST /api/v1/protected/accommodations/{id}/calendar-sync/sync` | `accommodation/protected/calendarSync.ts` | gate | `can_sync_external_calendar` | wired | HOS-157 Phase 2. requireEntitlement(CAN_SYNC_EXTERNAL_CALENDAR) + ACCOMMODATION_OCCUPANCY_MANAGE + ownership. On-demand sync; operational failures return status=error, not 5xx. |
| `GET /api/v1/protected/accommodations/{id}/calendar-sync/status` | `accommodation/protected/calendarSyncStatus.ts` | none | - | n/a | HOS-157 Phase 2. Read of connection sync health (safe projection). Ownership only, no entitlement — parity with the occupancy read route so a downgraded host can still see/clean up. |
| `DELETE /api/v1/protected/accommodations/{id}/calendar-sync/{provider}` | `accommodation/protected/calendarDisconnect.ts` | none | - | n/a | HOS-157 Phase 2. Soft-disconnect (isActive=false). ACCOMMODATION_OCCUPANCY_MANAGE + ownership; deliberately NO entitlement gate so a downgraded host can remove a stale connection. |
