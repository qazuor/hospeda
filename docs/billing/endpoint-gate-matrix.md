...
| `GET /api/v1/protected/gastronomies/{id}/menu-qr` | `gastronomy/protected/menuQr.ts` | gate | `menu_qr_analytics` | wired | HOS-1044: returns the venue's `purpose = MENU` QR (minting it via `QrCodeService.getOrCreateForEntity` on the FIRST call, reusing the same row on every later one — idempotent per `(entityType, entityId, purpose)`) plus its rendered SVG. `commerceVerticalEntitlementMiddleware('gastronomy')` loads the gastronomy-domain entitlements before `requireEntitlement(MENU_QR_ANALYTICS)` gates on them — same pair and same order as every other commerce route (HOS-1074: the global entitlement middleware only ever carries the ACCOMMODATION set). A TIER differentiator granted by the `gastronomy-premium` plan ROW alone, deliberately NOT by `ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL` (that map is the floor every tier gets). Minting happens ONLY behind this gate, never on the public `/carta/` page read — the public page would otherwise let any visitor of a non-premium venue create a live `qr_codes` row as the side effect of a `GET`. Ownership checked via `getById` + explicit `ownerId === actor.id \|\| COMMERCE_VIEW_ALL`, answering 404 with the same wording as `getById.ts`/`brochure.ts` — a 403 would confirm the id exists. |
| `GET /api/v1/protected/gastronomies/{id}/menu-qr/scans` | `gastronomy/protected/menuQrScans.ts` | gate | `menu_qr_analytics` | wired | HOS-1044 §6.4: aggregate read of `qr_code_scans` for the venue's `purpose = MENU` code — total, a gap-filled daily series, and breakdowns by `deviceType`/`os`/`browserLanguage` (a `NULL` derivation groups under the explicit `'unknown'` key, never dropped) — over a rolling window (`?window=7d\|30d`, default `30d`). Same gate pair and same order as `menuQr.ts` directly above (HOS-1074). Counting is by `qrCodeId`, never by the entity: a code's target can be repointed (a slug rename) and the scan history must survive that split. **Never mints** — the opposite of `menuQr.ts`'s own rule: a venue with no `MENU` code yet gets the all-zero aggregate (`buildEmptyQrCodeScanStats`) rather than a fresh row created as the side effect of opening the panel. Ownership checked the same way as `menuQr.ts` (404, never 403, on a foreign listing). |
