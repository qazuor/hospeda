...
| `GET /api/v1/protected/commerce/leads/mine` | `commerce/protected/my-lead.ts` | none | - | n/a | HOS-257: owner self-service pre-fill read. Auth-only, no COMMERCE_* permission — scoped to the caller's OWN `commerce_leads` row via `provisionedUserId = actor.id`, so there is nothing an elevated permission would unlock. Returns `{ lead: null }` (never an error/404) when the caller has no provisioned lead — pre-fill convenience only, never a gate (D-4). |
| **GASTRONOMY — ADMIN** | | | | | |
