...
| `POST /api/v1/protected/commerce/listings/gastronomy` | `commerce/protected/create.ts` | none | `max_gastronomies` | `enforceGastronomyLimit` | HOS-166 + HOS-687 + HOS-688: owner self-service listing creation. Auth-only — NO `requiredPermissions` and no COMMERCE_* check in the service's `_canCreate`, because this route is how an account BECOMES a commerce owner: it grants COMMERCE_OWNER inside the create transaction (`createForOwner`). Requiring COMMERCE_CREATE made the role unreachable. The ADMIN create route still requires COMMERCE_CREATE. Forces ownerId=actor.id, creates PRIVATE/DRAFT. LIMIT-gated per owner and per vertical (HOS-688): `commerceVerticalEntitlementMiddleware('gastronomy')` loads the gastronomy-domain limits (the global entitlement middleware loads the ACCOMMODATION domain's, which never carries this key) and `enforceGastronomyLimit()` reads them. Still NOT entitlement-gated: neither vertical grants an entitlement (§6.8), so unlike accommodation there is no `requireEntitlement` ahead of the limit check. Diverges from `enforceAccommodationLimit` on one point: a count failure answers 503 rather than calling next(), because this is the ONLY gate on the create path and waving it through gives the product away silently. |
| `POST /api/v1/protected/commerce/listings/experience` | `commerce/protected/create.ts` | none | `max_experiences` | `enforceExperienceLimit` | HOS-166 + HOS-687 + HOS-688: same handler file as the gastronomy create above, experience vertical. Same gating (auth-only, COMMERCE_OWNER granted on create, ownerId forced, PRIVATE/DRAFT). Limit-gated on its OWN key — this route only ever loads and reads `max_experiences`, which is what makes the two caps count independently instead of sharing a pool (AC-13). |
