...
| `POST /api/v1/protected/commerce/listings/gastronomy` | `commerce/protected/create.ts` | none | - | n/a | HOS-166 + HOS-687: owner self-service listing creation. Auth-only — NO `requiredPermissions` and no COMMERCE_* check in the service's `_canCreate`, because this route is how an account BECOMES a commerce owner: it grants COMMERCE_OWNER inside the create transaction (`createForOwner`). Requiring COMMERCE_CREATE made the role unreachable. Forces ownerId=actor.id, creates PRIVATE/DRAFT. The ADMIN create route still requires COMMERCE_CREATE. NOT entitlement-gated — commerce visibility is driven by the link table + reconciler, not the entitlement engine (the commerce-listing plan ships entitlements:[]). |
| `POST /api/v1/protected/commerce/listings/experience` | `commerce/protected/create.ts` | none | - | n/a | HOS-166 + HOS-687: same handler file as the gastronomy create above, experience vertical. Same gating (auth-only, COMMERCE_OWNER granted on create, ownerId forced, PRIVATE/DRAFT). |
