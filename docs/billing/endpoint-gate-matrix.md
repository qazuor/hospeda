...
| `PATCH /api/v1/protected/accommodations/{id}/featured-toggle` | `accommodation/protected/featured-toggle.ts` | gate | `featured_listing` | wired | SPEC-309 T-019. Owner self-service `isFeatured` toggle. NOT `requireEntitlement()` middleware — gated inline in `setAccommodationFeaturedToggle` via `resolveOwnerPlanGrantsFeatured` (plan) OR `resolveAccommodationHasActiveFeaturedAddon` (accommodation-scoped addon); 403 when neither source currently grants FEATURED_LISTING. |
| `GET /api/v1/protected/accommodations/{id}/featured-toggle` | `accommodation/protected/featured-toggle.ts` | none | - | n/a | SPEC-309 T-020. Read-only counterpart: returns the current `isFeatured` value and whether the gate above would currently allow the PATCH. Reports gate status, does not itself enforce one; auth + ownership check in handler. |
