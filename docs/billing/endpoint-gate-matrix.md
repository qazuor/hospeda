...
| `POST /api/v1/protected/gastronomies/{id}/menu-file` | `gastronomy/protected/uploadMenuFile.ts` | none | - | n/a | HOS-895: uploads a photo or PDF of the printed menu and writes `menu_file_url`/`menu_file_public_id`/`menu_file_kind` in the SAME request that stores the asset (HOS-372's rule: uploading to Cloudinary and deferring the association is how an abandoned form leaves an asset billing with nothing pointing at it). Ungated on purpose — the uploaded menu is how a `-basico` venue shows a menu at all, and gating it would take away something every gastronomy tier has had since SPEC-239; only the STRUCTURED carta is the paid capability. Ownership checked BEFORE the body is parsed, answering 404, so a non-owner cannot spend the Cloudinary quota. Carries a per-user sliding-window rate limit (10/min) rather than an entitlement or a limit key. |
| `DELETE /api/v1/protected/gastronomies/{id}/menu-file` | `gastronomy/protected/deleteMenuFile.ts` | none | - | n/a | HOS-895: clears the three `menu_file_*` columns and deletes the stored asset via `menu_file_public_id`. Ungated for the same reason as the POST above, and deletion is ungated across this codebase anyway (`removeFaq.ts`, `removeMedia.ts`) — removing your own content is always allowed. Ownership enforced in the handler, answering 404. The provider deletion is best-effort and runs AFTER the columns are cleared: the owner asked for the menu to stop being shown, and a Cloudinary outage must not block that. |
