...
| `GET /api/v1/protected/owner-promotions/exclusive-deals` | `owner-promotion/protected/exclusive-deals.ts` | gate | `exclusive_deals` | to-wire | HOS-21 T-008: route built, reuses `findExclusiveDeals` (T-005/T-006). `gateExclusiveDeals` (currently a phantom gate below) + VIP_PROMOTIONS_ACCESS tier resolution wired in T-009 — until then every caller defaults to the 'plus' tier with no entitlement check. |
