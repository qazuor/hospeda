...
| `GET /api/v1/protected/accommodations/calendar-sync/google/callback` | `accommodation/protected/calendarGoogleCallback.ts` | none | - | n/a | HOS-157 Phase 2. OAuth return leg (fixed redirect URI, no :id). Secured by one-time CSRF state bound to accommodation+user + session actor==state.userId; no entitlement gate (already gated on connect). |
