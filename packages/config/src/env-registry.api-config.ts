...
export const API_CONFIG_ENV_VARS = [
    ...
    },
    {
        ...
        secret: false,
        defaultValue:
            'Content-Type,Authorization,X-Requested-With,X-Idempotency-Key,X-Client-Locale,X-Forwarded-For,X-Real-IP,X-Request-ID',
        exampleValue:
            'Content-Type,Authorization,X-Requested-With,X-Idempotency-Key,X-Client-Locale,X-Forwarded-For,X-Real-IP,X-Request-ID',
        apps: ['api'],
        category: 'api-config',
        howToObtain:
            'Headers the browser may send. Default covers Content-Type/Authorization/X-Requested-With + the IP/request-id headers the API actually reads. X-Idempotency-Key and X-Client-Locale are enforced in code (getCorsConfig REQUIRED_HEADERS) even if an override here omits them — a non-simple header missing from this list silently fails the browser preflight for every cross-origin mutation, not just the one that reads it, so this example lists them explicitly rather than relying on the code-side safety net alone. Add more if you introduce new x-* request headers (e.g. x-actor-id for impersonation).',
        howToObtainEs:
            'Headers que el navegador puede mandar. El default cubre Content-Type/Authorization/X-Requested-With + los headers de IP/request-id que la API realmente lee. X-Idempotency-Key y X-Client-Locale se fuerzan en código (getCorsConfig REQUIRED_HEADERS) aunque un override acá los omita — un header no-simple ausente de esta lista hace fallar el preflight del navegador para TODA mutación cross-origin, no sólo la que lo lee, así que este ejemplo los lista explícitamente en vez de depender sólo de la red de seguridad del código. Agregá más si incorporás nuevos headers x-* en el request (ej: x-actor-id para impersonar).'
    },
    {
    ...
    },
    {
        ...
        category: 'api-config',
        howToObtain:
            'Default true. Master switch for BOTH admin limiters on /api/v1/admin/*: the IP-keyed anti-abuse tier and the per-user admin:user sliding window that actually governs operator traffic. Set false to disable admin rate limiting entirely (e2e runs rely on this).',
        howToObtainEs:
            'Por defecto true. Interruptor maestro de AMBOS limiters de /api/v1/admin/*: el tier anti-abuso por IP y la ventana deslizante admin:user por usuario, que es la que realmente gobierna el tráfico de operadores. En false desactiva el rate limiting de admin por completo (las corridas e2e dependen de esto).'
    },
    {
        ...
        category: 'api-config',
        howToObtain:
            'Window for the IP-keyed admin counter (ms). Default 60000 (1 min), narrowed from 600000 by HOS-325 so this tier can be compared directly against the per-user admin:user budget (300/min) — a fixed 10-minute window made 2000 requests mean only 200/min, i.e. TIGHTER than the per-user limit it was supposed to sit above.',
        howToObtainEs:
            'Ventana del contador de admin por IP (ms). Por defecto 60000 (1 min), reducida desde 600000 por HOS-325 para poder compararla directamente contra el presupuesto por usuario admin:user (300/min) — con una ventana fija de 10 minutos, 2000 requests eran apenas 200/min, o sea MÁS ajustado que el límite por usuario sobre el que debía estar.'
    },
    {
        name: 'API_RATE_LIMIT_ADMIN_MAX_REQUESTS',
        description: 'Maximum requests allowed per window for the admin IP-keyed limiter',
        descriptionEs: 'Máximo de requests por ventana en el limiter de admin (por IP)',
        type: 'number',
        required: false,
        secret: false,
        defaultValue: '3000',
        ...
        category: 'api-config',
        howToObtain:
            'Max admin requests per IP per window — NOT per user: the per-user governor is the admin:user sliding window (300/min) in routes/index.ts. Default 3000/min, raised from 200/10min by HOS-325. This tier is an anti-abuse guard only, so it must stay ABOVE per-user-max x operators-sharing-one-egress-IP; 3000/min leaves room for 10. Set it too low and it silently becomes the binding constraint again, collapsing everyone behind one NAT/CGNAT IP into one budget.',
        howToObtainEs:
            'Máximo de requests de admin por IP por ventana — NO por usuario: el gobernador por usuario es la ventana deslizante admin:user (300/min) en routes/index.ts. Por defecto 3000/min, subido desde 200/10min por HOS-325. Este tier es sólo un freno anti-abuso, así que tiene que quedar POR ENCIMA de max-por-usuario x operadores-que-comparten-una-IP-de-salida; 3000/min da lugar a 10. Si queda bajo vuelve a ser en silencio la restricción que ata, colapsando en un solo presupuesto a todos los que comparten una IP NAT/CGNAT.'
    },
    {
    ...
    },
    {
        ...
        descriptionEs:
            'Activa el rate limiter dedicado para webhooks y endpoints máquina-a-máquina',
        type: 'boolean',
        required: false,
        secret: false,
        defaultValue: 'true',
        ...
        category: 'api-config',
        howToObtain:
            'Default true. High-throughput bucket for /webhooks/ paths. Also governs the ai-inbound (/api/v1/ai/*) and make-callback (/api/v1/integrations/make/*) tiers, which map to this same bucket — tuning it moves all three.',
        howToObtainEs:
            'Por defecto true. Bucket de alto throughput para rutas /webhooks/. También gobierna los tiers ai-inbound (/api/v1/ai/*) y make-callback (/api/v1/integrations/make/*), que mapean a este mismo bucket: tocarlo mueve los tres.'
    },
    {
        name: 'API_RATE_LIMIT_WEBHOOK_WINDOW_MS',
        description: 'Webhook rate-limit window duration in milliseconds',
        descriptionEs: 'Duración de la ventana de rate-limit de webhooks, en ms',
        type: 'number',
        required: false,
        secret: false,
        defaultValue: '60000',
        ...
        apps: ['api'],
        category: 'api-config',
        howToObtain: 'Window for the webhook counter (ms). Default 60000 (1 min).',
        howToObtainEs: 'Ventana del contador de webhooks (ms). Por defecto 60000 (1 min).'
    },
    {
        name: 'API_RATE_LIMIT_WEBHOOK_MAX_REQUESTS',
        description: 'Maximum requests allowed per window for the webhook limiter',
        descriptionEs: 'Máximo de requests por ventana en el limiter de webhooks',
        type: 'number',
        required: false,
        secret: false,
        defaultValue: '100',
        ...
        category: 'api-config',
        howToObtain:
            'Max webhook requests per IP per window. Default 100/min. Raise if a provider (e.g. MercadoPago) bursts retries above this.',
        howToObtainEs:
            'Máximo de requests de webhook por IP por ventana. Por defecto 100/min. Subilo si un proveedor (ej. MercadoPago) hace ráfagas de reintentos por encima.'
    },
    {
        name: 'API_RATE_LIMIT_WEBHOOK_MESSAGE',
        description: 'Error message returned when the webhook rate limit is exceeded',
        descriptionEs: 'Mensaje de error cuando se excede el rate limit de webhooks',
        type: 'string',
        required: false,
        secret: false,
        defaultValue: 'Too many webhook requests, please try again later.',
        exampleValue: 'Too many webhook requests, please try again later.',
        apps: ['api'],
        category: 'api-config',
        howToObtain: 'Free text returned when the webhook limiter trips.',
        howToObtainEs: 'Texto libre que se devuelve cuando salta el limiter de webhooks.'
    },
