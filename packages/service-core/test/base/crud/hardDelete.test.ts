...
describe('BaseService: hardDelete', () => {
    ...
    it('refuses a non-owner with the SAME answer as an id that does not exist', async () => {
        ...
        // asserted (`model.hardDelete` is never called); only the disclosure is
        // gone.
        ...
        const foreign = await service.hardDelete(nonAdminActor, MOCK_ENTITY_ID);

        asMock(modelMock.findById).mockResolvedValue(null);
        const invented = await service.hardDelete(nonAdminActor, MOCK_ENTITY_ID);
