...
const mockCatalog: AdPricingCatalog = {
    ...
    adSlotId: '00000000-0000-4000-8000-000000000002',
    channel: CampaignChannelEnum.WEB,
    basePrice: 100.0,
    currency: 'USD',
    pricingModel: PricingModelEnum.CPM,
    dailyRate: 10.0,
    weeklyRate: 60.0,
    monthlyRate: 200.0,
...
const mockCatalog2: AdPricingCatalog = {
    ...
    pricingModel: PricingModelEnum.CPC,
    basePrice: 2.5
};
...
const mockAdminActor: Actor = {
    ...
    role: RoleEnum.ADMIN,
    permissions: [
        PermissionEnum.AD_PRICING_CATALOG_CREATE,
        PermissionEnum.AD_PRICING_CATALOG_UPDATE,
        PermissionEnum.AD_PRICING_CATALOG_VIEW,
        PermissionEnum.AD_PRICING_CATALOG_DELETE
    ]
};
...
const mockModel = {
    ...
    count: vi.fn()
} as unknown as AdPricingCatalogModel;
    ...
    describe('Permission Hooks', () => {
        describe('_canCreate', () => {
            it('should allow ADMIN to create', async () => {
                ...
                const result = await service.create(mockAdminActor, {
                    ...
                    currency: 'USD',
                    pricingModel: PricingModelEnum.CPM,
                    pricingConfig: {}, // Required field, can be empty object
                    isActive: true,
                    weekendMultiplier: 1.5,
                    holidayMultiplier: 2.0
                });
        ...
        describe('_canSoftDelete', () => {
            it('should allow ADMIN to soft delete', async () => {
                vi.mocked(mockModel.findById).mockResolvedValue(mockCatalog);
                vi.mocked(mockModel.softDelete).mockResolvedValue(1);
        ...
        describe('_canHardDelete', () => {
            it('should allow ADMIN to hard delete', async () => {
                vi.mocked(mockModel.findById).mockResolvedValue(mockCatalog);
                vi.mocked(mockModel.hardDelete).mockResolvedValue(1);
        ...
        describe('_canList', () => {
            it('should allow ADMIN to list', async () => {
                // BaseCrudService.list() uses findAllWithRelations when getDefaultListRelations() returns {}
                vi.mocked(mockModel.findAllWithRelations).mockResolvedValue({
    ...
    describe('Business Methods', () => {
        describe('findByAdSlot', () => {
            ...
            it('should deny unauthorized user', async () => {
                const result = await service.findByAdSlot(
                    mockUnauthorizedActor,
                    mockCatalog.adSlotId
                );
