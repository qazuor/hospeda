/**
 * @fileoverview
 * Test suite for the AccommodationService.getTopRatedByDestination method.
 ...
 */
describe('AccommodationService.getTopRatedByDestination', () => {
    it('should throw Not implemented error', async () => {
        const service = createServiceTestInstance(AccommodationService);
        const input: Record<string, unknown> = {};
        const actor = new ActorFactoryBuilder().host().build();
        await expect(service.getTopRatedByDestination(input, actor)).rejects.toThrow(
            'Not implemented'
        );
    });
});
