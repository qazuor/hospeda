...
describe('TagService.restore', () => {
    let service: TagService;
    let tagModelMock: TagModel;
    let loggerMock: ReturnType<typeof createLoggerMock>;
    let actor: Actor;
    const tag = TagFactoryBuilder.create({ name: 'Tag', slug: 'tag', color: TagColorEnum.BLUE });
    const softDeletedTag = { ...tag, deletedAt: new Date() };

    beforeEach(() => {
        tagModelMock = createTypedModelMock(TagModel, ['findById', 'restore']);
    ...
    it('should restore a tag (success)', async () => {
        asMock(tagModelMock.findById).mockResolvedValue(softDeletedTag);
        asMock(tagModelMock.restore).mockResolvedValue(1);
        const result = await service.restore(actor, tag.id);
