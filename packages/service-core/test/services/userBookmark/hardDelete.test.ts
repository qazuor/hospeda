...
describe('UserBookmarkService.hardDelete', () => {
    ...
    beforeEach(() => {
        modelMock = createTypedModelMock(UserBookmarkModel, ['findById', 'hardDelete']);
    ...
    it('should hard delete a bookmark (success)', async () => {
        asMock(modelMock.findById).mockResolvedValue(bookmark);
        asMock(modelMock.hardDelete).mockResolvedValue({
            ...bookmark,
            deletedAt: new Date()
        });
        const result = await service.hardDelete(actor, bookmark.id);
    ...
    it('should return INTERNAL_ERROR if model throws', async () => {
        asMock(modelMock.findById).mockResolvedValue(bookmark);
        asMock(modelMock.hardDelete).mockRejectedValue(new Error('DB error'));
        const result = await service.hardDelete(actor, bookmark.id);
