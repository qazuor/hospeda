/**
 * experience.media.update.test.ts
 *
 * Unit tests for `updateExperienceMedia` (HOS-1036) — the text-metadata PATCH that
 * closes the gap left by HOS-390: `experience_media` rows had `alt`, `caption`,
 ...
 *  - the cross-experience ownership check disappearing, which would let a media id
 *    from someone else's experience be edited through your own experience's URL;
...
import type { ExperienceMedia, ExperienceMediaUpdateInput } from '@repo/schemas';
import { ModerationStatusEnum, PermissionEnum, RoleEnum, ServiceErrorCode } from '@repo/schemas';
import { beforeEach, describe, expect, it, vi } from 'vitest';
import { updateExperienceMedia } from '../../../src/services/experience/experience.media';
...
const OTHER_EXPERIENCE_ID = '00000000-0000-4000-a000-000000000099';
...
/** An experience listing owned by `ownerActor`. */
...
type ModelArg = Parameters<typeof updateExperienceMedia>[0];

/** Run the helper against a mocked model, keeping the cast in one place. */
function run(
    entity: Record<string, unknown> | null,
    actor: Actor,
    input: ExperienceMediaUpdateInput
) {
    return updateExperienceMedia(makeExperienceModel(entity) as unknown as ModelArg, actor, input);
}
...
beforeEach(() => {
    ...
    mockMediaModel.update.mockImplementation(
        async (_where: unknown, patch: Record<string, unknown>) =>
            makeMediaRow(patch as Partial<ExperienceMedia>)
    );
});

// ---------------------------------------------------------------------------

describe('updateExperienceMedia (HOS-1036)', () => {
    describe('happy path', () => {
    ...
    describe('ownership and existence — 404, never 403', () => {
        it('returns NOT_FOUND when the media row belongs to another experience', async () => {
            mockMediaModel.findById.mockResolvedValue(
                makeMediaRow({ experienceId: OTHER_EXPERIENCE_ID } as Partial<ExperienceMedia>)
            );
        ...
        it('returns FORBIDDEN when the actor may not edit the experience', async () => {
            const result = await run(makeExperience(), strangerActor, {
                experienceId: EXPERIENCE_ID,
                mediaId: MEDIA_ID,
                alt: 'No debería escribirse'
            });
