/**
 * @fileoverview
 * Unit tests for DestinationReviewService._executeAdminSearch override.
 *
 * The override extracts `minRating` and `maxRating` from entityFilters and converts
 * them into Drizzle `gte`/`lte` conditions on the `destinationReviews.averageRating` column.
 * All other filters pass through unchanged to super._executeAdminSearch().
 *
 * DestinationReviewService defines default relations (user + destination), so the base
 * class calls model.findAllWithRelations(relations, where, options, extraConditions).
 */
...
vi.mock('@repo/db', async (importOriginal) => {
    ...
    return {
        ...
        DestinationReviewModel: vi.fn().mockImplementation(() => mockReviewModelInstance)
    };
});

/** Shared mock instance so tests can spy on its methods. */
const mockReviewModelInstance = {
    ...
    hardDelete: vi.fn(),
    getTable: vi.fn(),
    getTableName: vi.fn().mockReturnValue('destination_reviews')
};
...
function callExecuteAdminSearch(
    service: DestinationReviewService,
...
describe('DestinationReviewService: _executeAdminSearch override', () => {
    ...
    describe('minRating filter', () => {
        it('should add one gte condition when minRating is provided', async () => {
            // Arrange
            const params = buildDefaultParams({ entityFilters: { minRating: 2 } });
    ...
    describe('maxRating filter', () => {
        it('should add one lte condition when maxRating is provided', async () => {
            // Arrange
            const params = buildDefaultParams({ entityFilters: { maxRating: 4 } });
    ...
    describe('minRating and maxRating combined', () => {
        it('should add exactly two conditions when both minRating and maxRating are provided', async () => {
            // Arrange
            const params = buildDefaultParams({ entityFilters: { minRating: 2, maxRating: 4 } });
    ...
    describe('no rating filters', () => {
        ...
        it('should pass remaining entityFilters through to the where clause', async () => {
            // Arrange
            const params = buildDefaultParams({ entityFilters: { destinationId: 'dest-abc' } });
            ...
            const [, whereArg] = asMock(mockReviewModelInstance.findAllWithRelations).mock
                .calls[0] as [unknown, Record<string, unknown>, unknown, unknown];
            expect(whereArg).toHaveProperty('destinationId', 'dest-abc');
        });
    });
    ...
    describe('return value', () => {
        it('should return the paginated result from the model', async () => {
            // Arrange
            const expected = { items: [{ id: 'drev-1' }], total: 1 };
