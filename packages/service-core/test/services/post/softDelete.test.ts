...
describe('PostService.softDelete', () => {
    ...
    it('should soft delete the post if actor has permission', async () => {
    ...
    it('should return count 0 if post is already deleted', async () => {
        const deletedPost = { ...post, deletedAt: new Date() };
        (modelMock.findById as Mock).mockResolvedValue(deletedPost);
        const result = await service.softDelete(actorWithPerm, postId);
        expectSuccess(result);
