...
describe('OwnerPromotionService.hardDelete', () => {
    ...
    beforeEach(() => {
        ...
        actor = createActor({ permissions: [PermissionEnum.OWNER_PROMOTION_HARD_DELETE_ANY] });
        vi.clearAllMocks();
    });

    it('should hard delete an owner promotion when permissions are valid', async () => {
        modelMock.findById.mockResolvedValue(existing);
        modelMock.hardDelete.mockResolvedValue(true);
