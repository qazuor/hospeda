 ...
 *
 *  - that the cap handed over is gastronomy's own (30), not another vertical's;
...
describe('addGastronomyFeaturedMedia (HOS-803)', () => {
    ...
    it('threads gastronomyId, not another vertical key, through the gallery count', async () => {
        const model = makeGastronomyModel({ id: GASTRONOMY_ID, ownerId: OWNER_ID });
        ...
        expect(where.gastronomyId).toBe(GASTRONOMY_ID);
