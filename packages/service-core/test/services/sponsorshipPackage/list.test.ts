...
describe('SponsorshipPackageService.list', () => {
    ...
    it('should list sponsorship packages when permissions are valid', async () => {
        const items = [
            createMockSponsorshipPackage({ id: getMockSponsorshipPackageId('mock-id-1') })
        ];
    ...
    it('should return an empty list if there are no sponsorship packages', async () => {
        modelMock.findAllWithRelations.mockResolvedValue({
            items: [],
            total: 0,
            page: 1,
    ...
    it('should list multiple sponsorship packages', async () => {
        const items = [
            createMockSponsorshipPackage({ id: getMockSponsorshipPackageId('mock-id-1') }),
            createMockSponsorshipPackage({ id: getMockSponsorshipPackageId('mock-id-2') })
        ];
        modelMock.findAllWithRelations.mockResolvedValue({
            items,
            total: 2,
    ...
    it('should return paginated results', async () => {
        ...
        modelMock.findAllWithRelations.mockResolvedValue({
            ...
            page: 2,
            pageSize: 1
        } as PaginatedMock<ReturnType<typeof createMockSponsorshipPackage>>);
