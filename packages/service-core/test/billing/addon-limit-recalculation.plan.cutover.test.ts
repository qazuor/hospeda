...
// `inArray` off `@repo/db`. Every `limitKey` this file exercises is
// `'max_accommodations'` — the `'accommodation'` domain, which fails OPEN on a
// `null`/missing result — so an empty hydration read (the default here) is the
// correct behaviour for every test in this file, not a shortcut around it.
vi.mock('@repo/db', () => ({
