...
class NotificationService {
    async send(_notification: { type: string; data: unknown }) {}
}

class SearchIndexService {
    async index(_article: Article) {}
    async update(_article: Article) {}
    async remove(_articleId: string) {}
}
...
> {
    ...
    protected async _afterCreate(entity: Article, actor: Actor): Promise<Article> {
        ...
        this.searchIndexService
            .index(entity)
            .catch((err) => this.logger.error('Failed to index article', err));
     ...
     */
    protected async _afterUpdate(entity: Article, _actor: Actor): Promise<Article> {
        ...
        this.searchIndexService
            .update(entity)
            .catch((err) => this.logger.error('Failed to update search index', err));
