...
| `FeatureService` | `accommodation` (via `r_accommodation_feature`), `feature` | **no (model-layer instead)** | HOS-288: `getAccommodationsByFeature` used to load the accommodations through the junction model's `findAllWithRelations({ accommodation: true })`, which joins `accommodations` unfiltered — soft-deleted listings came back. It now resolves the junction's accommodation ids and re-reads the rows through `AccommodationModel.findAll`, whose default excludes soft-deleted rows. This is a different mechanism from the `filterSoftDeletedRelations` contract stated above: there is no post-hoc strip, the relation is simply never loaded unfiltered — do not go looking for that call here. `visibility: PUBLIC` + `lifecycleState: ACTIVE` are narrowed too, and that is load-bearing: `/api/v1/public/features` is in `PUBLIC_CACHE_ENDPOINTS`, public cache keys carry no Authorization component, and `cacheMiddleware` runs before `authMiddleware`, so the response is shared across actors and must be anonymous-safe. (The permission gate itself is decorative there — `CACHEABLE_STATUS_CODES` is `{200, 404}`, so a guest's 403 is never stored while a privileged 200 is; that gate/cache mismatch is a known open follow-up, out of scope for HOS-288.) (The reverse argument — "it is gated by `ACCOMMODATION_FEATURES_EDIT`, so the audience is staff who must see `PRIVATE`/`DRAFT`" — was tried and is wrong twice: that permission is held by `RoleEnum.HOST`, which is multi-tenant with no owner scoping here, and the cache would hand the result to anonymous callers regardless.) `getFeaturesForAccommodation` still joins `feature` unfiltered — a deleted feature on an existing relation is acceptable historical context. |
| `AmenityService` | `accommodation` (via `r_accommodation_amenity`), `amenity` | **no (model-layer instead)** | HOS-288: predicate-level twin of `FeatureService` above (its return contract differs — this method projects a summary shape, not full `Accommodation` rows) — `getAccommodationsByAmenity` moved from the unfiltered junction join to a re-read through `AccommodationModel.findAll`, with the same predicates and the same cache-key rationale (`/api/v1/public/amenities` is likewise in `PUBLIC_CACHE_ENDPOINTS`). It backs no route today; it is hardened to match its twin so that wiring the symmetric route cannot reintroduce either leak. `getAmenitiesForAccommodation` still joins `amenity` unfiltered, same rationale as its feature twin. |
