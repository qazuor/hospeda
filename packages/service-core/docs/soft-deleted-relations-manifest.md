...
| `FeatureService` | `accommodation` (via `r_accommodation_feature`), `feature` | **filter `accommodation`** | HOS-288: `getAccommodationsByFeature` used to load the accommodations through the junction model's `findAllWithRelations({ accommodation: true })`, which joins `accommodations` unfiltered — soft-deleted listings came back. It now resolves the junction's accommodation ids and re-reads the rows through `AccommodationModel.findAll`, whose default excludes soft-deleted rows. Visibility/lifecycle are deliberately NOT narrowed: the method requires `ACCOMMODATION_FEATURES_EDIT`, so its audience is staff/hosts, who must see `PRIVATE`/`DRAFT` rows. `getFeaturesForAccommodation` still joins `feature` unfiltered — a deleted feature on an existing relation is acceptable historical context. |
| `AmenityService` | `accommodation` (via `r_accommodation_amenity`), `amenity` | **filter `accommodation`** | HOS-288: exact twin of `FeatureService` above — `getAccommodationsByAmenity` moved from the unfiltered junction join to a re-read through `AccommodationModel.findAll`. Gated by `ACCOMMODATION_AMENITIES_EDIT` and backs no route today, so visibility/lifecycle are likewise not narrowed. `getAmenitiesForAccommodation` still joins `amenity` unfiltered, same rationale as its feature twin. |
