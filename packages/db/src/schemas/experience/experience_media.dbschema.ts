...
/**
 * PostgreSQL enum for the visibility state of a single experience media row.
 *
 ...
 */
export const ExperienceMediaStatePgEnum = pgEnum('experience_media_state_enum', [
    'visible',
    'archived'
]);

/**
 * `experience_media` — relational table for experience listing gallery photos.
 *
 * Replaces the JSONB `experiences.media.gallery` / `media.featuredImage` sub-fields
 * with a proper per-row structure. Videos live in the dedicated `experiences.videos`
...
export const experienceMedia = pgTable(
    'experience_media',
    {
        ...
        /**
         * FK to the owning experience listing. ON DELETE CASCADE ensures photo rows are
         ...
         */
        state: ExperienceMediaStatePgEnum('state').notNull().default('visible'),
        /**
    ...
    (table) => ({
        ...
        experienceMedia_experienceId_idx: index('experienceMedia_experienceId_idx').on(
            table.experienceId
        ),
        /** Index to filter quickly by state (visible vs archived gallery reads). */
        experienceMedia_state_idx: index('experienceMedia_state_idx').on(table.state),
        /** Index to quickly locate the featured photo for a given listing. */
        experienceMedia_isFeatured_idx: index('experienceMedia_isFeatured_idx').on(
            table.isFeatured
        ),
        /** Composite index for the canonical gallery-read query: listing + state + order. */
        experienceMedia_experienceId_state_sortOrder_idx: index(
            'experienceMedia_experienceId_state_sortOrder_idx'
        ).on(table.experienceId, table.state, table.sortOrder),
        /** Soft-delete filter index. */
        experienceMedia_deletedAt_idx: index('experienceMedia_deletedAt_idx').on(table.deletedAt)
    })
);

/**
 * Drizzle relations for `experience_media`.
 *
 * Each media row belongs to exactly one experience listing.
 */
export const experienceMediaRelations = relations(experienceMedia, ({ one }) => ({
 ...
 */
export type SelectExperienceMedia = typeof experienceMedia.$inferSelect;

/**
 * Type inferred from the Drizzle table definition for INSERT queries.
 * Use this instead of a manual interface — it always stays in sync with the schema.
 */
export type InsertExperienceMedia = typeof experienceMedia.$inferInsert;
