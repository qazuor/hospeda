...
/**
 * PostgreSQL enum for the visibility state of a single post media row.
 *
 ...
 */
export const PostMediaStatePgEnum = pgEnum('post_media_state_enum', ['visible', 'archived']);

/**
 * `post_media` — relational table for a post's gallery photos (HOS-390).
 *
 * Replaces the JSONB `posts.media.gallery` / `media.featuredImage` sub-fields
 ...
 * in the `posts.media` JSONB column** (SPEC-204 D1) — they carry no per-row
 * state and no gallery ordering.
 *
 * Why this table exists at all: `POST /protected/media/posts/:id` uploads to
 ...
 * an author to attach a photo to their own post from the web (HOS-374 slice 2D).
 *
 ...
 * @see packages/schemas/src/entities/post/subtypes/post.media.schema.ts — the Zod mirror
 */
export const postMedia = pgTable(
    'post_media',
    {
        ...
        /**
         * FK to the owning post. ON DELETE CASCADE so photo rows disappear with
         * the post on a hard delete (a soft-deleted post keeps its rows).
         */
        postId: uuid('post_id')
            .notNull()
        ...
        /** Accessible alt text; falls back to caption / post title at render time. */
        alt: text('alt'),
        /**
        ...
        /**
         * Content moderation state of the PHOTO — independent of the post's own
         * verdict (HOS-374 §7.6.1). Approving a post says nothing about whether
         * each of its images passed review.
         */
        moderationState: ModerationStatusPgEnum('moderation_state').notNull().default('PENDING'),
        /** Visibility state within the post's media collection. */
        state: PostMediaStatePgEnum('state').notNull().default('visible'),
        /**
         * When `true` this row is the post's featured / cover image. At most one
         * per post (partial unique index, extras carril); MUST be false while
    ...
    (table) => ({
        /** The dominant query: every media row of one post. */
        postMedia_postId_idx: index('postMedia_postId_idx').on(table.postId),
        /** Filter by state (visible vs archived reads). */
        postMedia_state_idx: index('postMedia_state_idx').on(table.state),
        /** Locate the featured photo of a post. */
        postMedia_isFeatured_idx: index('postMedia_isFeatured_idx').on(table.isFeatured),
        /** The canonical gallery read: post + state + order. */
        postMedia_postId_state_sortOrder_idx: index('postMedia_postId_state_sortOrder_idx').on(
            table.postId,
            table.state,
            table.sortOrder
        ),
        /** Soft-delete filter. */
        postMedia_deletedAt_idx: index('postMedia_deletedAt_idx').on(table.deletedAt)
    })
);

/**
 * Drizzle relations for `post_media`. Each row belongs to exactly one post; the
 * inverse `many` side is wired in `post.dbschema.ts`.
 */
...
export type SelectPostMedia = typeof postMedia.$inferSelect;
