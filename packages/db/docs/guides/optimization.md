...
Comprehensive guide to database optimization strategies for the Hospeda project, including query optimization, indexing, N+1 query prevention, connection pooling, and monitoring.
...
- [Relations Optimization](#relations-optimization)
...
| **Buffers** | Memory usage | shared hit = cache hit (good) |
...
export const queryCounterMiddleware = async (c, next) => {
  ...
  if (queryCount > 10) {
    console.warn(`${queryCount} queries in single request!`);
  }
};
```
...
### Solution 4: Eager Loading with Model

**Good**: Load relations eagerly with model patterns
...
-- Query using index
SELECT * FROM accommodations WHERE city = 'Concepcion del Uruguay';
```
...
SELECT * FROM accommodations WHERE city = 'Concepcion';

-- Uses index (both columns)
SELECT * FROM accommodations
WHERE city = 'Concepcion' AND is_active = true;
...
**Cons**: Cannot use transactions

**When to Use**: Rarely (transaction mode is better)
...
**Alternative pg Pool Configuration:**
...
EXPLAIN SELECT * FROM accommodations WHERE city = 'Concepcion';
```
