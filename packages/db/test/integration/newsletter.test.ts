...
describe('Newsletter schema — integration', () => {
    ...
    describe('Schema sanity', () => {
        ...
        it('applies the partial UNIQUE index from manual SQL 0022', async () => {
            const db = getTestDb();
        ...
        it('applies the delivery UNIQUE + partial pending indexes from manual SQL 0023', async () => {
            const db = getTestDb();
    ...
    describe('newsletter_subscribers (user_id, channel) partial UNIQUE', () => {
        it('rejects a second active subscription for the same (user, channel) pair', async () => {
        ...
        it('allows a NEW subscription after the previous one was soft-deleted', async () => {
            await withTestTransaction(async (tx) => {
                const user = testData.user();
                await tx.insert(users).values(user);
    ...
    describe('newsletter_campaign_deliveries (campaign, subscriber, channel) UNIQUE', () => {
        it('prevents duplicate deliveries for the same campaign + subscriber + channel', async () => {
            await withTestTransaction(async (tx) => {
                const user = testData.user();
                await tx.insert(users).values(user);
