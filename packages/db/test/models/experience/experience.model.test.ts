/**
 * Unit tests for `ExperienceModel.findIdsByOwnerId` (HOS-734).
 *
 * Mirrors `AccommodationModel.findIdsByOwnerId`: resolves the IDs of every
 * non-deleted experience listing owned by a given user, used by
...
describe('ExperienceModel.findIdsByOwnerId', () => {
    ...
    it('uses the provided transaction client instead of getDb()', async () => {
        const where = vi.fn().mockResolvedValue([{ id: 'exp-tx' }]);
        const from = vi.fn().mockReturnValue({ where });
        const select = vi.fn().mockReturnValue({ from });
