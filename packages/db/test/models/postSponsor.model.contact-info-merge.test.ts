/**
 * @file postSponsor.model.contact-info-merge.test.ts
 * @description A partial `contactInfo` PATCH on `post_sponsors` must NOT wipe
 * the column.
 *
 * `post_sponsors.contact_info` is one JSONB column that can hold several
 ...
 * this fix `PostSponsorModel` never did, so a patch of
 ...
 * `PostSponsorModel.mergeableJsonbColumns` turns 7 of the 8 tests in this file red
...
function buildMockInnerTx() {
    const returning = vi.fn().mockResolvedValue([{ id: 'sponsor-1' }]);
    const where = vi.fn(() => ({ returning }));
    const set = vi.fn(() => ({ where }));
    const update = vi.fn(() => ({ set }));
    const execute = vi.fn().mockResolvedValue({ rows: [{ id: 'sponsor-1' }] });
...
describe('PostSponsorModel — `contactInfo` is a mergeable JSONB column', () => {
    beforeEach(() => {
        vi.restoreAllMocks();
    ...
    it('declares exactly `contactInfo` as mergeable', () => {
        ...
        const mergeable = (
            new PostSponsorModel() as unknown as { mergeableJsonbColumns: readonly string[] }
    ...
    it('opens a transaction for a partial `contactInfo` patch instead of replacing the column', async () => {
        ...
        // Act
        await new PostSponsorModel().update({ id: 'sponsor-1' }, { contactInfo: PHONE_PATCH });
    ...
    it('serialises ONLY the sent key — sibling contact fields are never named, so they survive the `||` merge', () => {
        // Arrange: the real `post_sponsors` table and the real mergeable list.
        const mergeable = (
            new PostSponsorModel() as unknown as { mergeableJsonbColumns: readonly string[] }
        ).mergeableJsonbColumns;

        // Act
        const result = buildMergeSetClause({ contactInfo: PHONE_PATCH }, postSponsors, mergeable);
    ...
    it('still clears the WHOLE column when the patch value itself is null', async () => {
        ...
        await new PostSponsorModel().update({ id: 'sponsor-1' }, {
            contactInfo: null
        } as unknown as Parameters<PostSponsorModel['update']>[1]);
