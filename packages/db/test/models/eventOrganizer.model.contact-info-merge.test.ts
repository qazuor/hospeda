/**
 * @file eventOrganizer.model.contact-info-merge.test.ts
 * @description A partial `contactInfo` PATCH on `event_organizers` must NOT
 * wipe the column.
 *
 * `event_organizers.contact_info` is one JSONB column that can hold several
 ...
 *   - removing `contactInfo` from `EventOrganizerModel.mergeableJsonbColumns` turns
...
function buildMockInnerTx() {
    const returning = vi.fn().mockResolvedValue([{ id: 'organizer-1' }]);
    const where = vi.fn(() => ({ returning }));
    const set = vi.fn(() => ({ where }));
    const update = vi.fn(() => ({ set }));
    const execute = vi.fn().mockResolvedValue({ rows: [{ id: 'organizer-1' }] });
...
describe('EventOrganizerModel — `contactInfo` is a mergeable JSONB column', () => {
    ...
    it('opens a transaction for a partial `contactInfo` patch instead of replacing the column', async () => {
        ...
        // Act
        await new EventOrganizerModel().update({ id: 'organizer-1' }, { contactInfo: PHONE_PATCH });
    ...
    it('serialises ONLY the sent key — sibling contact fields are never named, so they survive the `||` merge', () => {
        // Arrange: the real `event_organizers` table and the real mergeable list.
        const mergeable = (
            new EventOrganizerModel() as unknown as { mergeableJsonbColumns: readonly string[] }
        ).mergeableJsonbColumns;
