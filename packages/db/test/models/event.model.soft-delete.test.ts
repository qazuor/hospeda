/**
 * Tests for the default soft-delete exclusion on `EventModel.findAll`,
 * `EventModel.findAllWithRelations`, and `EventModel.count` (HOS-274).
 *
 * Before this fix, none of the PUBLIC list/count/getByX endpoints for events
 * filtered `deletedAt IS NULL` — the model-layer fix here closes that gap
 * ONCE for every current and future caller (see `EventModel#softDeleteCondition`
 * JSDoc). Covers:
 *   1. Default injection of `isNull(events.deletedAt)` on all three methods,
 *      across both the delegation (default sort) and synthetic-sort
 *      (`mostSaved`/`startDate`) code paths.
...
/**
 * Chainable mock for the synthetic-sort branch (`mostSaved`/`startDate`),
 * which uses `db.select().from().where().orderBy().limit().offset()` (no
 * `$dynamic()`) for items, plus `db.select().from().where()` for count.
 */
...
/** Chainable mock for `findAllWithRelations`' delegation branch: `db.query.events.findMany()` + `db.select().from().where()` for count. */
...
describe('EventModel — default soft-delete exclusion (HOS-274)', () => {
    ...
    describe('findAllWithRelations — delegation branch (default sort)', () => {
        it('injects isNull(deletedAt) by default', async () => {
    ...
    describe('findAllWithRelations — synthetic-sort branch (startDate)', () => {
        it('injects isNull(deletedAt) by default', async () => {
            // The startDate branch uses db.query.events.findMany, not db.select — build
            // a dedicated mock matching that shape.
            ...
            getDb.mockReturnValue(startDateDb as never);

            await model.findAllWithRelations({ author: true }, {}, { sortBy: 'startDate' });

            const callArgs = findManyFn.mock.calls[0]?.[0] as { where?: unknown } | undefined;
            expect(hasSoftDeleteCondition(callArgs?.where)).toBe(true);
        });
        ...
        it('items/total consistency: forwards includeDeleted to the manual this.count() call', async () => {
            ...
            await model.findAllWithRelations(
                { author: true },
                {},
                { sortBy: 'startDate', includeDeleted: true }
            );
