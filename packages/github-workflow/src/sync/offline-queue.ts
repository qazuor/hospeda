...
import type { OfflineQueueOptions, QueueStatistics, QueuedOperation } from './types';
...
export class OfflineQueue {
    ...
    async load(): Promise<void> {
        try {
            if (existsSync(this.filePath)) {
                ...
                logger.info(
                    { filePath: this.filePath, operations: this.data.operations.length },
    ...
    async getAllOperations(): Promise<QueuedOperation[]> {
        ...
        return this.data?.operations ? [...this.data.operations] : [];
    }
