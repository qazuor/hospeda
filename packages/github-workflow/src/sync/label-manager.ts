...
export class LabelManager {
    ...
    async warmup(): Promise<void> {
        ...
        for (const label of commonLabels) {
            await this.githubClient.createLabel({
                name: label.name,
                color: label.color,
    ...
    generateForTask(input: GenerateLabelsInput): string[] {
        ...
        if (task.estimate !== undefined) {
            const estimateNum = this.parseEstimate(task.estimate);
            if (estimateNum !== undefined) {
                const priority = this.estimateToPriority(estimateNum);
                if (priority) {
        ...
        if (task.estimate !== undefined) {
            const estimateNum = this.parseEstimate(task.estimate);
            if (estimateNum !== undefined) {
                const difficulty = this.estimateToDifficulty(estimateNum);
                if (difficulty) {
    ...
    generateForComment(comment: CodeComment): string[] {
        ...
        const suggestedLabels = this.analyzeCommentForLabels(comment.content);
        labels.push(...suggestedLabels);
    ...
    async ensureLabelsExist(labelNames: string[]): Promise<void> {
        for (const name of labelNames) {
            ...
            await this.githubClient.createLabel(definition);
        }
    }
    ...
    /**
     * Parse estimate string to number
     *
     * Extracts numeric value from estimate strings like "4h", "2 hours", "8"
     *
     * @param estimate - Estimate string
     * @returns Numeric value or undefined if cannot parse
     */
    private parseEstimate(estimate: string): number | undefined {
        // Remove common suffixes and parse
        const cleaned = estimate.toLowerCase().replace(/[^0-9.]/g, '');
