        ...
        switch (operation.type) {
            case 'create-issue':
                await this.githubClient.createIssue(
                    data as Parameters<typeof this.githubClient.createIssue>[0]
                );
                break;

            case 'update-issue': {
                // Extract issueNumber and pass rest of data
                const { issueNumber, ...updateData } = data as {
                    issueNumber: number;
                    [key: string]: unknown;
                };
                if (issueNumber === undefined) {
                    throw new Error('issueNumber is required for update-issue operation');
                }
            ...
            case 'close-issue': {
                ...
                if (issueNumber) {
                    await this.githubClient.closeIssue(issueNumber);
                }
                break;
            }

            case 'add-labels': {
                const { issueNumber, labels } = data as { issueNumber?: number; labels?: string[] };
                if (issueNumber && labels) {
                    await this.githubClient.addLabels(issueNumber, labels);
                }
                break;
            }

            case 'link-issues': {
                const { parentNumber, childNumber } = data as {
                    parentNumber?: number;
                    childNumber?: number;
                };
                if (parentNumber && childNumber) {
                    await this.githubClient.linkIssues(parentNumber, childNumber);
                }
                break;
            }
