...
import type { PlanningContext, EnrichmentOptions, EnrichmentTask } from './types.js';
 ...
 * Extract tech analysis content
 */
function extractTechAnalysisContent(
	content: string
): PlanningContext['techAnalysis'] | undefined {
...
function extractTaskDependencies(taskSection: string): string[] {
	...
	while ((match = taskCodeRegex.exec(depText)) !== null) {
