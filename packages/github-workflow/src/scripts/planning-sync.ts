...
async function main(): Promise<void> {
    ...
    try {
        ...
        if (result.success) {
            ...
            console.info('');
            console.info('✅ Planning sync completed');
            console.info('');
            console.info('📊 Statistics:');
            console.info(`  Created: ${result.statistics.created}`);
            console.info(`  Updated: ${result.statistics.updated}`);
            console.info(`  Failed: ${result.statistics.failed}`);

            if (result.statistics.created > 0) {
                console.info('');
                console.info('🔗 Created Issues:');
                for (const created of result.created) {
                    console.info(`  #${created.issueNumber} - ${created.taskCode}`);
                }
            }
        } else {
