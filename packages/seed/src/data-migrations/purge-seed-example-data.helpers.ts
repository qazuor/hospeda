...
/**
 * Local-parts of the accounts that must NEVER be deleted, whatever else says.
 *
 * This is a belt over the allowlist's braces, and it exists because of a near
 * miss found on 2026-08-19: `EXAMPLE_USER_EMAILS` listed `guest@hospeda.com`,
 * while the real invited actor is `guest@hospeda.com.ar` — the domains moved to
 * `.com.ar` in data-migration `0057-staff-email-domain-to-com-ar`. The guest
 * account therefore survived this purge by a **stale domain**, not by design:
 * anyone "fixing" that line to match production would have deleted the actor
 * every anonymous API request is built from.
 *
 * Matching on the local-part makes the protection survive the next domain move.
 */
export const NEVER_DELETE_LOCAL_PARTS: readonly string[] = [
    'guest',
    'superadmin',
    'admin',
    'editorial'
...
/**
 * Reports whether an account is infrastructure and must never be hard-deleted.
 *
 * @param params - Receive-object payload.
 * @param params.email - The account email to classify.
 * @returns `true` when the account is protected infrastructure.
 */
export const isProtectedInfrastructureEmail = (params: { email: string }): boolean => {
    const localPart = params.email.split('@')[0]?.toLowerCase() ?? '';
    return NEVER_DELETE_LOCAL_PARTS.includes(localPart);
};
