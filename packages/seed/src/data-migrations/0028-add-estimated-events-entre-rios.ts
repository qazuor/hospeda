...
export async function up(ctx: SeedMigrationCtx): Promise<SeedMigrationResult> {
    ...
    return {
        summary: `Inserted ${eventsInserted} of ${EVENTS.length} estimated (month-precision) Entre Ríos tourist events (rest already present), across ${locationsInserted} of ${LOCATIONS.length} newly-created event locations. Skipped ${eventsSkipped} events and ${locationsSkipped} locations (destinations absent in this environment).`,
        counts: {
            eventsInserted,
