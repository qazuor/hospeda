/**
 * @fileoverview
 * Data migration: 0058-purge-seed-example-data
 *
...
} from './purge-seed-example-data.helpers.js';
import type { SeedMigrationCtx, SeedMigrationModule, SeedMigrationResult } from './types.js';

export const meta = {
    name: '0058-purge-seed-example-data',
...
export async function up(ctx: SeedMigrationCtx): Promise<SeedMigrationResult> {
    ...
    for (const user of candidateUsers) {
        // Infrastructure wins over the allowlist, always. `guest` is the actor
        // every anonymous API request is built from, and it reached production
        // owning seed rows; on 2026-08-19 it was found listed in
        // EXAMPLE_USER_EMAILS under a stale domain, one typo away from being
        // deleted here. This check matches on the local-part so the protection
        // outlives the next domain move.
        if (user.email && isProtectedInfrastructureEmail({ email: user.email })) {
            usersProtectedAsInfrastructure += 1;
            logger.warn(
                `purge: candidate user ${user.id} (email=${user.email}) is protected infrastructure — refusing to delete it.`
            );
            continue;
        }
