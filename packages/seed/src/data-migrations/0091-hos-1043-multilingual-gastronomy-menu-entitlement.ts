/**
 * @fileoverview
 * Data migration: 0091-hos-1043-multilingual-gastronomy-menu-entitlement
 *
 * Dual-write counterpart (HOS-25) for HOS-1043. The baseline gains one
 * `EntitlementKey` member — `multilingual_gastronomy_menu`, translating the
 * structured carta into `{es,en,pt}` — and grants it on `gastronomy-premium`;
 ...
 *
 * The same split `0082`/`0084` describe. Four commerce keys are resolved from
 * CODE (`ENTITLEMENT_KEYS_BY_COMMERCE_VERTICAL`), because they are the FLOOR
 * every tier of a vertical receives and a lagging DB row must never be able
 * to lock an owner out of them. This key is the opposite: a TIER
 * differentiator. Put in that map it would be handed to `-basico` and `-pro`
 * as well, which is the precise give-away the premium step exists to
 * prevent. So it is granted on the row and reaches the gate through the union
 * `resolveOwnerGastronomyPlanEntitlementSet` performs over the subscribed
 * plan's `entitlements` column.
 *
 * The window to be honest about: between the deploy and this migration
 * running, a `gastronomy-premium` subscriber is refused menu translations. It
 * fails CLOSED, not open, and today it is empty in practice — HOS-818 retired
 ...
 *
 * ## ONE vertical, like 0084 and unlike 0080
 *
 * `experience-premium` is deliberately absent. An experience has no carta, so
 * there is nothing for this key to translate; granting it there would
 * advertise a capability whose route an experience owner can never reach.
 *
 * ## No column dependency, hence no `meta.requiresColumns`
 *
 * The structural migration in this same release adds `name_i18n` and
 * `description_i18n` to `gastronomy_menu_sections`/`gastronomy_menu_items`,
 * but this migration does not read or write any of them — it touches
 ...
 *
 * Union, never replacement — same rule as `0077`, `0080` and `0084`. An
 * operator who granted an extra key through the SPEC-168 admin editor keeps
 * it, and this migration can only ever add the one key it owes.
 *
...
export const meta = {
    name: '0091-hos-1043-multilingual-gastronomy-menu-entitlement',
    group: 'required',
    destructive: false
...
export async function up(ctx: SeedMigrationCtx): Promise<SeedMigrationResult> {
    ...
    const summary = changed
        ? `HOS-1043: created ${entitlementsCreated} billing_entitlements row(s) and granted multilingual_gastronomy_menu on ${plansGranted} gastronomy premium plan row(s).`
        : 'HOS-1043: multilingual_gastronomy_menu already granted on gastronomy-premium — no change.';
