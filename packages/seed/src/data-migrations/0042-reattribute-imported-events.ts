/**
 * @fileoverview
 * Data migration: 0042-reattribute-imported-events
 *
 ...
 * identity `0025` and `0041` use — never by slug or id, both of which differ per
 * environment.
 *
 ...
 *   abort the batch — and under HOS-25 G-5 also block `0043`/`0044` — over a
...
export const meta = {
    name: '0042-reattribute-imported-events',
    group: 'required',
    destructive: false,
    contentOnly: true
...
export async function up(ctx: SeedMigrationCtx): Promise<SeedMigrationResult> {
    ...
    if (matchedBefore === 0) {
        ...
        throw new Error(
            `0042-reattribute-imported-events matched ZERO events and the editorial author holds none either, so nothing was re-attributed. ` +
    ...
    if (moved.length === 0) {
        throw new Error(
            `0042-reattribute-imported-events found ${matchedBefore} matching event(s) but updated none. ` +
