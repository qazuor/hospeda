...
/**
 * Mapping data structure that includes both ID and optional name
 */
...
export class IdMapper {
    ...
    private loadMappings(): void {
        try {
            if (fs.existsSync(this.mappingsFilePath)) {
                ...
                const mappingsData = JSON.parse(data) as Record<
                    string,
                    Record<string, string | MappingData>
                >;

                for (const [entityType, mappings] of Object.entries(mappingsData)) {
                    const entityMap = new Map<string, MappingData>();
                    for (const [seedId, mappingValue] of Object.entries(mappings)) {
                        // Handle both old format (string) and new format (object)
                        if (typeof mappingValue === 'string') {
                            entityMap.set(seedId, { id: mappingValue });
                        } else {
                            entityMap.set(seedId, mappingValue);
                        }
                    }
    ...
    private saveMappings(): void {
        try {
            const mappingsData: Record<string, Record<string, MappingData>> = {};

            for (const [entityType, mappings] of this.mappings.entries()) {
                mappingsData[entityType] = {};
                for (const [seedId, mappingData] of mappings.entries()) {
                    mappingsData[entityType][seedId] = mappingData;
                }
            }
     ...
     * @param name - Optional name for better logging
     */
    setMapping(entityType: string, seedId: string, realId: string, name?: string): void {
        if (!this.mappings.has(entityType)) {
            this.mappings.set(entityType, new Map());
        }
        const entityMappings = this.mappings.get(entityType);
        if (entityMappings) {
            entityMappings.set(seedId, { id: realId, name });
        }
    ...
    getRealId(entityType: string, seedId: string): string | undefined {
        return this.mappings.get(entityType)?.get(seedId)?.id;
    }

    /**
     * Gets the mapping data (ID and name) for a given seed ID and entity type.
     * @param entityType - The type of entity
     * @param seedId - The seed ID to look up
     * @returns The mapping data if found, undefined otherwise
     */
    getMappingData(entityType: string, seedId: string): MappingData | undefined {
        return this.mappings.get(entityType)?.get(seedId);
    }

    /**
     * Gets a display name for an entity, using the stored name or falling back to the seed ID.
     * @param entityType - The type of entity
     * @param seedId - The seed ID to look up
    ...
    getMappingStats(entityType: string): { count: number; examples: string[] } {
        ...
        const examples = Array.from(mappings.entries())
            ...
            .map(([seedId, mappingData]) => `${seedId} → ${mappingData.id.slice(0, 8)}...`);
