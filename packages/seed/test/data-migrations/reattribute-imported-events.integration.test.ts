/**
 * @fileoverview
 * Integration tests for `0042-reattribute-imported-events.ts` (HOS-375 T-007).
 *
...
import * as reattributeImportedEvents from '../../src/data-migrations/0042-reattribute-imported-events.js';
...
describe('HOS-375 T-007: 0042-reattribute-imported-events (integration)', () => {
    beforeAll(async () => {
    ...
    it('is a no-op — not a failure — where no event carries the import signature', async () => {
        ...
        // deliberately support, and under HOS-25 G-5 also blocks `0043`/`0044`
        // from ever applying.
        await withRollback(async (tx) => {
            await clearTargets(tx);
