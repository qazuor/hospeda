/**
 * @fileoverview
 * Unit tests for the `0100-hos-1030-ai-translate-url-rule` data migration,
 * using a fully mocked `ctx.db` (no real database connection) — the same
 * "mock the drizzle chain" style as
 * `0070-hos-789-ai-prompt-brand-voice.test.ts`, which this migration follows
 * line for line (scoped down to a single feature).
 *
 * The contract these tests protect:
 *
 * 1. **Scope.** Each `content`/`rules` update must filter on the exact
 *    pre-HOS-1030 value, not just `feature = 'translate'`. Dropping that
 *    condition would let the migration overwrite a `translate` prompt an
 *    admin already edited from the panel — exactly the failure mode
 *    `aiPrompts.seed.ts`'s `ON CONFLICT DO NOTHING` exists to prevent on the
 *    seed side.
 ...
 *    the two into one write (or one combined predicate) would silently skip
 *    the guardrail update whenever the prose was ever touched.
 ...
 * 4. **Data coherence.** The colocated `.data.json` really has the HOS-1030
 *    URL-preservation wording on the `after` side and NOT on the `before`
 *    side. This guards against the two halves being swapped or regenerated
 *    backwards.
 *
 * This suite is a mocked-`ctx.db` unit test only. The migration's real
 * behavior against a Postgres database (applied once, verified idempotent on
 * a second run, verified it never clobbers an operator-edited column) was
 * additionally proven live against a throwaway clone of `hospeda_template`
 * during HOS-1030 review — see the PR discussion for that run's output.
 *
 * @module test/data-migrations/0100-hos-1030-ai-translate-url-rule
 */
...
import * as urlRule from '../../src/data-migrations/0100-hos-1030-ai-translate-url-rule.js';
import type { SeedMigrationCtx } from '../../src/data-migrations/types.js';

const STUB_ACTOR: Actor = {
    id: 'actor-stub-hos-1030-ai-translate-url-rule-test',
...
const DATA_PATH = path.resolve(
    ...
    '../../src/data-migrations/0100-hos-1030-ai-translate-url-rule.data.json'
);
...
/** One column the migration is expected to issue an update for, given the
 ...
 * column updates `up()` is expected to issue and in what order (`content`
 * before `rules`). Never hardcoded: if the colocated `.data.json` changes,
 * this recomputes instead of silently going stale.
 */
function computeExpectedCalls(): ExpectedCall[] {
    const calls: ExpectedCall[] = [];
    if (before.prompts.translate !== after.prompts.translate) {
        calls.push({
            column: 'content',
            oldValue: before.prompts.translate,
            newValue: after.prompts.translate
        });
    }
    if (before.rules.translate !== after.rules.translate) {
        calls.push({
            column: 'rules',
            oldValue: before.rules.translate,
            newValue: after.rules.translate
        });
    }
    return calls;
}

describe('0100-hos-1030-ai-translate-url-rule', () => {
    ...
    // `feature = 'translate'`. This is what stops the migration from
    // overwriting a `translate` prompt an admin has already edited.
    // -----------------------------------------------------------------------

    it('AC-1: scopes every update to rows that still hold the exact pre-HOS-1030 value', async () => {
        // Arrange
        const { ctx, captures } = buildCtx(() => [{ id: 'row-id' }]);

        // Act
        await urlRule.up(ctx);

        // Assert — one query per expected column, each WHERE referencing
        // both the feature AND the exact frozen old value.
        ...
        captures.forEach((capture, index) => {
            ...
            expect(where).toContain('translate');
            expect(where).toContain(expected.oldValue);
            // The scope must not be satisfied by the feature alone: a WHERE
            // that dropped the value condition would still contain
            // "translate", so the value check above is the load-bearing one.
        });
    });
    ...
    // -----------------------------------------------------------------------
    // AC-3: idempotency — zero matching rows (already migrated, or the row
    // operator-edited) reports "no change" and does not throw.
    // -----------------------------------------------------------------------
    ...
    it('AC-3: is idempotent — reports no changes and does not throw when nothing matches', async () => {
        ...
        // Act
        const result = await urlRule.up(ctx);
    ...
    // -----------------------------------------------------------------------
    // AC-4: data coherence — `after` carries the HOS-1030 URL-preservation
    // wording, `before` does not.
    // -----------------------------------------------------------------------

    it('AC-4: the frozen "after" snapshot carries the HOS-1030 URL-preservation wording and "before" does not', () => {
        // Arrange
        const beforeText = `${before.prompts.translate}\n${before.rules.translate}`.toLowerCase();
        const afterText = `${after.prompts.translate}\n${after.rules.translate}`.toLowerCase();

        // Act / Assert — anchored on a phrase from the HOS-1030 fix that
        // could not have appeared in the pre-existing prompt (the bug report
        // measured zero mentions of "url"/"href"/"link" in the old prompt),
        // not the full sentence, so future copy edits to the surrounding
        // wording don't break this test.
        expect(beforeText).not.toContain('web address');
        expect(afterText).toContain('web address');
    });

    it('is declared non-destructive and belongs to the required group', () => {
        expect(urlRule.meta.destructive).toBe(false);
        expect(urlRule.meta.group).toBe('required');
    });
});
