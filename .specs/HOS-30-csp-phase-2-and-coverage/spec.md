...
# CSP Phase 2 + remaining SPEC-046 coverage gaps

> Migrated from `.qtm/specs/SPEC-142-csp-phase-2-and-coverage/spec.md` on 2026-07-01 as part of the Linear tracking migration. Canonical tracking is now HOS-30.

## Status — verified 2026-07-01

Despite the stale local index showing `draft`, meaningful work has already landed on staging
via an active worktree (`~/projects/WEBS/hospeda-142-csp-phase-2-and-coverage`, branch
`spec/SPEC-142-csp-phase-2-and-coverage`):

- **PR #1857** `[SPEC-142] fix(web): emit CSP header on prerendered pages + baseline audit` —
  **MERGED**. Covers workstream 2.C (home CSP header fix, diagnosed as an Astro prerender /
  adapter interaction) plus part of 2.A (the initial baseline audit crawl).
- Local task tracking (`.qtm/tasks/SPEC-142-csp-phase-2-and-coverage/state.json`) shows
  **8 of 28 tasks completed**:
  - T-001, T-002, T-003, T-004 — CSP middleware audit, public-page crawl baseline, home-route
    header-emission diagnosis, and the fix itself.
  - T-018 — unit test locking in home-route header emission.
  - T-024, T-025, T-026 (all optional 2.E cleanups) — logo-pattern blind-spot sweep, Sentry
    environment config fix (`HOSPEDA_ENV` wiring), staging trace sample-rate bump via
    `PUBLIC_SENTRY_TRACES_SAMPLE_RATE`.
  - T-006 is `in_progress` (setting up a staging authenticated session for the auth crawl).
- **Substantial work remains** — this is genuinely ~30% done, not near-complete:
  - Workstream **2.A** (authenticated crawl): T-007 through T-017 still `pending` — auth flows,
    `/mi-cuenta/*` routes, owner property forms, and the MercadoPago checkout crawl are all
    unstarted.
  - Workstream **2.B** (MercadoPago Brick CSP allowlist integration): entirely `pending`.
  - Workstream **2.D** (Report-Only → enforce flip + 48h soak): T-019 through T-022 `pending`.
  - T-023 (optional Sentry `beforeSend` eval-probe filter) and T-027/T-028 (final report +
    docs close-out) also `pending`.

Anyone picking this up should resume from T-006 (finish the authenticated staging session
setup), not restart from scratch.
