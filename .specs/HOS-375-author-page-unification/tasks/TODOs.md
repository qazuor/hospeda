...
- [x] **T-020** (complexity: 3) — Create the `/autores/[slug]/` page with both content blocks
  ...
  - **Verified in the browser**, which found a pre-existing production bug the tests could not: `GET /api/v1/public/events/author/:id` answered **500 for every author** (handler returned `{ items, total }` where the list factory demands `{ items, pagination }`; an `as never` cast hid it, and the shared test mock returned an envelope the real service never produces). Fixed in `6cfcb8d94`. Until T-021/T-022 land, the pagination links the page emits resolve to 404 — expected, not a defect.
