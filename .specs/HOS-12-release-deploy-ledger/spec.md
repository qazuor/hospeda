...
# Release Deploy Ledger (Env-Var + Merged-Spec Tracking)

> Migrated from `.qtm/specs/SPEC-305-release-deploy-ledger/spec.md` on 2026-07-01 as part of the Linear tracking migration. Canonical tracking is now HOS-12.

> This spec's premise (tracking env-var/migration deltas against the legacy
> CSV/index.json system) is superseded by Linear's per-issue Env vars/Migrations
> sections (see the Spec Implementation issue template). Re-scope or close as
> obsolete when picked up.
