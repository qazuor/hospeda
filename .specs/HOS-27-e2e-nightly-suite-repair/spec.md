...
# E2E Nightly Test Suite Repair

> Migrated from `.qtm/specs/SPEC-105-e2e-nightly-suite-repair/spec.md` on 2026-07-01 as part of the Linear tracking migration. Canonical tracking is now HOS-27.

## Status — verified 2026-07-01

Despite the stale local index showing `draft`, this spec is substantially implemented and
already merged to staging via an active worktree
(`~/projects/WEBS/hospeda-105-e2e-nightly-suite-repair`, branch
`spec/SPEC-105-e2e-nightly-suite-repair`):

- **PR #1856** `[SPEC-105] ci(e2e-nightly): repair suite + re-enable nightly cron` — **MERGED**.
- Local task tracking (`.qtm/tasks/SPEC-105-e2e-nightly-suite-repair/state.json`) shows
  **5/6 tasks completed**:
  - T-105-01 (Phase 0 failure audit) — done, produced `docs/test/e2e-failure-audit-2026-05-13.md`.
  - T-105-02 (Phase 1 local repro) — done.
  - T-105-03 (Phase 2a credential drift) — done (N/A, no credential drift found).
  - T-105-04 (Phase 2b schema/API drift) — done, 4 drifts fixed (conversation creation SQL
    columns, host inbox endpoint + response type, list-response shape mismatches).
  - T-105-05 (Phase 2c infra drift) — done, 3 stale CI script paths fixed.
  - T-105-06 (Phase 3 re-enable + observe) — **`in_progress`**. The cron was re-enabled
    (`0 5 * * *` UTC = 02:00 ART) but the 3-consecutive-successful-run observation window had
    not been confirmed as of the last local state update.
- **Remaining work to fully close**: confirm 3 consecutive green nightly runs (or fix whatever
  surfaces if they're not green), then finalize `docs/test/e2e-failure-audit-2026-05-13.md` as
  the closing record referenced by the acceptance criteria below.
- One test (`host-07e`) is guarded by `test.fixme()` per DRIFT-1, deferred — a legitimate
  exception per this spec's own risk section ("Tests for retired features... need full deletion
  not fixing").

Given the near-complete state, treat this as a small residual task (observe + close), not a
fresh investigation.
