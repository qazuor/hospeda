...
3. **Lo que toca plata no se ejecuta solo.** Toda divergencia de monto, estado o cobro **abre una
   ...
   **El motivo es parte de la regla**: el corpus escribe ~~quince~~ dieciséis marcas distintas sobre la misma
...
esta pasada.** El `02` §2.5 —los ~~quince~~ dieciséis motivos— **no figuraba en la columna de capítulos de
...
dieciséis** motivos los abren actos de otras unidades —`S18` (B8), `S21` (B10), **que desde
`DEC-RF-006` abre dos**, las seis comprobaciones del `09` §3 (B12) y **el reintento del barrido
sobre las salvedades 1 y 4 del `09` §3, que abre el 16** (FASE 8 completa, `F-8CB1-013`, owner
2026-09-25; esas dos salvedades están en la columna de B10)—, así
