...
| 15 | el **cambio de plan** con una cuota impaga viva — la que el proveedor reintenta **mientras la predecesora sigue en `GRACE_PERIOD`**, y también la que el cliente puede pagar a mano (cap. 03 §7, `MP1` o `MP4`): son las **dos puertas del mismo pago** mientras esa ventana dura, y el desenlace es idéntico — **si la predecesora ya está `SUSPENDED` (pagador con tarjeta), `S6` cerró la puerta del proveedor y sólo queda la manual** (`DEC-SUB-019`) | que **el cobro de la cuota impaga puede entrar igual**, antes de confirmar — **y qué pasa con esa plata según lo que el cliente haga**: si termina el checkout **se le devuelve**, si lo abandona **le queda** y le paga el período que está usando. Nunca reactiva la suscripción vieja mientras el cambio esté en curso. **Y que la devolución no es instantánea**: la confirma una persona (`DEC-RF-002`), así que lleva unas horas | cap. 12 §5.3, `NUCLEO/07` §6, cap. 03 §3.2 (`S19`) |
