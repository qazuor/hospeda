...
| `COMPLEMENTO_CON_PERÍODO_COBRADO` | **depende del disparador, y es el único de la tabla que no se lee en una casilla** (`DEC-RF-004`). **`DEVOLVER`** cuando la instancia llegó a `CANCELLED` porque **se revocó el grant que era su título** —la tercera cláusula de `A5`—: ahí el cliente no hizo nada y pierde días que pagó. **NO devolver** en los otros **tres** disparadores —`A6`, y la primera y la segunda cláusula de `A5`—, y no es una laguna: `B/16` §4.4 decidió que *«el período ya pagado no se reembolsa»*, con `DEC-GRANT-001` y el §3.4 de ese capítulo como precedente. **Los cuatro disparadores están enumerados uno por uno, con su lado, en `B/03` §3.2**, *«la propuesta del 14 depende del disparador»*. La persona confirma la propuesta que le toca o ve la razón para apartarse. **Y la fila dice en voz alta CUÁNDO apartarse, porque el disparador 2 no es homogéneo** (`DEC-RF-004`, la condición obligatoria de su ampliación): de las **doce** transiciones que sacan al título de las filas vivas (`B/16` §4.3), **cinco no son un acto del cliente** —`S17`, que es nuestra; `S12` cuando su `CANCEL_SCHEDULED` lo puso `S26`; y `S25`, `S27` y `S28`, que son el fin de una pausa sobre un plan retirado y las dos terminales de la discontinuación—. **Por el criterio de la propia decisión esas cinco irían a `DEVOLVER` y quedan en `NO DEVOLVER` por MECANISMO**: `S21` conoce la cláusula de `A5` que disparó, **no cuál de las doce mató al título tres saltos antes**, y hacérsela llegar es mecanismo nuevo. **Un default es una propuesta y no una sentencia**; lo que no puede ser es una propuesta que contradice el criterio **en silencio**, y por eso esto se lee acá y no se deduce. **Y `S27` y `S28` son las dos que más lo necesitan**: no traen el piso de **60 días** con el que las otras llegan compensadas —`DEC-SUB-018` se lo niega a la suspendida— así que en esos dos casos lo único que la persona tiene delante es esta propuesta |
