...
**Y `G-R6-B` es el guard de LA LISTA de esa misma columna —sus dos mitades—, que existe porque el
...
falló**, porque un guard con dos predicados y un solo texto afirma más de lo que verificó — la
misma regla con la que esta decisión rechazó el segundo guard, abajo. **La razón entera, con sus
dos casos que lo hacen fallar a propósito —uno por mitad— y lo que sigue sin verificar, está escrita
