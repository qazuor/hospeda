...
propia transición; queda en *«lo que esta mitad NO cierra»*. **No la llevan las
...
*«la disputa se resolvió a tu favor; tu suscripción sigue cancelada; podés volver cuando
quieras»*; al correr `P7` por `reimbursed`, *«la disputa se resolvió; el cargo era correcto; podés
volver a suscribirte desde acá»*. ⚠️ **Qué lectura ve `settled` no está escrito** (*«lo que esta
mitad NO cierra»*).
...
- ~~**Con qué transición se corta la sucesora cuando `S6` corre por un contracargo sobre su
  ...
  `ABANDONED` desde `PENDING_AUTHORIZATION` y `SUSPENDED` desde `ACTIVE`; el saldo diferido se
...
- ⚠️ **Lo que `S31` deja abierto** (FASE 8 completa, pendiente 8):
  1. **Quién cierra la sucesión cuando `S31` deja a la sucesora `SUSPENDED`.** Sigue viva, así que
     ...
     owner lo pide *«como en `S3`»*, donde la sucesora sí muere.
  2. **Quién reintenta si la cancelación o el correo fallan de forma transitoria.** `S31` no está
     ...
     `PENDING_AUTHORIZATION` y a `S6` desde `ACTIVE`, y elegir no está decidido.
