...
> escribe como un **conjunto de filas** —y en esta tabla hay **seis**: `S13`, `S20`, **`S21`** y
> las **tres** de la discontinuación, `S26`, `S27` y `S28` (decía «cinco» y omitía `S21`;
> recontado el 2026-09-25, FASE 8 completa, `F-8CD1-016`)— tiene que decir si alcanza también a las de
> complemento, porque son filas de `subscription` como cualquier otra y **entran por pertenencia
> sin que nadie lo decida**. **Las seis lo contestan, y no las seis igual**: `S13` dice
> **«principal»** y no las alcanza; `S20` y `S21` dicen **«de complemento»** y **sólo** las alcanzan; las
...
- **si falla de forma transitoria, la cancelación no se ejecuta en esta corrida y se reintenta**
  ...
  FASE 8 completa, owner 2026-09-25, residuo A de `R12`) y, por la salvedad 1, la cancelación de `A5`/`A6` sobre la suscripción de
...
remitiendo acá, **las filas que cancelan en el proveedor**: `S3`, `S6`, `S11`, `S13`, **`S16`**, `S17`,
`S20`, `S22`, `S23`, `S24`, `S25`, `S26`, `S27`, `S28` **y `S31`** (FASE 8 completa, pendiente 8,
owner 2026-09-25; **`S16`**, FASE 8 completa, owner 2026-09-25) —**quince**, recontadas sobre esta
lista—, y `A5` y `A6` del §8. ~~**`S31` no está en ninguno de los dos grupos de la rama
...
### 3.4 Las cuatro precisiones que `M-SUB-01` pedía sobre `PENDING_AUTHORIZATION`
...
`S13`, si la fila es la predecesora de una sucesión que se consuma o le cae un grant, **o por
`S26`**, que la manda a `CANCEL_SCHEDULED` si `SUPER_ADMIN` discontinúa la vertical (`B/10` §4.3);
en una fila de complemento, además, por `S20` o `S21` (§3.2; FASE 8 completa, `F-8CD1-011`). **Mientras
...
por S10 — **o termina, sin reanudar, por `S22` (la persona pide la baja), por `S25` (llega el fin
de la pausa sobre un plan que ya no se presta, `DEC-SUB-015`), por `S13` (le cae un grant) o por
`S17` (la sucede otra fila que se autorizó)**; en una fila de complemento, además, por `S20` o
`S21` (§3.2; FASE 8 completa, `F-8CD1-011`: este párrafo nombraba dos terminales y omitía `S25` y
`S17`). ~~Las dos terminales~~ **Las cuatro terminales** mandan la fila a `CANCELLED` y **cierran
la pausa** —el `fin_real` lo escriben `S22`, `S25`, **y desde el 2026-09-25 también `S13` y
`S17`** cuando la fila que cancelan estaba `PAUSED` (FASE 8 completa, `F-8CD1-007`)—, así que la que sostiene lo que sigue es S10. **Y una pausa por cortesía se corta además por `S6`**, si un
...
~~y que el proveedor la haya cancelado al rechazarlo~~ —desde la FASE 8 completa ya no exige que el
proveedor la haya cancelado: la cancela él (owner 2026-09-25)—, y un pagador manual **no tiene
autorización en el proveedor** (`B/06` §7), así que ~~las dos mitades son falsas~~ esa condición es
