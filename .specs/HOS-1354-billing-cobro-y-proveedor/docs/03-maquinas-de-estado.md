...
llegó a cobrar. **Lo que la marca NO hace es decidir**: `B/16` §4.4 ya decidió que la regla es no
devolver, y el listado lo propone así (`B/19` §6). La marca es lo que pone el caso delante de una
persona con el pago y el monto al lado, en vez de dejarlo esperando a que el cliente reclame.
