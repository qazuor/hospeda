...
> `B/02` §2.2 y §2.5). Este corpus escribe **catorce** marcas distintas sobre la misma casilla y
...
adentro.** El motivo es **`COMPLEMENTO_CON_PERÍODO_COBRADO`** (motivo 14) y la condición es que
...
##### La propuesta del 14 depende del disparador, y los disparadores son CUATRO
...
| # | disparador | qué pasó | propuesta |
|---|---|---|---|
| 1 | **`A5`, primera cláusula** — *«se da de baja»* | el cliente dio de baja el complemento | **NO DEVOLVER** |
| 2 | **`A5`, segunda cláusula** — *«queda huérfana»* (la condición de `B/16` §4.2, con sus **tres** mitades) | murió el **objetivo** del addon, y ni una sucesión ni un grant vivo lo relevan | **se PARTE**: **DEVOLVER** si al objetivo lo mató la **discontinuación de su vertical** —`S25`, `S27` o `S28`—, **NO DEVOLVER** en los demás caminos (abajo) |
| 3 | **`A5`, tercera cláusula** — *«se revoca el grant del que cuelga el ancla que era su título»* | **el cliente no hizo nada** y pierde los días que pagó | **DEVOLVER** |
| 4 | **`A6`** — se borra la ficha destino | el cliente borró su propia ficha, y `DEC-ADDON-001` ya declara que el complemento *«se consume»* | **NO DEVOLVER** |
...
| camino dentro del disparador 2 | quién lo causa | propuesta |
|---|---|---|
| **`S25`, `S27` y `S28`** — la discontinuación de la vertical | nuestra, **y la causa es legible en el acto mismo** | **DEVOLVER** |
| **`S17`**, y **`S12` cuando su `CANCEL_SCHEDULED` lo puso `S26`** | nuestra, **y la causa NO es legible en la transición que mata al título** | **NO DEVOLVER**, por mecanismo y dicho en voz alta (`B/19` §6) |
| las otras **siete** transiciones, y la mitad de `S12` que no viene de `S26` | el cliente actuó | **NO DEVOLVER** |
...
que la marca *«lleva de qué disparador vino»*, y lo que la partición hace es que ese dato tenga que
distinguir **cinco** casos donde distinguía cuatro — el quinto es *«la orfandad la causó la
discontinuación»*, y lo escribe la transición que mata al título, que es la que lo sabe. **Ninguna
transición nueva, ninguna llamada nueva y ningún dato que haya que trazar hacia atrás.** Lo que sí
queda abierto es **dónde vive ese dato**: la enumeración de columnas de `reconciliation_mark`
(`B/02` §2.2) no lo nombra, y eso **es anterior a la partición** — `S21` ya tenía que distinguir
cuatro disparadores desde que `DEC-RF-004` le quitó el default único.

> **La rama que devuelve POR REVOCACIÓN DEL GRANT no tiene población vacía, y conviene decir dónde
> vive**, porque el §
...
|---|---|---|---|---|
| MP1 | `AWAITING` | el admin registra el pago | `REGISTERED` | la suscripción sale de `GRACE_PERIOD` por `S5` — **o, si la cuota es la PRIMERA y la fila está todavía en `PENDING_AUTHORIZATION`, es este registro el que la habilita a llegar a `ACTIVE`**, por la fila que el capítulo 13 todavía debe (§7.2, *«cómo entra el grace»*): ahí no hay grace del que salir, porque no lo hubo — **o queda pendiente por `S19`, si es la predecesora de una sucesión en curso**: el efecto de `MP1` es el de `S5` y hereda su condición, porque el daño no depende de por qué puerta entró el pago. **`S19` lo admite por su propio evento**, que nombra las dos puertas (§3.2): sin eso la derivación apuntaba a una fila que no podía recibirlo. **Y avanza un ciclo la fecha del próximo cobro** (`B/02` §2.2): registrada la cuota de este período, lo que queda por cobrar es el siguiente. Es **acá** y no en `S5`, cuya celda de efectos es *«se apaga el reloj»* y nada más (§7.2, *«qué mueve la fecha»*) |
...
| *«los estados terminales de una suscripción no se barren»* y sus puertas —**nueve** cuando se escribió esta fila, **quince** hoy— | `B/09` §3, y `B/16` §4.4 que las contó | **sigue verdadera**: `DECLARED_UNPAID` es del `manual_payment` y nunca estuvo en esa tabla, cuyos sujetos son `CANCELLED`, `ABANDONED` y `CHARGE_DECLINED` | sin tocar |
