...
> FASE 8 completa, owner 2026-09-25) y
...
- **si falla de forma transitoria, la cancelación no se ejecuta en esta corrida y se reintenta**
  ...
  que ya salió no se repite**: precisión 3, abajo— y la relectura después, **hasta tres corridas
  seguidas**; recién a la tercera abre la marca con motivo `CANCELACIÓN_SIN_CONFIRMAR` (`B/02`
...
`S20`, `S22`, `S23`, `S24`, `S25`, `S26`, `S27` y `S28`, y `A5` y `A6` del §8. **No la llevan las
...
1. **Bloquea la LLAMADA, no la relectura.** Las filas cancelan con la regla de relectura de
   ...
   hay preapproval ni débito que detener (`B/06` §7), así que en `S3`, `S6`, `S23`, `S24`, `S27` y
   `S28` la condición corre sólo sobre el pagador con tarjeta.
...
en meses: FASE 8 completa, `F-8CB1-001`, owner 2026-09-25).
...
pediría guardar cuál era; no se agregó (*«lo que esta mitad NO cierra»*).
...
comprobación de pagos acreditados del barrido (`B/09` §3).
...
- ~~**La cancelación de `S11` o `S26` que no llegó, mientras la fila sigue en `CANCEL_SCHEDULED`**
  ...
  **terminales** (`B/09` §3, salvedades 1 y 4), y ésta todavía no lo es: lo será por `S12`, que
  ...
  `CANCEL_SCHEDULED`.
- **Dónde se cuentan las «tres corridas seguidas»** del reintento de una cancelación nuestra, y las
  de `B/09` §6.2. **Es decisión pendiente del owner** y no se resolvió acá (`B/09`, *«lo que este
  capítulo NO cierra»*).
...
- **Un contracargo sobre una fila que no está en `ACTIVE` ni `GRACE_PERIOD`** —`CANCEL_SCHEDULED`,
  ...
  (§6, `P6`); si corresponde algo más, `DEC-SUB-020` no lo dice.
- **Un contracargo sobre un pago ya `PARTIALLY_REFUNDED`.** `P6` sale sólo de `SUCCEEDED`; cubrirlo
  pide guardar a qué estado vuelve `P7`, y no se agregó sin una decisión.
