...
| **Crear una segunda app de prueba con producto `Checkout API`** | **La causa está medida**: `GET /applications/{id}` declara `urn:mp:online:payments:refunds/`**`read-only`** contra `urn:mp:online:subs-recurring:pre-approval/`**`read-write`**. No es el `write` del panel (está tildado): es el **scope por recurso**, que viene del producto de la app. **NO le cambies el producto a la app actual**: tiene 8 suscripciones vivas del reloj, y además **todo lo medido hoy se midió con `producto = Suscripciones`**, así que cambiarlo convierte las 38 filas en no-medidas (`S-MP-02`). Con la segunda app, `GET /applications/{id2}` dice **por API** si conserva `subs-recurring:*/read-write` — o sea que se puede saber si migrar es seguro **sin tocar nada** |
