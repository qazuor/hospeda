...
| **Crear una segunda app de prueba con producto `Checkout API`** | La app actual **no puede escribir nada** sobre un pago —ni un `PUT {"description":"x"}` inocuo— aunque **sí tiene el permiso `write`** (se miró el panel: `read`, `offline access` y `write`, los tres tildados). Lo único que la distingue es su **producto: `Suscripciones`**, no `Checkout API`. **No le cambies el producto a la app actual**: tiene 8 suscripciones vivas del reloj y podría perder el acceso a `preapproval`. Una segunda app deja todo intacto. Hasta entonces `RF-1`/`RF-2`/`RF-3` no se pueden medir y `M-LEGAL-01` queda **sin respuesta técnica** |
