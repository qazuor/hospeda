...
| **Dar permiso de ESCRITURA sobre `payments`** a la aplicación | No es un problema de reembolsos: la app **no puede escribir nada** sobre un pago — ni un `PUT {"description":"x"}` inocuo da otra cosa que `401`. Lee pagos y escribe preapprovals sin problema. Hay que mirar los **scopes de la aplicación** en el panel de desarrolladores. Hasta entonces `RF-1`/`RF-2`/`RF-3` no se pueden medir, y `M-LEGAL-01` (derecho de revocación con devolución) queda **sin respuesta técnica** |
