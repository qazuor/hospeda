...
| `EX-39` | tampoco se mueve la fecha de una `pending`: la inmutabilidad **no depende del estado** | sobre el pagador con tarjeta *«la salida no es corregir: es no llegar a ese caso»* — la ventana de la sucesora **vence al inicio del día del cobro de la predecesora**, y con menos de 24 h el cambio se ofrece después del cobro (**regla fijada el 2026-09-24**). Sobre el pagador manual no hay ventana reducida: se corrige, porque la fecha es nuestra | `12-suscripcion.md` §5.4 · `DEC-SUB-017` |
