...
| **la disputa se resolvió a favor de la persona** ✚ | transaccional | **al leer `settled`** sobre un pago en `CHARGED_BACK` (cap. 03 §6, épica de billing): la disputa se perdió para nosotros y el pago queda en `CHARGED_BACK`. **Texto base, del owner**: *«La disputa se resolvió a tu favor; tu suscripción sigue cancelada; podés volver cuando quieras.»* ⚠️ Qué lectura ve `settled` no está escrito (cap. 03, *«lo que esta mitad NO cierra»*, épica de billing) | FASE 8 completa, pendiente 8, owner 2026-09-25; cap. 19 §4 fila 10-ter (épica de billing) |
...
| **cambio de plan con una cuota en reintento** | transaccional | **antes de confirmar el cambio**, mientras la predecesora siga viva. **Vale igual si la cuota se paga a mano** —transferencia registrada por `MP1` o por `MP4`, cap. 03 §7 (épica de billing)—: es la segunda puerta del mismo pago, **y es la del pagador manual**: las dos puertas son una por método de pago. **Sobre un pagador con tarjeta ya `SUSPENDED` este aviso no corre**: `S6` canceló el preapproval al suspender y no hay cuota que pueda entrar (`DEC-SUB-019`). ⚠️ **Desde `DEC-SUB-021` (owner 2026-09-25) no tiene población al confirmar**: en el grace el cambio de plan no se ofrece, así que no se confirma ninguno con una cuota en reintento. La cuota puede aparecer **después**, si la predecesora entra en el grace durante la ventana (`S4`); qué correo le corresponde entonces **no está decidido** (cap. 12 §5.3, épica de billing) | cap. 12 §5.3 (épica de billing) |
...
(`V/02` §4.1) y ~~lo que vuelve es una ficha vacía~~ **la ficha no vuelve**: quedó en `PURGED`,
que es final y no la republica nada (`V/03` §9, `PB9`; FASE 8 completa, `F-8CA2-008`, owner
2026-09-25). Anunciar la vuelta y callar eso es prometer una
