...
| **reapertura tras un pago manual tardío** | transaccional | al reabrir (`MP4`, cap. 03 §7.1, épica de billing). **Dice dos cosas y ninguna es opcional**: que el servicio volvió y desde cuándo, y **qué pasó con la ficha** — vuelve sola si estaba archivada **y el cupo del plan le alcanza** (`PB7`), y si el hard delete ya corrió, **que el contenido no vuelve** | cap. 03 §7.1 (épica de billing), `V/02` §4.1 |
