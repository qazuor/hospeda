...
| 6 | el **listado accionable** | cap. 19 (billing) §6 | ordena por motivo, pone adelante los **cinco** que devuelven plata **y con ellos la marca del 14 cuya rama es `DEVOLVER`** (`DEC-RF-004`, cap. 19 §6) y muestra **todos los pagos colgados de la marca con su monto total** y **el default de lo que el sistema propone** (`DEC-RF-003`) — que en el motivo **14** no se lee por motivo sino **por rama**, o sea por cuál de los cuatro disparadores de `S21` corrió **y, en el de la orfandad, por cuál de las doce transiciones mató al título** (`DEC-RF-004` y su ampliación) |
