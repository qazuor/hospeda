...
| **abrir** la marca —con motivo **`COMPLEMENTO_CON_PERÍODO_COBRADO`** y el pago colgado— sobre la **suscripción de complemento** que muere con un período cobrado **sin terminar** | **`S21`**, en el mismo acto en que la lleva a `CANCELLED`. **Y la marca lleva la RAMA con la que nació**, porque de ella depende lo que el listado propone: devolver si la instancia murió por la **revocación del grant**, y también si murió **huérfana porque a su título lo mató la discontinuación de la vertical** —`S25`, `S27` o `S28`, que la escriben en su propio acto—; no devolver en el resto (`DEC-RF-004` y su ampliación del 2026-09-23). **Son cinco casos a distinguir y no cuatro**: los cuatro disparadores, con el de la orfandad partido en dos | `B/03` §3.2, `B/02` §2.5 **motivo 14**, `B/16` §4.4, `DEC-ADDON-004`, `DEC-RF-004` |
