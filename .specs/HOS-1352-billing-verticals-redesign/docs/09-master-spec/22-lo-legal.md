...
**Ya está resuelto por el capítulo 07 y no hacía falta un abogado**: el §44 exige que el intento
de notificación quede registrado, y el outbox guarda destinatario, plantilla, estado,
identificador del proveedor e intentos. La clave de una-sola-vez del capítulo 07 §2 —el sujeto más
el hito, `sub:<id>:aumento:-30d`— hace que **cada aviso sea localizable por lo que es**, no por
una búsqueda de texto.

Y los tres avisos de aumento de `DEC-MP-002` son **transaccionales no suprimibles** (cap. 07
§4.1): no los apaga el opt-out ni el tope diario. Un aviso obligatorio que el opt-out pudiera
silenciar no sería evidencia de nada.
