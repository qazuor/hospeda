...
| 7 | [`docs/06-mp-validation-matrix.md`](./docs/06-mp-validation-matrix.md) | Qué sabemos de Mercado Pago, medido: **94 filas, 90 cerradas, 4 `UNKNOWN`** (24/09 a la noche, con `EX-40`) |
...
| FASE 1C — experimentación contra Mercado Pago | 🟡 **94 filas · 90 cerradas · 4 `UNKNOWN`** (`EX-40` sumada a las 93 recontadas el 2026-09-24 con `contar-filas-de-la-matriz.py`, **después de arreglarlo**: descartaba en silencio cuatro filas cuyo identificador está en negrita) — **tres** son el camino del cobro fallido (`RN-3` en curso, `GR-1`, `GR-2`), que el proveedor no deja fabricar a voluntad, y `RF-3` necesita un pago de **más de 180 días** que todavía no existe |
