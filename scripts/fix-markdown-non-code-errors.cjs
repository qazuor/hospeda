...
function fixMD036(content) {
  ...
  const result = lines.map((line, index) => {
    ...
    if (match) {
      ...
      if (text.includes('✅') || text.includes('❌') || text.includes(':') || text.includes('Issue:')) {
        return line;
      }
...
function fixMD029(content) {
  ...
  const result = lines.map((line) => {
    ...
    if (listMatch) {
      ...
      const expectedNum = parseInt(originalNum);
...
function main() {
  ...
  console.log(`  Total fixes: ${stats.md036Fixed + stats.md029Fixed + stats.md024Fixed + stats.md051Fixed + stats.md033Fixed}`);
