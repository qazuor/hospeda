...
import { collectCode, collectPackages, scanWorkspace } from './collectors/code.js';
import { collectDebt } from './collectors/debt.js';
import { collectGit } from './collectors/git.js';
import { collectI18n } from './collectors/i18n.js';
import { collectLinear } from './collectors/linear.js';
import { collectPrs } from './collectors/prs.js';
...
import { renderHelp } from './report/help.js';
import {
    drawCode,
    drawDebt,
    drawGit,
...
import { collectDebtDetail, drawDebtDetail } from './reports/debt-detail.js';
import { collectStalled, drawStalled } from './reports/linear-stalled.js';
import { collectOpenPrs, drawOpenPrs } from './reports/prs-open.js';
import { collectUntranslated, drawUntranslated } from './reports/untranslated.js';
import {
    type Period,
    type PeriodSpec,
    REPORT_IDS,
...
program
    ...
    .argument('[repo]', 'ruta del repositorio', process.cwd())
    ...
    .action(async (repoArg: string, opts) => {
        ...
        if (resolved === null) {
            process.stderr.write(pc.red(`no es un repositorio git: ${repoArg}\n`));
            process.exitCode = 1;
            return;
        }
        ...
        if (needsWorkspace) {
            ...
            if (ws.ok) {
                ...
                if (wants('packages')) {
                    ...
                    const cases = scan.ok ? scan.data.casesByPackage : new Map<string, number>();
        ...
        if (opts.diff === true) {
            ...
            if (before === null) {
                ...
                if (rows.length === 0) {
                    process.stdout.write(`  ${pc.dim('sin cambios en las métricas comparadas')}\n`);
                } else {
                    process.stdout.write(`  ${pc.dim(`comparando contra ${before['ts']}`)}\n\n`);
                    for (const r of rows) {
                        const delta =
