...
- T-007 and T-010 are mandatory quality-gate tasks. T-007 is GREEN. T-010 automated checks are GREEN (typecheck + biome + 20 tests pass); the pending item is manual visual smoke (browser) deferred to owner / Phase 6.
...
- T-010 visual smoke (browser check of polished card + responsive reflow + empty state) is PENDING owner — cannot be run without a browser. All automated gates are green.
