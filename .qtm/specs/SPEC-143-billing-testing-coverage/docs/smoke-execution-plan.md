...
### 📋 Audit: enforcement middleware wiring matrix (2026-05-24)

Triggered by Finding #15 (MAX_PHOTOS unenforced). Inventoried every `enforce*Limit` / `gate*` middleware and grepped routes/schemas to determine which have a target endpoint or schema field today.

| Middleware | Type | Routes wired | Has target? | Action |
|---|---|---|---|---|
| `enforceAccommodationLimit` | Limit | 3 (create, createDraft, host-onboarding/start) | Yes | ✅ Already wired (host-onboarding/start added by Finding #8 fix) |
| `enforcePromotionLimit` | Limit | 1 | Yes | ✅ Already wired |
| `enforceFavoritesLimit` | Limit | 1 | Yes | ✅ Already wired |
| `enforcePhotoLimit` | Limit | 0 | **Yes** (`POST /api/v1/admin/media/upload`) | 🔴 **Fix in this PR — Finding #15** |
| `enforcePropertiesLimit` | Limit | 0 | No (complex accommodations vNEXT) | ⏸ Defer; stub for future feature |
| `enforceStaffAccountsLimit` | Limit | 0 | No (staff mgmt vNEXT) | ⏸ Defer; stub for future feature |
| `gateFavorites` | Tourist | 0 | Partial — bookmarks endpoint exists but `enforceFavoritesLimit` already gates on the same route; redundant entitlement gate | ⏸ Defer; redundant with limit middleware today |
| `gateAlerts` | Tourist | 0 | No (price alerts vNEXT) | ⏸ Defer |
| `gateComparator` | Tourist | 0 | No (comparator vNEXT) | ⏸ Defer |
| `gateReviewPhotos` | Tourist | 0 | No (review photos endpoint TBD) | ⏸ Defer |
| `gateSearchHistory` | Tourist | 0 | No (search history vNEXT) | ⏸ Defer |
| `gateRecommendations` | Tourist | 0 | No (recommendations vNEXT) | ⏸ Defer |
| `gateExclusiveDeals` | Tourist | 0 | No (exclusive deals vNEXT) | ⏸ Defer |
| `gateEarlyEventAccess` | Tourist | 0 | No (early-access events vNEXT) | ⏸ Defer |
| `gateRichDescription` | Accom | 0 | **No** — no `richDescription`/`markdown` field exists on the accommodation schema | ⏸ Defer; stub for future field |
| `gateVideoEmbed` | Accom | 0 | **No** — no `videoEmbedUrl` field exists on the accommodation schema | ⏸ Defer; stub for future field |
| `gateCalendarAccess` | Accom | 0 | **No** — no calendar endpoint exists under `/api/v1/protected/accommodations/:id/calendar` | ⏸ Defer; stub for future endpoint |
| `gateExternalCalendarSync` | Accom | 0 | **No** — no calendar sync endpoint | ⏸ Defer; stub for future endpoint |
| `gateWhatsAppDisplay` | Accom | 0 | UI-only (display flag on accommodation data, no endpoint) | ⏸ Defer; UI gate is the correct surface |
| `gateWhatsAppDirect` | Accom | 0 | UI-only (direct contact action, no separate endpoint) | ⏸ Defer; UI gate is the correct surface |
| `gateReviewResponse` | Accom | 0 | **No** — no review-response endpoint (`POST /accommodation-reviews/:id/response` doesn't exist) | ⏸ Defer; stub for future endpoint |

**Verdict**: 21 middlewares audited, 5 already wired, 1 actionable gap (Finding #15 = `enforcePhotoLimit`), 15 are stubs for future features/fields/endpoints. The earlier hypothesis behind Finding #7 (entitlement-enforcement gap) **was correct in shape but overestimated in impact** — most "unwired gates" guard things that don't exist yet. Wiring them now would be premature: when the feature lands, the wiring happens at that time alongside the route definition.

**This PR fixes**: only `enforcePhotoLimit` on `POST /api/v1/admin/media/upload` (Finding #15).

**Closes Finding #7**: with the inventory above, the original entitlement-gap hypothesis is RESOLVED (downgrade from "potential security gap" to "stubs awaiting features"). No further action needed for Finding #7 in this PR.
...
| 12 | 1.15 verification | Navigation inconsistency on `/mi-cuenta/*` for users in intermediate onboarding state (USER role promoted to HOST when the DRAFT was created, but no ACTIVE/published accommodation yet). The **left sidebar** of `/mi-cuenta/*` does NOT show "Mis propiedades" (or equivalent) link, but the **top-right user dropdown** DOES surface it. Both surfaces should be driven by the same predicate (likely "user has at least one accommodation owned regardless of lifecycleState" OR "user has HOST role"); divergence means a host who started but didn't finish onboarding can't navigate back to their draft from the main /mi-cuenta dashboard, only via the user menu. Discovered post-deploy of Finding #9 fix while validating the admin GET. **Action**: audit the `/mi-cuenta` left-sidebar nav predicate in `apps/web/src/components/account/*` (or equivalent) and align it with the top-right user dropdown's predicate. Likely a check against `lifecycleState=ACTIVE` accommodations vs. an unfiltered ownership check. | TBD | Out of billing scope but found during SPEC-143 smoke; bundles with Finding #9 user-journey work. |
| 13 | 1.15-A.2 attempt | UI banner "Galería de Imágenes - Límite alcanzado: Has alcanzado el límite de tu plan actual" appears on the accommodation edit page with **zero photos uploaded** (count=0/5 for owner-basico). Either the client-side limit-check reads the wrong count, or it's reading the hardcoded `getGalleryCap(accommodation) === 50` as the denominator and somehow comparing wrong. Either way, the UI gates the user out of uploading even the first photo, making 1.15-A.2 (MAX_PHOTOS=5) UI-untestable. **Action**: trace the `LimitReachedBanner` (or similar) component in `apps/admin/src/routes/.../accommodation-edit` / gallery panel and fix the count source. | TBD | Discovered while trying to validate MAX_PHOTOS limit on draft `a404c854`. |
| 14 | 1.15-A.2 attempt | 🔴 **BLOCKING user journey**: a HOST role cannot save updates to their own draft through the admin panel — server responds `"Insufficient admin permissions"`. The onboarding flow (`/host-onboarding/start`) redirects the user to the admin panel after draft creation, but the admin update endpoint requires `ACCOMMODATION_UPDATE_ALL` (admin permission) while a HOST only has `accommodation.update.own`. Net effect: a real HOST cannot complete their first accommodation onboarding by themselves — they get stuck on the edit form with no save path. **Architecture question**: should the onboarding redirect target a HOST-facing edit view (using protected-tier endpoints) instead of the admin panel? Or should the admin endpoint accept ownership as an alternative to ACCOMMODATION_UPDATE_ALL? **Non-blocking for SPEC-143 smoke** because we can bypass with curl/admin user, but this MUST be fixed before HOST onboarding can ship. Tracked separately from billing scope. | TBD | Server logs would show the actual permission check (likely `_canUpdate` in `accommodation.permissions.ts`). |
| 15 | 1.15-A.2 attempt | 🔴 **BLOCKING for billing enforcement**: `MAX_PHOTOS_PER_ACCOMMODATION` plan-limit is **NOT enforced at the API level**. The `enforcePhotoLimit` middleware (in `apps/api/src/middlewares/limit-enforcement.ts`) is implemented, fully tested (14 tests in `limit-enforcement-photo.test.ts`), and documented (`apps/api/docs/entitlements/limit-enforcement.md`) — but it is wired to **zero production routes** (grep shows usage only in tests + docs + dist). The actual gallery upload route `POST /api/v1/admin/media/upload` uses `getGalleryCap(entityType)` from `@repo/schemas` which is a **hardcoded per-entity cap of 50** for accommodations. The code itself acknowledges this with a `TODO(billing): respect billing-tier addon entitlements for gallery cap`. Result: any user on any plan (free or paid) can upload up to 50 photos per accommodation; the plan-tier differentiation is purely UI-gated (Finding #13's banner) and trivially bypassable via direct API call. **Fix landed in this PR**: inlined the plan-limit check inside `POST /api/v1/admin/media/upload` (the middleware as-designed reads `c.req.param('id')` which doesn't fit this route's multipart-form-body shape). The check uses the same `checkLimit` helper + `ServiceError(LIMIT_REACHED, …)` envelope as the wired middlewares (Finding #10 fix applied). Semantics: only fires when `entityType === 'accommodation' && role === 'gallery'` AND `actor.id === accommodation.ownerId` (admin uploading on behalf of an owner bypasses the plan limit as a documented v1 trust override — admin ops are manual interventions). `getRemainingLimit` returns `-1` (unlimited) when the actor's plan does not cap photos (e.g., admins with no billing customer), so the bypass falls through naturally via the standard `checkLimit` path. Hardcoded gallery cap of 50 remains the absolute upper bound. Regression test deferred — same rationale as Finding #8: full integration test requires billing+entitlement mock infrastructure not yet in place for this route; fix is validated post-deploy by re-running 1.15-A.2 against owner-basico (MAX_PHOTOS=5). | DONE | Auditing the 21 middlewares while fixing this surfaced that #15 is the ONLY actionable enforcement gap today — all other unwired middlewares (gateRichDescription, gateVideoEmbed, gateCalendarAccess, etc.) protect features that don't have endpoints or schema fields yet. See audit matrix above for the full picture. Closes Finding #7 (downgrade from "potential security gap" to "stubs awaiting features"). |
