...
| D17 | **Accepted risk — unbounded `adminTours` map growth (review 2026-06-04)**: an authenticated user can fabricate distinct `tourId` strings (max 100 chars) at 30 req/min, inflating only THEIR OWN settings JSONB row (self-DoS, no cross-user impact, no escalation). Mitigations: per-call body bounds + the 30/min rate limit. Same accepted pattern as SPEC-175's `seenIds`. A stored-map cap or catalog-id validation can be added later if abuse is ever observed. |
