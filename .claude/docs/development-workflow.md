...
- Commits go on a branch cut from `staging` (NEVER directly to `main` or `staging`). See [Git Branch Workflow](git-branch-workflow.md) for the full 6-step flow (worktree → branch → green → PR → staging → main).
