...
- **Phase 3 kickoff:** T-039 landed (Sponsorship DB schema — column rename + lifecycleState add). Phase 3 now 12/13 remaining.

### T-039 completed 2026-04-18T18:22

- **File:** `packages/db/src/schemas/sponsorship/sponsorship.dbschema.ts`
- Import: added `LifecycleStatusPgEnum` alongside existing `SponsorshipStatusPgEnum`, `SponsorshipTargetTypePgEnum`.
- Column rename: `status: SponsorshipStatusPgEnum('status')` → `sponsorshipStatus: SponsorshipStatusPgEnum('sponsorship_status')`. Default `'pending'` intact, enum type unchanged.
- New column: `lifecycleState: LifecycleStatusPgEnum('lifecycle_state').notNull().default('ACTIVE')`.
- Index rename: `sponsorships_status_idx` → `sponsorships_sponsorshipStatus_idx` pointing to `table.sponsorshipStatus`.
- New index: `sponsorships_lifecycleState_idx` on `table.lifecycleState`.
- **Decision (user-approved option 1, 2026-04-18):** SQL column also renamed (`status` → `sponsorship_status`), not just TS prop. Rationale: push-only friendly (no rollback concern), keeps TS/SQL aligned, grep of raw SQL stays coherent. Same criterion OwnerPromotion T-003 followed.
- **YAGNI call:** NO composite index `lifecycleState+targetType` — same decision as DestinationReview T-028; planner can combine. Revisit if query plans show need.
- Lint: pass. Typecheck: **deferred** (cascade: consumers still reference `sponsorships.status` until T-043 zod base / T-047 model findByStatus+findActiveByTarget / T-050..T-054 routes + admin UI).
- Cascade consumers (out of T-039 scope, per later tasks): `packages/db/src/models/sponsorship/sponsorship.model.ts`, `apps/admin/src/routes/_authed/sponsor/sponsorships.tsx`, `apps/admin/src/features/sponsorships/components/{SponsorshipsTab,SponsorshipLevelsTab,SponsorshipPackagesTab}.tsx`.
- `packages/i18n/src/types.ts` reference is auto-generated; will be regenerated by i18n sweep task analogous to T-027 when Sponsorship i18n needs updating.
- Commit boundary: 1 feat(db) + 1 chore(tasks).
