...
**Completed 2026-04-20 (current session)**: 9 tasks — T-041 + T-048 + T-042 + T-049 + T-043 + T-050 + T-065 (service-core hardDelete tests) + T-066 (API integration tests) + T-068 (reconciliation partial close)
**Splits applied**: 20 parent tasks split into 45 child tasks (68 total vs 47 original)

**Remaining pending (9 tasks)**:
...
- **T-026** (complexity 1.5, blocked on T-022 — done) — Seed SSRF allowlist + `--reset`/`--clean-images` flag integration. Deferred: unblocked but not prioritized this session; pure seed hardening with no downstream blockers.
- **T-027** (complexity 1.5, blocked on T-022 — done) — Seed cache flush + `--validate-cache` + Zod validation + `.gitignore`. Deferred: same rationale as T-026; bundled follow-up for next seed-focused sweep.
- **T-038** (complexity 2.5, blocked on T-017 + T-029 — both done) — Admin UI wire upload/delete for destinations/events/posts. **Requires product consult** — multi-entity wiring replicating accommodations/$id_.edit.tsx pattern needs UX decision before implementation.
- **T-044** (complexity 2.0, blocked on T-038) — Admin a11y: GalleryField dnd-kit refactor.
- **T-045** (complexity 2.0, blocked on T-038) — Admin a11y: ImageField error banner + confirmation dialog + reduced-motion.
- **T-046** (complexity 2.0, blocked on T-038) — Admin UX: HEIC/AVIF handling + p-limit + progress indicator.
- **T-047** (complexity 1.5, blocked on T-038) — Admin UX: shadcn Avatar + maxFileSizeMb prop.
