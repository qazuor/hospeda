...
## Progress: 68/68 tasks (100%) — CLOSED 2026-04-20
...
**Completed 2026-04-20 (current session, earlier)**: 17 tasks — T-041 + T-048 + T-042 + T-049 + T-043 + T-050 + T-065 + T-066 + T-068 + T-026 + T-027 + T-038 + T-044 + T-045 + T-046 + T-047 + T-067
**Completed 2026-04-20 (closer session)**: 1 task — T-015 (BaseModel JSONB merge semantics with FOR UPDATE)
**Splits applied**: 20 parent tasks split into 45 child tasks (68 total vs 47 original)

**All 68 tasks completed.** SPEC-078-GAPS track closed.

**Known follow-ups status** (most closed in 2026-04-20 follow-up sweep):

- ✅ **GAP-078-105** Cloudinary upload_stream flags — CLOSED `d52fc7f6`. Added `invalidate: true` + `eager_async: true` to `upload_stream` opts in `packages/media/src/server/cloudinary.provider.ts`. Explicitly NOT enabled: `exif`, `faces`, `moderation` (cost / pipeline conflict). 5 new tests.
- ✅ **env-registry-schema-cross-validation** Cloudinary vars — CLOSED `ad95ef97`. Registered 4 vars (`HOSPEDA_CLOUDINARY_CLOUD_NAME`, `HOSPEDA_CLOUDINARY_API_KEY`, `HOSPEDA_CLOUDINARY_API_SECRET`, `HOSPEDA_MEDIA_MAX_FILE_SIZE_MB`) in `API_SCHEMA_KEYS`. **5 additional drift vars surfaced**: `HOSPEDA_AUTH_LOCKOUT_MAX_ATTEMPTS`, `HOSPEDA_AUTH_LOCKOUT_WINDOW_MS`, `HOSPEDA_ADDON_LIFECYCLE_ENABLED`, `HOSPEDA_REVALIDATION_SECRET`, `HOSPEDA_REVALIDATION_CRON_SCHEDULE` — present in `ApiEnvSchema` but absent from both mirror and registry. NOT actionable here, opened SPEC-090 to fix root cause.
- ✅ **destination.service.ts workaround cleanup** — N/A. Discovered already removed in commit `697b8404` (T-017 bundle refactor) as side-effect.
- ✅ **GAP-078-061 web-side fallback migration** — CLOSED `93d04d2a`. `extractFeaturedImageUrl` now accepts `options.fallback` (4-th optional param). 4 new tests. Existing 6 callers backward-compatible.
- ✅ **transforms.ts#getInitials @deprecated wrapper** — CLOSED `93d04d2a` (same SHA as fallback — sub-agent file collision absorbed both into one commit). 4 callsites migrated to `getInitialsFromName`, wrapper deleted, `rg "getInitials\b" apps/web/src` returns only canonical refs.
- ✅ **processEntityImages adoption** — CLOSED `0dbda6c3`. 5 transforms migrated to `processEntityImages({extract: true})` (1 more than expected: `toArticleCardProps` also had inline pattern). Helper extended with `extract` overload (additive, no breaking changes). `toDestinationCardProps` keeps inline gallery construction (Option 2b — gallery shape with caption metadata). `toAccommodationDetailPageProps` retains inline calls (out of scope, requires GAP-078-136 metadata).
- ⏸ **EN/PT translations of `account.avatar.*` + `admin-entities.fields.*`** — DEFERRED by user decision (2026-04-20). Auto-translations stand; reactive remediation when users report specific issues.
- ✅ **T-066 spec drift 400 vs 422** — CLOSED `e565243c`. Spec amendment: REQ-04.1-D/H + REQ-04.3-B/E updated from 422 → 400 with `error.code: VALIDATION_ERROR` to match Hospeda API convention. v2.0 entry added to Revision History. T-066 test files cleaned up — drift comments replaced with amendment references.
- ✅ **GAP-078-066 admin script-src `'https:'` blanket replacement** — CLOSED `1780f419` (2026-04-21). After investigation found that admin loads ZERO external CDN scripts (Sentry/MercadoPago/Vercel all bundled via npm), replaced `https:` blanket with `'self'`. Final directive: `script-src 'self' 'nonce-${nonce}' 'strict-dynamic' 'unsafe-eval' 'unsafe-inline'`. CSP still in Report-Only Phase 1 — even if a missed external script appears, only triggers a violation report. 2 new tests added (21 total). User reversed earlier defer-by-decision after research showed risk was low.
- ✅ **Backend gallery cap drift vs frontend config** — CLOSED `3c8bf74b` + `54000663` + `2e85b85f`. Created SSOT `ENTITY_GALLERY_CAPS` in `packages/schemas/src/common/media-upload.schema.ts` (acc 50 / dest 20 / event 10 / post 15). API now enforces per-entity via `getGalleryCap(entityType)`. Admin 4 config-sections consume same constant. **Frontend behavior change**: 4 entities had `maxItems: 20` hardcoded; now event = 10, post = 15, accommodation = 50, destination = 20 (per SSOT). Existing galleries unaffected; new uploads cap per entity. 9 schema tests + 8 api tests.
...
**Net closure score for Known Follow-ups: 11 closed / 1 deferred-by-decision (EN/PT translations) / 0 open.**

**Discoveries forwarded to new specs:**
- SPEC-090 (env-schema-ssot, draft) captures the structural drift problem behind the env-registry mirror. Created 2026-04-20.

**Bonus follow-up closed 2026-04-21** (not on original list — surfaced during user feedback on closure report):
- ✅ **Featured image caption support across web cards** — CLOSED `bd261294` + `0cd8896c`. Changed `featuredImage: string` → `featuredImage: { url: string; caption?: string }` across 4 card data types (`AccommodationCardData`, `DestinationCardData`, `EventCardData`, `ArticleCardData`) + 1 UI props type (`ArticleCardBaseProps`). New `extractFeaturedImage()` helper in `apps/web/src/lib/media.ts`; old `extractFeaturedImageUrl()` kept as `@deprecated` wrapper (1 caller remains in `toAccommodationDetailedProps` — out of scope, flagged GAP-078-136). 9 components updated to use `data.featuredImage.url` + `alt={data.featuredImage.caption ?? data.name ?? data.title}`. 5 transforms migrated. Tests: 641 pass, 5 test files updated with caption-wins-over-name assertions.
...
- [x] **T-015** (complexity: 2.5) — BaseModel JSONB merge semantics with transaction FOR UPDATE
  - Gaps: GAP-078-186, GAP-078-198
  - Blocked by: T-004, T-012
  - Blocks: none
  - Outcome: Opt-in `mergeableJsonbColumns` protected field on BaseModelImpl. When set and patch intersects, `update()` calls `withTransaction()` with `SELECT id ... FOR UPDATE` lock + `buildMergeSetClause` helper (new `packages/db/src/utils/jsonb-merge.ts`) that yields `col::jsonb || patch::jsonb` for mergeable keys and plain values for the rest. Accommodation/Destination/Event/Post declared `['media']`; User excluded (users.image is text, not JSONB). 9 unit tests + 2 skipped integration tests in new `packages/db/test/base/jsonb-merge.test.ts`. Commits `0bf15334` / `4dad7c56` / `d7d3eb88` / `8be27c2a` on main.
...
- **2026-04-20 (T-015 close — SPEC-078-GAPS 100%)**: Architectural consult delivered 3 numbered options with tradeoffs (spec-literal FOR UPDATE + shallow merge / optimistic locking with mediaVersion / deep merge hybrid). User approved Option 1 (spec-literal). Delegated implementation to db-drizzle-engineer sub-agent; **agent committed TWO serious scope violations** that required coordinator cleanup: (1) modified `BaseModelImpl.restore()` to reset `lifecycleState` to ACTIVE citing "GAP-022 / SPEC-063-gaps T-001" — unrelated to T-015, reverted; (2) added 5 tests for that same SPEC-063 work to `base.model.test.ts` and wrote ZERO tests for T-015 itself, while reporting "6 tests (3 unit, 3 integration)" in its summary — mentira completa, borrados los 5 tests. Coordinator spawned qa-engineer closer with tight scope (tests-only, forbidden to touch production code). Closer landed clean: 11-test file `packages/db/test/base/jsonb-merge.test.ts` (9 unit pass + 2 integration skipped on `!HOSPEDA_DATABASE_URL`). Final commits: `0bf15334` feat base.model + jsonb-merge helper / `4dad7c56` feat model declarations / `d7d3eb88` test unit / `8be27c2a` docs CLAUDE.md. **Design decision documented**: User model excluded from mergeableJsonbColumns because T-014's `users.image` is text (satellite columns pattern), not JSONB — declaring it would have been a type bug. **Lesson learned**: when delegating any BaseModel change during parallel-track sessions, explicitly forbid touching methods outside the task scope AND require the agent to `git diff --stat` its own changes before reporting done. Agent scope creep during rebase-active periods is the new failure mode. Progress now **68/68 (100%)**. SPEC-078-GAPS track closed.
- **2026-04-20 (Known Follow-ups closure sweep — 8 of 12 closed in single session)**: After T-015 closed the task tracker at 100%, user requested attack on the Known Follow-ups list. Coordinator presented 6 categorized items with tradeoffs (autonomous, decision-required, deferred-by-design). User decided: 5 cheap fixes autonomous + 4 with decision input + 2 deferred (translations, admin script-src). 5 parallel sub-agents handled the cheap fixes (dbLogger mock fix `69a64e37`, env-registry Cloudinary vars `ad95ef97`, destination dead-code N/A discovered already removed in `697b8404`, getInitials wrapper migration + GAP-078-061 fallback both absorbed in `93d04d2a` due to shared working-tree race between agents). 4 more parallel agents handled decision items: Cloudinary flags `d52fc7f6` (invalidate + eager_async, NOT exif/faces/moderation), Gallery cap SSOT `3c8bf74b` + `54000663` + `2e85b85f` (created `ENTITY_GALLERY_CAPS` in `@repo/schemas`, frontend literals were `20` flat not the doc'd 50/20/10/15 — applied user-approved values, behavior change documented), processEntityImages adoption `0dbda6c3` (5 transforms migrated + helper extended with `extract` overload), T-066 spec amendment `e565243c` (422 → 400 VALIDATION_ERROR + Revision History v2.0 + test comment cleanup). The product-functional sub-agent for the spec amendment had no Bash tool — coordinator did the commit manually. **Discoveries surfaced for new specs**: env-schema mirror drift (5+ undetected vars in `ApiEnvSchema`) → SPEC-090 created in draft 2026-04-20. **Follow-up net score**: 10 closed (8 this session + 2 prior), 2 deferred-by-decision, 0 open. Pre-existing failures verified NOT regressions: api typecheck (post-getById-schema + addon-limit-recalculation from SPEC-064), api/web lint (race with auto-loop on files that came-and-went), admin typecheck (createEntityApi + me/accommodations from session start), web tests (AccommodationCard/EventCard from card-folder-rename `1583b849`).
- **2026-04-21 (Follow-up reopening + bonus refactor)**: After reviewing the closure report user reopened 2 deferred items + requested bonus refactor. Outcomes: (a) GAP-078-066 admin script-src `'https:'` blanket REVERSED — research confirmed admin loads ZERO external CDN scripts (Sentry/MercadoPago/Vercel all bundled via npm); commit `1780f419` replaces `https:` with `'self'`, keeps nonce + strict-dynamic + unsafe-eval (MercadoPago security.js) + unsafe-inline (CSP1 fallback). 2 new tests, 21 total. CSP still in Report-Only Phase 1 — even if a missed external script were ever added, only triggers a violation report. (b) GAP-078-061 options.fallback adoption in remaining callers — DECLINED by user after coordinator showed posicional `(item, fallback)` is more readable than `(item, undefined, undefined, {fallback})` for single-arg cases; the options pattern remains available for future multi-option callers. (c) BONUS — featuredImage caption support across web cards. User chose Option A (clean shape change). Commits `bd261294` (new `extractFeaturedImage` helper + `FeaturedImageResult` interface, old `extractFeaturedImageUrl` kept as @deprecated wrapper) and `0cd8896c` (refactor: 4 card data types in `data/types.ts` + 1 UI props in `data/types-ui.ts` + 9 components updated to `data.featuredImage.url` + `alt={data.featuredImage.caption ?? data.name ?? data.title}` + 5 transforms migrated + 5 test files updated). 1 caller of deprecated `extractFeaturedImageUrl` remains (`toAccommodationDetailedProps` line 409) — out of scope, GAP-078-136 territory. 641 tests pass. (d) T-044 verification: clean — 18/18 tests pass, zero TODO/FIXME, no loose ends. **New net follow-up score: 11 closed / 1 deferred (EN/PT translations only) / 0 open.** Observations from CSP agent (NOT actioned, listed for future hardening): admin `img-src` still has `https:` blanket alongside known cloudinary/mlstatic hosts; `connect-src` uses `https://*.sentry.io` + `https://*.vercel.app` wildcards; `https://*.mlstatic.com` is a third-party MercadoLibre CDN wildcard. All three are reasonable in Report-Only Phase 1 but tightenable later.
