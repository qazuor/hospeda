...
| 2026-05-24 | qazuor | local | superadmin@hospeda.com | (acts on host-basico sub) | 2.2 Subscription cancel | **FAIL** | Setup: re-set superadmin password locally + inserted credential account for `admin@hospeda.com`. Observed: `ADMIN` role does NOT have `MANAGE_SUBSCRIPTIONS` perm (192 perms vs SUPER_ADMIN's bypass via `actor.ts:152-156` which grants all perms). **Finding #26 (open, policy question)**: should ADMIN carry MANAGE_SUBSCRIPTIONS by default? Currently NO role in `rolePermissions.seed.ts` has it (the SUPER_ADMIN path bypasses the table entirely). Smoke executed with SUPER_ADMIN. `POST /api/v1/admin/billing/subscriptions/{id}/cancel` returned **HTTP 200** with success envelope and a `canceledAt` timestamp populated, BUT `status` remained `"active"` in both response body and DB (verified directly: `SELECT status, canceled_at FROM billing_subscriptions WHERE id = ...`). `cancel_at_period_end=false`, `cancel_at=null`, so this is meant to be an immediate cancel — but the state transition didn't happen. Entitlements still resolve to owner-basico after cancel (re-attempted MAX_PHOTOS upload — still 403, same limit, sub still active). **Finding #27 (open, CRITICAL)**: admin cancel route reports success and sets `canceled_at` but does not transition `status`, leaving the sub effectively active. Related to the engram pin `bug/cancel-spelling-drift` (US `canceled` vs UK `cancelled`) referenced in this section's note, but the bug here is more fundamental — no status transition at all, regardless of spelling. |
