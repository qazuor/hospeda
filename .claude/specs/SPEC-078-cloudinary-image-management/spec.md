...
3. The client (web app avatar component) receives the URL and calls the user profile update endpoint `PATCH /api/v1/protected/users/${userId}` with `{ image: returnedUrl }`, where `${userId}` is the session user's own ID. The endpoint MUST enforce an ownership check: the session user MUST equal the path parameter `userId`, otherwise HTTP 403 is returned.
4. The user profile PATCH endpoint validates and persists the updated `image` field to the database.

**Prerequisite**: A `PATCH /api/v1/protected/users/${userId}` endpoint accepting `{ image: string }` MUST exist for the avatar flow to complete end-to-end. This is the canonical convention across every `/api/v1/protected/*` route in this repo: resources are addressed by their explicit ID with an ownership/permission check on the handler, NOT by a `/me` alias. If the endpoint does not exist at implementation time, it MUST be created as part of this spec's implementation. The endpoint only needs to accept `{ image: string }` for this use case (other profile fields are out of scope).

**Note on the `/me` pattern**: Some REST style guides recommend a `/me` alias (for example, `PATCH /users/me`) that resolves to the session user server-side. This repo intentionally does NOT use that pattern today; all protected routes use the explicit-ID form with an ownership check. Adopting a `/me` alias is a cross-cutting API-conventions change that affects every protected resource (users, bookings, reviews, saved searches, etc.) and its introduction MUST go through a dedicated SPEC rather than being smuggled in through this spec. Until such a SPEC is approved, `AvatarUpload` and any other client code MUST use `PATCH /api/v1/protected/users/${userId}`.
...
| User profile endpoint | New or Existing | `PATCH /api/v1/protected/users/${userId}` for avatar URL persistence (REQ-04.2-FLOW), with session-user ownership check. Follows the repo convention for all protected routes (explicit ID + permission check, NOT a `/me` alias). Create if it does not exist |
