...
parent: SPEC-134
priority: medium
findings_addressed: [F-010, F-022, F-024, F-025, F-026, F-027, F-028, F-029, F-031]
...
### Group 1 — Routing bug (do FIRST, isolated)

| Finding | Severity | Issue |
|---|---|---|
| F-022 | high | `/accommodations/$id/gallery` renders byte-for-byte identical EDIT page |

**Fix**: Inspect `apps/admin/src/routes/_authed/accommodations/$id_.gallery.tsx`. Likely the route imports the wrong component or re-exports the EDIT view by accident. Wire it to the actual Gallery component (image grid, upload, reorder) — verify whether that component already exists in `apps/admin/src/features/accommodations/` or needs to be built.

### Group 2 — Error UX (do SECOND)
...
- [ ] `/accommodations/$id/gallery` renders a gallery-specific UI (image grid, upload), NOT the EDIT form
...
- [ ] Re-run audit script (see SPEC-134 §5) — F-010, F-022, F-024, F-025, F-026, F-027, F-028, F-029, F-031 no longer flagged
...
- F-022 (gallery routing) might reveal that the Gallery feature was never implemented, not just routed wrong. If so, file separately as a feature gap, don't bundle here.
