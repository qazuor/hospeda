...
function AmenityCreatePage() {
    ...
    const { sections, entityConfig } = useMemo(() => {
        ...
        return {
            ...
            entityConfig: {
                ...
                description: 'Crear una nueva amenidad para alojamientos',
                entityName: consolidatedConfig.metadata?.entityName || 'Amenidad',
                entityNamePlural: consolidatedConfig.metadata?.entityNamePlural || 'Amenidades',
                sections: createSections,
                viewSections: [],
                editSections: createSections,
                routes: {
                    base: '/content/accommodation-amenities',
                    view: '/content/accommodation-amenities/$id',
                    edit: '/content/accommodation-amenities/$id/edit',
                    sections: {},
                    editSections: {}
                },
                permissions: {
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            adminLogger.debug('[AmenityCreate] Saving values', values);

            const result = await createMutation.mutateAsync(values);

            addToast({
                title: 'Amenidad creada',
                message: 'La amenidad se ha creado exitosamente',
                variant: 'success'
            });

            // Navigate to the new amenity's view page
            const newId = (result as { id: string }).id;
            navigate({ to: `/content/accommodation-amenities/${newId}` });
        } catch (error) {
            adminLogger.error('Failed to create amenity', error);

            let errorMessage = 'Error inesperado al crear la amenidad';
    ...
    const handleCancel = () => {
        navigate({ to: '/content/accommodation-amenities' });
    };
            ...
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-2xl">Crear Amenidad</CardTitle>
                            <p className="text-gray-600">
                                Crear una nueva amenidad para alojamientos
                            </p>
                        </div>
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            >
                                                ...
                                                <div className="mt-6 flex justify-end gap-3 border-t pt-6">
                                                    ...
                                                    >
                                                        {isSaving ? 'Creando...' : 'Crear Amenidad'}
                                                        {!overallProgress.readyForSubmission && (
