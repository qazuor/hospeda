...
function FeatureCreatePage() {
    ...
    const { sections, entityConfig } = useMemo(() => {
        ...
        return {
            ...
            entityConfig: {
                ...
                description: 'Crear una nueva característica para alojamientos',
                entityName: consolidatedConfig.metadata?.entityName || 'Característica',
                entityNamePlural:
                    consolidatedConfig.metadata?.entityNamePlural || 'Características',
                sections: createSections,
                viewSections: [],
                editSections: createSections,
                routes: {
                    base: '/content/accommodation-features',
                    view: '/content/accommodation-features/$id',
                    edit: '/content/accommodation-features/$id/edit',
                    sections: {},
                    editSections: {}
                },
                permissions: {
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            adminLogger.debug('[FeatureCreate] Saving values', values);

            const result = await createMutation.mutateAsync(values);

            addToast({
                title: 'Característica creada',
                message: 'La característica se ha creado exitosamente',
                variant: 'success'
            });

            // Navigate to the new feature's view page
            const newId = (result as { id: string }).id;
            navigate({ to: `/content/accommodation-features/${newId}` });
        } catch (error) {
            adminLogger.error('Failed to create feature', error);

            let errorMessage = 'Error inesperado al crear la característica';
    ...
    const handleCancel = () => {
        navigate({ to: '/content/accommodation-features' });
    };
            ...
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-2xl">Crear Característica</CardTitle>
                            <p className="text-gray-600">
                                Crear una nueva característica para alojamientos
                            </p>
                        </div>
