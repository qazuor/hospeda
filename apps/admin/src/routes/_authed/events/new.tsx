...
function EventCreatePage() {
    ...
    const { sections, entityConfig } = useMemo(() => {
        ...
        return {
            ...
            entityConfig: {
                ...
                entityName: consolidatedConfig.metadata?.entityName || 'Evento',
                entityNamePlural: consolidatedConfig.metadata?.entityNamePlural || 'Eventos',
                sections: createSections,
                viewSections: [],
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            adminLogger.debug('[EventCreate] Saving values', values);
            ...
            // Navigate to the new event's view page
            const newId = (result as { id: string }).id;
            navigate({ to: `/events/${newId}` });
        } catch (error) {
            adminLogger.error('Failed to create event', error);

            let errorMessage = 'Error inesperado al crear el evento';
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-gray-600">Crear un nuevo evento</p>
                            </div>
