...
function EventLocationCreatePage() {
    ...
    const { sections, entityConfig } = useMemo(() => {
        const consolidatedConfig = createEventLocationConsolidatedConfig();
        const createSections = filterSectionsByMode(consolidatedConfig.sections, 'create');

        return {
            sections: createSections,
            entityConfig: {
                id: 'event-location-new',
                entityType: 'event-location',
                title: 'Crear Ubicación de Evento',
                description: 'Crear una nueva ubicación para eventos',
                entityName: consolidatedConfig.metadata?.entityName || 'Ubicación de Evento',
                entityNamePlural:
                    consolidatedConfig.metadata?.entityNamePlural || 'Ubicaciones de Eventos',
                sections: createSections,
                viewSections: [],
                editSections: createSections,
                routes: {
                    base: '/events/locations',
                    view: '/events/locations/$id',
                    edit: '/events/locations/$id/edit',
                    sections: {},
                    editSections: {}
                },
                permissions: {
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            adminLogger.debug('[EventLocationCreate] Saving values', values);

            const result = await createMutation.mutateAsync(values);

            addToast({
                title: 'Ubicación creada',
                message: 'La ubicación de evento se ha creado exitosamente',
                variant: 'success'
            });

            // Navigate to the new event location's view page
            const newId = (result as { id: string }).id;
            navigate({ to: `/events/locations/${newId}` });
        } catch (error) {
            adminLogger.error('Failed to create event location', error);

            let errorMessage = 'Error inesperado al crear la ubicación';
            ...
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-2xl">Crear Ubicación de Evento</CardTitle>
                            <p className="text-gray-600">Crear una nueva ubicación para eventos</p>
                        </div>
