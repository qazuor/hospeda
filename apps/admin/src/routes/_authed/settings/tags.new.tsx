...
function TagCreatePage() {
    ...
    const { sections, entityConfig } = useMemo(() => {
        ...
        return {
            ...
            entityConfig: {
                ...
                entityName: consolidatedConfig.metadata?.entityName || 'Etiqueta',
                entityNamePlural: consolidatedConfig.metadata?.entityNamePlural || 'Etiquetas',
                sections: createSections,
                viewSections: [],
                editSections: createSections,
                routes: {
                    base: '/settings/tags',
                    view: '/settings/tags/$id',
                    edit: '/settings/tags/$id/edit',
                    sections: {},
                    editSections: {}
                },
                permissions: {
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            adminLogger.debug('[TagCreate] Saving values', values);

            const result = await createMutation.mutateAsync(values);

            addToast({
                title: 'Etiqueta creada',
                message: 'La etiqueta se ha creado exitosamente',
                variant: 'success'
            });

            // Navigate to the new tag's view page
            const newId = (result as { id: string }).id;
            navigate({ to: `/settings/tags/${newId}` });
        } catch (error) {
            adminLogger.error('Failed to create tag', error);

            let errorMessage = 'Error inesperado al crear la etiqueta';
            ...
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-2xl">Crear Etiqueta</CardTitle>
                            <p className="text-gray-600">Crear una nueva etiqueta</p>
                        </div>
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            >
                                                ...
                                                <div className="mt-6 flex justify-end gap-3 border-t pt-6">
                                                    ...
                                                    >
                                                        {isSaving ? 'Creando...' : 'Crear Etiqueta'}
                                                        {!overallProgress.readyForSubmission && (
