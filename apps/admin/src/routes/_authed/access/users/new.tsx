...
function UserCreatePage() {
    ...
    const { sections, entityConfig } = useMemo(() => {
        ...
        return {
            ...
            entityConfig: {
                ...
                entityName: consolidatedConfig.metadata?.entityName || 'Usuario',
                entityNamePlural: consolidatedConfig.metadata?.entityNamePlural || 'Usuarios',
                sections: createSections,
                viewSections: [],
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            adminLogger.debug('[UserCreate] Saving values', values);
            ...
            // Navigate to the new user's view page
            const newId = (result as { id: string }).id;
            navigate({ to: `/access/users/${newId}` });
        } catch (error) {
            adminLogger.error('Failed to create user', error);

            let errorMessage = 'Error inesperado al crear el usuario';
