...
function PostCreatePage() {
    ...
    const { sections, entityConfig } = useMemo(() => {
        ...
        return {
            ...
            entityConfig: {
                ...
                entityName: consolidatedConfig.metadata?.entityName || 'Artículo',
                entityNamePlural: consolidatedConfig.metadata?.entityNamePlural || 'Artículos',
                sections: createSections,
                viewSections: [],
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            adminLogger.debug('[PostCreate] Saving values', values);

            const result = await createMutation.mutateAsync(values);

            addToast({
                title: 'Artículo creado',
                message: 'El artículo se ha creado exitosamente',
                variant: 'success'
            });

            // Navigate to the new post's view page
            const newId = (result as { id: string }).id;
            navigate({ to: `/posts/${newId}` });
        } catch (error) {
            adminLogger.error('Failed to create post', error);

            let errorMessage = 'Error inesperado al crear el artículo';
            ...
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-2xl">Crear Artículo</CardTitle>
                            <p className="text-gray-600">Crear un nuevo artículo para el blog</p>
                        </div>
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            >
                                                ...
                                                <div className="mt-6 flex justify-end gap-3 border-t pt-6">
                                                    ...
                                                    >
                                                        {isSaving ? 'Creando...' : 'Crear Artículo'}
                                                        {!overallProgress.readyForSubmission && (
