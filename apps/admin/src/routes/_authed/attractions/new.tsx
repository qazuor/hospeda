...
export const Route = createFileRoute('/_authed/attractions/new')({
...
function AttractionCreatePage() {
    ...
    const { sections, entityConfig } = useMemo(() => {
        ...
        return {
            ...
            entityConfig: {
                ...
                description: 'Crear una nueva atracción turística',
                entityName: consolidatedConfig.metadata?.entityName || 'Atracción',
                entityNamePlural: consolidatedConfig.metadata?.entityNamePlural || 'Atracciones',
                sections: createSections,
                viewSections: [],
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            adminLogger.debug('[AttractionCreate] Saving values', values);

            const result = await createMutation.mutateAsync(values);

            addToast({
                title: 'Atracción creada',
                message: 'La atracción se ha creado exitosamente',
                variant: 'success'
            });

            // Navigate to the new attraction's view page
            const newId = (result as { id: string }).id;
            navigate({ to: `/attractions/${newId}` });
        } catch (error) {
            adminLogger.error('Failed to create attraction', error);

            let errorMessage = 'Error inesperado al crear la atracción';
            ...
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-2xl">Crear Atracción</CardTitle>
                            <p className="text-gray-600">Crear una nueva atracción turística</p>
                        </div>
