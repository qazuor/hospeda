...
function SponsorCreatePage() {
    ...
    const { sections, entityConfig } = useMemo(() => {
        ...
        return {
            ...
            entityConfig: {
                ...
                entityName: consolidatedConfig.metadata?.entityName || 'Patrocinador',
                entityNamePlural: consolidatedConfig.metadata?.entityNamePlural || 'Patrocinadores',
                sections: createSections,
                viewSections: [],
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            adminLogger.debug('[SponsorCreate] Saving values', values);

            const result = await createMutation.mutateAsync(values);

            addToast({
                title: 'Patrocinador creado',
                message: 'El patrocinador se ha creado exitosamente',
                variant: 'success'
            });

            // Navigate to the new sponsor's view page
            const newId = (result as { id: string }).id;
            navigate({ to: `/sponsors/${newId}` });
        } catch (error) {
            adminLogger.error('Failed to create sponsor', error);

            let errorMessage = 'Error inesperado al crear el patrocinador';
            ...
            <Card>
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-2xl">Crear Patrocinador</CardTitle>
                            <p className="text-gray-600">Crear un nuevo patrocinador</p>
                        </div>
