...
import { TranslationStatus, type TranslationFieldState } from './TranslationStatus';
...
export function TranslationSection({ entityType, entityId, entity }: TranslationSectionProps) {
    ...
    const fieldStates = useMemo(() => {
        ...
        const meta = (entity.translationMeta as Record<string, Record<string, { autoTranslated?: boolean; translatedAt?: string }>> | null) ?? {};
        ...
        return fields.map((fieldType): TranslationFieldState => {
            ...
            const locales: Record<string, { value: string; autoTranslated: boolean; translatedAt?: string }> = {};
    ...
    const handleTranslateNow = useCallback(async (fieldType: string) => {
        try {
            ...
            if (result.success) {
                ...
                addToast({
                    ...
                    message: result.error?.message ?? t('admin-common.aiTranslate.error.default'),
                    variant: 'error'
                });
            }
        } catch {
    ...
    const handleOverrideSaved = useCallback(async (fieldType: string, locale: string, value: string) => {
        try {
