...
export const stableDestinationSearchFn = async (query: string): Promise<SelectOption[]> => {
    ...
    try {
        const searchPath = `/api/v1/admin/destinations?search=${encodeURIComponent(query)}&pageSize=15`;
...
export const stableDestinationLoadAllFn = async (): Promise<SelectOption[]> => {
    const searchPath = '/api/v1/admin/destinations?pageSize=100'; // Load all destinations
