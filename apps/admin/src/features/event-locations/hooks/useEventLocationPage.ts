...
/**
 * Hook for managing event location entity pages
 * Centralizes all event location-specific logic in one place
 */
export const useEventLocationPage = (entityId: string) => {
    ...
    const query = useEventLocationQuery(entityId);
    const updateMutation = useUpdateEventLocationMutation(entityId);

    // Consolidated configuration
    const entityConfig = useMemo(() => {
        const consolidatedConfig = createEventLocationConsolidatedConfig(t);
    ...
    const permissions = useMemo(
        () => ({
            ...
            create: [PermissionEnum.EVENT_LOCATION_CREATE],
            delete: [PermissionEnum.EVENT_LOCATION_DELETE]
        }),
        []
    );
    ...
    // Navigation
    const goToList = () => navigate({ to: '/events/locations' });
    const goToView = () => navigate({ to: `/events/locations/${entityId}` });
    const goToEdit = () => navigate({ to: `/events/locations/${entityId}/edit` });
