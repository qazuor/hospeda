...
async function fetchInvoices(filters: Record<string, unknown> = {}) {
    ...
    const result = await fetchApi<{
        ...
        path: `/api/v1/admin/billing/invoices?${params.toString()}`
    });
...
async function fetchInvoice(id: string) {
    const result = await fetchApi<{ success: boolean; data: Record<string, unknown> }>({
        path: `/api/v1/admin/billing/invoices/${id}`
    });
...
async function payInvoice(id: string) {
    const result = await fetchApi<{ success: boolean; data: Record<string, unknown> }>({
        path: `/api/v1/admin/billing/invoices/${id}/pay`,
        method: 'POST'
    });
    return result.data.data;
}
...
async function voidInvoice(id: string) {
    const result = await fetchApi<{ success: boolean; data: Record<string, unknown> }>({
        path: `/api/v1/admin/billing/invoices/${id}/void`,
        method: 'POST'
    });
    return result.data.data;
}
