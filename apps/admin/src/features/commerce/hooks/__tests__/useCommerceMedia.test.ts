 ...
 * (`'gastronomy'` → `/gastronomies/...`, `'experience'` → `/experiences/...`):
 *   - Mock fetchApi at module level
 *   - Wrap hooks in a fresh QueryClientProvider with retries disabled
 *   - Assert: correct path/method/body sent to fetchApi, per vertical
    ...
    it('calls GET on the gastronomies endpoint for vertical="gastronomy"', async () => {
        ...
        expect(mockedFetchApi).toHaveBeenCalledWith(
            expect.objectContaining({
                path: '/api/v1/admin/gastronomies/ent-1/media?state=visible'
            })
        );
        expect(result.current.data).toHaveLength(2);
    });

    it('calls GET on the experiences endpoint for vertical="experience"', async () => {
        mockedFetchApi.mockResolvedValue({
        ...
        expect(mockedFetchApi).toHaveBeenCalledWith(
            expect.objectContaining({
                path: '/api/v1/admin/experiences/ent-2/media?state=visible'
            })
        );
    });
...
describe('useCommerceMediaAdd', () => {
    it('POSTs to the vertical-specific media endpoint and returns the created row', async () => {
        ...
        expect(mockedFetchApi).toHaveBeenCalledWith(
            expect.objectContaining({
                path: '/api/v1/admin/experiences/ent-2/media',
                method: 'POST',
...
describe('useCommerceMediaRemove', () => {
    it('calls DELETE on the correct vertical endpoint and invalidates list', async () => {
        ...
        expect(mockedFetchApi).toHaveBeenCalledWith(
            expect.objectContaining({
                path: '/api/v1/admin/experiences/ent-2/media/media-42',
                method: 'DELETE'
            })
        );
        expect(invalidateSpy).toHaveBeenCalledWith(
...
describe('useCommerceMediaSetFeatured', () => {
    it('PUTs to /featured on the correct vertical endpoint and returns the updated row', async () => {
        ...
        expect(mockedFetchApi).toHaveBeenCalledWith(
            expect.objectContaining({
                path: '/api/v1/admin/gastronomies/ent-1/media/media-7/featured',
                method: 'PUT'
            })
        );
