...
/**
 * Component tests for ContentGalleryManager (HOS-390).
 *
 * Ported from `ContentGalleryManager.test.tsx` (HOS-382). Tests:
 ...
 *  9. entityType passed to uploadEntityImage matches the `entity` prop — the
 *     entity-agnostic wiring, and the one thing a copy-paste port gets wrong
 *     silently (a post upload landing under `event` would still 200)
 *
 * Mocking strategy: the four useContentMedia* hooks and useMediaUpload are
...
vi.mock('@/features/content-media/hooks/useContentMedia', () => ({
    useContentMediaList: () => mockListData,
    useContentMediaAdd: () => ({
...
vi.mock('@repo/schemas', () => ({
    ENTITY_GALLERY_CAPS: { post: 15, event: 10 },
...
import { ContentGalleryManager } from '../ContentGalleryManager';
...
describe('ContentGalleryManager — split: featured vs gallery', () => {
...
describe('ContentGalleryManager — add gallery photo', () => {
    it('calls upload then addMedia when a file is selected for the gallery, with entityType matching the entity prop', async () => {
...
describe('ContentGalleryManager — remove gallery photo', () => {
...
describe('ContentGalleryManager — remove portada', () => {
...
describe('ContentGalleryManager — grid remove button uses its own accessible name', () => {
...
describe('ContentGalleryManager — alt derived from entity name', () => {
...
describe('ContentGalleryManager — set portada (upload → add → setFeatured)', () => {
...
describe('ContentGalleryManager — gated on entity-detail loading too (race regression guard)', () => {
    it('keeps the loading skeleton (no upload controls) while isEntityLoading is true, even though the media query already settled', () => {
        ...
        // usePostQuery/useEventQuery) is still in flight. If the
...
describe('ContentGalleryManager — load error', () => {
...
describe('ContentGalleryManager — entity prop wiring', () => {
    it('passes entityType="event" to the upload mutation when entity="event"', async () => {
        const file = new File(['data'], 'photo.jpg', { type: 'image/jpeg' });
        mockUploadEntityImageMutateAsync.mockResolvedValue({
            url: 'https://cdn.example.com/new.jpg',
            publicId: 'hospeda/dev/new'
