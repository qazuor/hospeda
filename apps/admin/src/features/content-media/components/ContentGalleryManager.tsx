 ...
 *   editorial entity — see `useContentMedia` module docs)
...
import { ContentGalleryPortadaSection } from '@/features/content-media/components/ContentGalleryPortadaSection';
import {
    type ContentMedia,
...
} from '@/features/content-media/hooks/useContentMedia';
...
/**
 * Props for ContentGalleryManager.
 */
export interface ContentGalleryManagerProps {
    /** Which editorial entity this gallery belongs to. */
    readonly entity: ContentMediaEntity;
    /** UUID of the post or event whose gallery is being managed. */
    readonly entityId: string;
    /**
     * Display name of the post or event, if already available to the caller (e.g.
...
/**
 * Full gallery management panel for a single post or event.
 *
 ...
 */
export function ContentGalleryManager({
    entity,
    entityId,
    entityName,
    isEntityLoading = false
}: ContentGalleryManagerProps) {
    ...
    const {
        ...
        isError
    } = useContentMediaList(entity, entityId);
    // Gate on BOTH queries — see `isEntityLoading` JSDoc above for why.
    const isLoading = isMediaLoading || isEntityLoading;

    const addMutation = useContentMediaAdd(entity, entityId);
    const removeMutation = useContentMediaRemove(entity, entityId);
    const setFeaturedMutation = useContentMediaSetFeatured(entity, entityId);
    ...
    // ── Derived state ─────────────────────────────────────────────────────────
    const featuredRow: ContentMedia | undefined = allMedia.find((m) => m.isFeatured);
    const galleryRows: ContentMedia[] = allMedia.filter((m) => !m.isFeatured);

    const galleryCap = ENTITY_GALLERY_CAPS[entity];
    ...
    const handleGalleryFile = React.useCallback(
        ...
        [entity, entityId, uploadEntityImage, addMutation, derivedAlt, t, clearErrors]
    );
