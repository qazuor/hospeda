...
const EXEMPT: ReadonlyArray<{ readonly file: string; readonly why: string }> = [
    ...
    },
    {
        ...
        why: 'Content-addressed by construction (HOS-369 W3-6): the filename carries sha256 of the sprite served, so an icon change ships a NEW URL rather than needing the old one flushed — and the endpoint 404s any hash it does not currently serve. Nothing to purge, so nothing for the catch-all to reach either.'
    },
    {
        file: 'middleware.ts',
