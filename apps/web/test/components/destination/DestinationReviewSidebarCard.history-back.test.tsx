/**
 * @file DestinationReviewSidebarCard.history-back.test.tsx
 * @description Regression tests for HOS-334: with the review dialog open on
 * the destination detail page, the system back button must close the dialog
...
// ---------------------------------------------------------------------------
// Module mocks — same shape as DestinationReviewSidebarCard.test.tsx
// ---------------------------------------------------------------------------
...
const PAGE_URL = 'http://localhost:3000/es/destinos/colon/';
...
describe('DestinationReviewSidebarCard — back button closes the dialog (HOS-334)', () => {
    beforeAll(async () => {
    ...
    it('closes the dialog on back instead of navigating off the page, keeping typed content', async () => {
        ...
        render(<DestinationReviewSidebarCard {...DEFAULT_PROPS} />);
        ...
        fireEvent.change(contentField, { target: { value: 'un destino hermoso' } });
        expect(contentField).toHaveValue('un destino hermoso');
        ...
        expect(window.location.pathname).toBe('/es/destinos/colon/');
