/**
 * @file DestinationFaqPlaceholder.test.ts
 * @description Source-based assertions for DestinationFaqPlaceholder.astro.
...
const src = readFileSync(
    resolve(__dirname, '../../../src/components/destination/DestinationFaqPlaceholder.astro'),
    'utf8'
);

describe('DestinationFaqPlaceholder.astro', () => {
    describe('imports', () => {
        it('should import FaqsIcon from @repo/icons', () => {
            expect(src).toContain("from '@repo/icons'");
            expect(src).toContain('FaqsIcon');
        });
    ...
    describe('rendering', () => {
        it('should render the FAQ icon', () => {
            expect(src).toContain('<FaqsIcon');
        });

        it('should render placeholder body copy via t()', () => {
            expect(src).toContain("'destination.detail.faq.placeholder'");
            expect(src).toContain('Estamos preparando respuestas');
        });
    ...
    describe('i18n', () => {
        ...
        it('should use t() for aria-label with fallback', () => {
            expect(src).toContain("t('destination.detail.faq.ariaLabel'");
    ...
    describe('accessibility', () => {
        ...
        it('should include aria-labelledby linking section to heading', () => {
            expect(src).toContain('aria-labelledby="dest-faq-title"');
        });
        ...
        it('should provide matching id on the section title', () => {
            expect(src).toContain('id="dest-faq-title"');
        });
    });
