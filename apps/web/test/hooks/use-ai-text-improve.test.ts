 ...
 * triggered.
 *
 * ## Web vs admin: absolute URL assertion
 *
 * Unlike the admin test (which asserts the bare relative path), the web
 * app builds an absolute URL via `getApiUrl()` from `@/lib/env`. The
 * project's `vitest.config.ts` sets `PUBLIC_API_URL=http://localhost:3001`
 * for the test environment, so we assert the fetch was called with the
...
import {
    ...
    parseSseStream,
    useAiTextImprove
} from '../../src/hooks/useAiTextImprove';
...
describe('useAiTextImprove — improve() / loading', () => {
    it('transitions to loading on improve() and POSTs to the protected route (absolute URL)', async () => {
        ...
        // Request shape: absolute URL (web is a separate origin from the
        // API), POST, JSON body, session cookie, SSE accept.
        expect(lastFetchUrl).toBe(`${getApiUrl()}/api/v1/protected/ai/text-improve`);
