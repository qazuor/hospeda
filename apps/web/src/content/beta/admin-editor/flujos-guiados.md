...
description: Cuatro procesos completos de uso real que combinan varios módulos del panel.
order: 7
role: admin-editor
section: Solo admin
---
...
Estos son procesos completos de uso real. Hacelos completos al menos una vez.

## Proceso 1 — Crear destino con todo

1. **Acceso → Usuarios** → crear un usuario "propietario o anfitrión (host) de prueba" con tu email + 1.
...
## Proceso 2 — Gestionar una suscripción de propietario

1. Crear un propietario nuevo (con otro email tuyo).
...
2. Volvé al admin → **Alojamientos → [el alojamiento]** → pestaña Reseñas.
...
## Proceso 4 — Publicar un post de blog completo

1. **Publicaciones → Agregar**.
2. Escribir título, contenido (con formato), imagen de portada.
3. Asignarle 3 etiquetas y 1 categoría.
...
8. Ir al sitio público y verificar:
   ...
   - Aparece bajo su categoría y bajo cada etiqueta
   - El SEO se aplica (mirá el title de la pestaña del navegador)
   - El sponsorship se ve

## Qué reportar

> 📋 **Reportá:** cualquier paso del proceso donde algo no funcionó como esperabas. Estos procesos son representativos del uso real, así que cada error (bug) acá vale doble.
