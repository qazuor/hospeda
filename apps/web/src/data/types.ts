 ...
 *   id: 'rev-001',
 *   quote: 'Una experiencia increíble. Volvería sin dudarlo.',
 *   rating: 5,
 *   reviewerName: 'Carlos Romero',
 *   reviewerOrigin: 'Buenos Aires, AR',
 *   reviewerAvatar: '/images/avatars/carlos.jpg',
 *   locale: 'es',
 *   badge: 'Huésped verificada',
 *   ratingCount: 24,
 ...
 *
 * Extends BlogPostCardData with additional fields for the articles grid layout.
 */
 ...
 *
 * The `value` is an integer rendered with a count-up animation.
 * The `labelKey` is an i18n key resolved via `t()` at render time.
 *
 * @example
 * ```ts
 * const stat: StatItemData = {
 *   icon: 'House',
 *   value: 200,
 *   labelKey: 'home.stats.accommodations',
 * };
 * ```
 */
 ...
 *
 * Both `titleKey` and `descriptionKey` are i18n keys resolved via `t()`.
 *
 * @example
 * ```ts
 * const feature: FeatureItemData = {
 ...
 *
 * Controls hero image rotation, section item counts, and review
 * display counts. Stored as a single export in the mock data layer
 * so all homepage sections share a consistent configuration.
 *
 * @example
 * ```ts
 ...
 *
 * Used to render a horizontal scrolling row of partner logos.
 * The `url` is optional — some partners may not have a clickable link.
 *
 * @example
 * ```ts
 * const partner: PartnerData = {
 *   name: 'Turismo Entre Ríos',
 *   logoPath: '/images/partners/turismo-er.svg',
 *   url: 'https://turismo.entrerios.gov.ar',
 * };
 * ```
 */
