...
/** A locale the platform can resolve i18n text to. */
export type ResolvedLocale = (typeof LOCALE_FALLBACK_ORDER)[number];

/**
 * Reports which locale {@link resolveI18nText} actually resolves `value` to,
 * following the same fallback order. This lets callers declare an honest
 * `inLanguage` for structured data: the language the emitted text is really
 * in, not merely the requested page locale.
 *
 * A plain string (legacy, unlocalized field) or an empty/absent value resolves
 * to `'es'` — the platform default, which is the language legacy text is in.
 *
 * @param value - An `I18nText` object (or partial), a plain string, null, or undefined.
 * @param locale - The desired locale (`es`, `en`, or `pt`).
 * @returns The locale the text resolves to (`es` when nothing better matches).
 *
 * @example
 * resolveI18nLocale({ es: 'Hola', en: 'Hi', pt: 'Oi' }, 'en') // → 'en'
 * resolveI18nLocale({ es: 'Hola', en: '', pt: '' }, 'en')      // → 'es' (fallback)
 * resolveI18nLocale('Hola', 'en')                              // → 'es' (legacy string)
 */
export function resolveI18nLocale(
    value: I18nText | I18nTextLike | string | null | undefined,
    locale: string
): ResolvedLocale {
    // Legacy plain string or empty value: it is the platform-default Spanish.
    if (!value || typeof value === 'string') return 'es';

    const i18n = value as I18nTextLike;

    const localeKey = locale as keyof I18nText;
    if (i18n[localeKey]) return localeKey as ResolvedLocale;

    for (const fallbackLocale of LOCALE_FALLBACK_ORDER) {
        if (i18n[fallbackLocale]) return fallbackLocale;
    }
