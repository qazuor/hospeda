 ...
 * `POST /api/v1/protected/ai/text-improve` (SPEC-198 T-008, ported to web
 * for SPEC-321 T-001).
 *
 ...
 * project's `apiClient` because that wrapper is JSON-only and designed
 * for request/response semantics, not server-sent events.
 *
 ...
 * use `fetch` + `ReadableStream`. Auth is via the shared session cookie
 * (`credentials: 'include'`), which is the same mechanism `useMyEntitlements`
 * uses.
 *
 * ## Web vs admin: absolute URL
 *
 * Unlike the admin app (same-origin/proxied to the API), the web app is a
 * separate Astro app on its own origin (dev: localhost:4321 vs API on
 * localhost:3001). We MUST build an absolute URL via `getApiUrl()` from
...
/** Path of the protected AI text-improve endpoint (appended to `getApiUrl()`). */
 ...
 * consumer (the `AiTextImprovePanel` component built in T-002/T-003) is
...
export function useAiTextImprove(): UseAiTextImproveReturn {
    ...
    const improve = useCallback(
        async (req: AiTextImprove): Promise<void> => {
            ...
            try {
                response = await fetch(`${getApiUrl()}${TEXT_IMPROVE_PATH}`, {
                    method: 'POST',
                    headers: {
                    ...
                    // Send the shared session cookie. The protected middleware
