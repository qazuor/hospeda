...
export const AI_ENTITLEMENT_BY_FEATURE: Readonly<Record<QuotaGatedAiFeature, EntitlementKey>> = {
    ...
    // HOS-400: all three chat features answer to the SAME entitlement key. The
    // per-vertical split lives on the METERING axis (`AiFeature`), never on the
    // catalogue axis — what differs between them is the subscription the key is
    // read from, which is `resolveCommerceVerticalGrants`'s job, not this map's.
    chat_gastronomy: EntitlementKey.AI_CHAT,
    chat_experience: EntitlementKey.AI_CHAT,
...
export const AI_LIMIT_BY_FEATURE: Readonly<Record<QuotaGatedAiFeature, LimitKey>> = {
    ...
    // HOS-400: unlike the entitlement map above, the LIMIT map DOES differ per
    // vertical — that is the whole point of the split. Each vertical's cap is
    // read from its own subscription's plan row.
    chat_gastronomy: LimitKey.MAX_AI_CHAT_GASTRONOMY_PER_MONTH,
    chat_experience: LimitKey.MAX_AI_CHAT_EXPERIENCE_PER_MONTH,
