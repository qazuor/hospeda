/**
 * PATCH /api/v1/admin/events/:id/media/:mediaId
 * Correct the text metadata of a event gallery photo (HOS-1036).
 *
 * The event twin of `accommodation/admin/updateMedia.ts` (HOS-388). Until this
 ...
 *
 * Permission model: gated on `EVENT_UPDATE`, the broad grant — the same gate the sibling admin media routes declare. The service helper re-checks via `checkEventCanEditMedia`.
 */
export const adminUpdateEventMediaRoute = createAdminRoute({
    method: 'patch',
    path: '/{id}/media/{mediaId}',
    summary: 'Correct photo text metadata in event gallery (admin)',
    description:
        ...
        'VALIDATION_ERROR, not a silent 200. Requires EVENT_UPDATE.',
    tags: ['Event', 'Media'],
