 ...
 * require a broad `EVENT_VIEW_ALL`-style grant; the service `findOptions` enforces the same
 ...
 */
export const adminEventOptionsRoute = createAdminRoute({
    method: 'get',
    path: '/options',
    summary: 'Event options (admin lookup)',
    description:
    ...
    handler: async (ctx: Context, _params, _body, query) => {
        ...
        const result = await eventService.findOptions(actor, { q, limit });
