/**
 * PATCH /api/v1/protected/posts/:id/media/:mediaId
 * Correct the text metadata of a post gallery photo (HOS-1036).
 *
 * The post twin of `accommodation/protected/updateMedia.ts` (HOS-388). Until this
 ...
 * another post, a non-existent id, or a soft-deleted row all answer `NOT_FOUND` (404) —
 * never `FORBIDDEN` (403), so a foreign media id cannot be confirmed to exist (see
 * `apps/api/docs/error-contract.md`).
 *
 * The PARENT post is NOT protected that way, and that is deliberate. The gate
 * (`checkPostCanEditMedia`) answers `FORBIDDEN` (403) on a post the actor may not edit
 * and `NOT_FOUND` (404) on one that does not exist, so an actor whose grant is
 * ownership-scoped (`POST_UPDATE_OWN` without `POST_UPDATE`) can tell a stranger's post
 * id from an invented one. The sibling media helpers — add, remove, reorder,
 ...
 *
 * Permission model: the service helper `updatePostMedia` gates on `checkPostCanEditMedia`, which delegates to `checkCanUpdatePost` — editing a post's photo text IS editing the post, so it cannot require less.
 */
export const protectedUpdatePostMediaRoute = createCRUDRoute({
    method: 'patch',
    path: '/{id}/media/{mediaId}',
    summary: 'Correct photo text metadata in post gallery',
    description:
        ...
        'VALIDATION_ERROR, not a silent 200. Requires the same permission as updating the post.',
