/**
 * GET /api/v1/protected/views/events
 *
 * Returns view-count statistics for a caller-supplied batch of EVENT entities
 * over a rolling window (7d or 30d).
 *
 * **Query params:**
 *   - `window` — rolling window: '7d' or '30d' (default '30d').
 *   - `entityIds` — comma-separated or repeated UUIDs of the events to query
 ...
 * event ID, zero-view entities included as `{ unique: 0, total: 0 }`.
 *
 * @module routes/views/protected/events
 * @see SPEC-159 T-010
 */
...
/**
 * GET /events?window=7d|30d&entityIds=uuid1,uuid2,...
 *
 * Returns view stats for a batch of EVENT entities (editor dashboard widget).
 * Requires `EVENT_VIEW_ALL` permission.
 */
export const eventViewStatsRoute = createProtectedRoute({
    method: 'get',
    path: '/events',
    summary: 'Get view stats for events',
    description:
        'Returns view-count statistics for a batch of EVENT entities over a rolling window ' +
        '(7d or 30d). Accepts up to 100 entity IDs as repeated params or a comma-separated ' +
        'string. Requires EVENT_VIEW_ALL permission.',
    tags: ['Views'],
