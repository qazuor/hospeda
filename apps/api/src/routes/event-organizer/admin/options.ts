/**
 * GET /api/v1/admin/event-organizers/options
 ...
 * require the `EVENT_ORGANIZER_VIEW` grant; the service `findOptions` enforces the same
 * admin-access gate. Results are DRAFT-inclusive.
 *
 * MUST be registered BEFORE the `/{id}` route so Hono does not resolve `options` as an id.
 */
export const adminEventOrganizerOptionsRoute = createAdminRoute({
    method: 'get',
    path: '/options',
    summary: 'Event organizer options (admin lookup)',
    description:
    ...
    handler: async (ctx: Context, _params, _body, query) => {
        ...
        const result = await eventOrganizerService.findOptions(actor, { q, limit });
