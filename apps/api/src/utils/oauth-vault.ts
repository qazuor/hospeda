/**
 * OAuth credential vault crypto util (HOS-45 T-004).
 *
 * Symmetric AES-256-GCM encryption/decryption for OAuth provider credentials
 * (access tokens, refresh tokens, client secrets) at rest. The master key
 * comes ONLY from `env.HOSPEDA_OAUTH_VAULT_MASTER_KEY` (read here and nowhere
 * else). This is a deliberate near-duplicate of `ai-vault.ts`, NOT a shared
 * extraction: the OAuth vault's key material and code path are kept fully
 * independent from the AI credential vault so the two blast radii never mix,
 * even though the two providers may one day both need rotation at the same
 * time.
 *
 * Output shape mirrors the OAuth credentials storage columns
 * (`ciphertext`, `iv`, `auth_tag`), all stored as base64 strings.
 *
 * The key derivation, IV length, and auth tag length mirror `ai-vault.ts`
 * exactly (`sha256(masterKey)`, 12-byte IV, 16-byte auth tag) for consistency
 * across the two vaults. Changing this derivation later would make all
 * previously-encrypted credentials undecryptable.
 *
 * @module utils/oauth-vault
 */
...
/**
 * The encrypted form of a secret, ready to persist into the OAuth
 * credentials storage columns. All fields are base64-encoded.
 */
 ...
 * @throws {Error} When `HOSPEDA_OAUTH_VAULT_MASTER_KEY` is not configured.
 */
function deriveKey(): Buffer {
    const masterKey = env.HOSPEDA_OAUTH_VAULT_MASTER_KEY;
    if (masterKey === undefined || masterKey.length === 0) {
        throw new Error(
            'HOSPEDA_OAUTH_VAULT_MASTER_KEY is not set. The OAuth credential vault cannot ' +
 ...
 * @example
 * ```ts
 * const enc = encryptSecret({ plaintext: 'APP_USR-refresh-token-value' });
