 ...
 *
 * @module services/promo-code.validation
 */
                ...
                apiLogger.info(
                    {
                        ...
                        validPlans: promoData.validPlans
                    },
        ...
        if (promoData.newCustomersOnly && context.userId) {
            ...
            if (hasExistingUsage) {
                apiLogger.info(
                    ...
                    'Promo code rejected: new customers only, user has existing promo usage'
                );
                return {
                    valid: false,
...
async function checkUserHasPromoUsage({ userId }: { readonly userId: string }): Promise<boolean> {
    try {
        ...
        apiLogger.warn(
            ...
            'Failed to check user promo usage, allowing code as fallback'
        );
        return false;
    }
}
