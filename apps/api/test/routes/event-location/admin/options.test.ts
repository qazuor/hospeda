/**
 * Tests for GET /api/v1/admin/event-locations/options (SPEC-169 §5.5 / T-018).
 *
 ...
 * `ACCESS_PANEL_ADMIN` must NOT be blocked (not 403/404). A guest (no auth) must be
 * rejected (401/403).
 *
 ...
 * (id/label/slug, with placeName→slug label fallback) and DRAFT-inclusivity are covered by
...
describe('GET /api/v1/admin/event-locations/options (SPEC-169 lookup gate)', () => {
    let app: AppOpenAPI;
    const base = '/api/v1/admin/event-locations/options';
    ...
    it('resolves /options before /{id} (does not 404 as a missing location)', async () => {
        const status = await requestStatus(editorPanelOnlyHeaders);
