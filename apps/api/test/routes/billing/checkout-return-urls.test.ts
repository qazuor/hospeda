...
describe('checkout-return-urls — annual builders (HOS-123 T-005)', () => {
    ...
    it('buildPartnerCheckoutReturnUrl lands on a PUBLIC page, never the admin panel', () => {
        // A partner is an external brand with no Hospeda account — the payment
        // link is generated by an admin and sent out. The old back_url
        // (`${HOSPEDA_ADMIN_URL}/partners`) dropped the person who had just
        // paid onto the admin login screen.
        for (const locale of ['es', 'en', 'pt'] as const) {
            const url = buildPartnerCheckoutReturnUrl(locale);
            expect(url).toBe(`https://site.test/${locale}/partners/checkout/pending/`);
            expect(url.startsWith('https://site.test/')).toBe(true);
        }
    });

    it('the partner return URL points at a route that actually exists in apps/web', () => {
        // Finding #8 class: Astro's locale middleware rewrites an unknown
        // segment into the 404 surface, so a back_url is only as good as the
        // page file behind it.
        const page = resolve(
            __dirname,
            '../../../../web/src/pages/[lang]/partners/checkout/pending.astro'
        );
        expect(existsSync(page)).toBe(true);
    });

    it('the partner send-link route no longer builds an admin-panel back_url', () => {
        const src = readFileSync(
            resolve(__dirname, '../../../src/routes/partners/admin/send-link.ts'),
            'utf8'
        );
        // Anchored on the env var itself: renaming the local builder function
        // must not let the admin URL slip back in.
        expect(src).not.toContain('HOSPEDA_ADMIN_URL');
        expect(src).toContain('buildPartnerCheckoutReturnUrl');
    });
