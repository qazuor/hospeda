...
describe('GET /api/v1/protected/user-bookmarks (list user bookmarks)', () => {
    let app: AppOpenAPI;
    ...
    it('route is registered and reachable (returns 200, 401, or 403)', async () => {
        try {
            const res = await app.request(base, {
                method: 'GET',
                headers: {
            ...
            // 404 means the route is not registered at all - that must not happen
    ...
    it('accepts pagination query params (page, pageSize) without crashing', async () => {
        try {
    ...
    it('returns response with bookmarks and total when authenticated', async () => {
        try {
            const res = await app.request(base, {
                method: 'GET',
                headers: {
    ...
    it('accepts combined entityType and pagination params', async () => {
        try {
            const res = await app.request(`${base}?entityType=EVENT&page=2&pageSize=20`, {
                method: 'GET',
                headers: {
                    'user-agent': 'vitest',
                    accept: 'application/json'
