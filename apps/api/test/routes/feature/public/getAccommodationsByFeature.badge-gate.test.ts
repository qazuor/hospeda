/**
 * HOS-341 regression — the accommodations-by-feature listing must gate
 ...
 *
 * The route also lives under `/api/v1/public/features`, a shared-cached prefix
 * whose cache key carries no `Authorization` header — an un-gated badge is
 * served to every visitor for the whole TTL, not just to the first one.
 *
 ...
 *
 * @module test/routes/feature/public/getAccommodationsByFeature.badge-gate
 */
...
const FEATURE_ID = 'ffffffff-0000-4000-8000-000000000341';
...
/**
 * Builds an accommodation row as the service returns it (full entity, no column
 * allowlist). `isVerified` defaults to `true` on purpose: the gate short-circuits
 * on falsy values, so a `false` fixture never reaches the branch under test.
 */
function makeAccommodation(
    overrides: Partial<{ id: string; slug: string; ownerId: string; isVerified: boolean }>
): Record<string, unknown> {
    return {
        id: 'acc-feature-default',
...
describe('publicGetAccommodationsByFeatureRoute — HOS-341 isVerified owner gate', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });
