/**
 * Unit/integration tests for PATCH /api/v1/protected/accommodations/:id/media/reorder
 * Reorder gallery photos — Protected (owner-facing) endpoint (SPEC-204)
 *
 * Coverage:
 ...
 * Actor is a HOST owner with UPDATE_OWN. requireEntitlement is passed via
 * the injected `userEntitlements` context set in the test environment.
 *
 * @module test/routes/accommodation-protected-reorder-media
 */
...
const BASE_URL = `/api/v1/protected/accommodations/${ACCOMMODATION_ID}/media/reorder`;
...
// Tests
// ---------------------------------------------------------------------------
describe('PATCH /api/v1/protected/accommodations/:id/media/reorder — reorderMedia (SPEC-204)', () => {
    let app: AppOpenAPI;
    ...
    describe('Error handling', () => {
        ...
        it('should return 400 when orderedIds is empty (Zod min(1) validation)', async () => {
            ...
            // 401/403 if auth/entitlement blocks first in test env.
            expect([400, 401, 403]).toContain(res.status);
        });
    ...
    describe('Route registration', () => {
        ...
        it('GET on the reorder path should return 4xx (wrong method — not route-not-found)', async () => {
            // Ensures "reorder" is not matched as a UUID param by any GET route.
            const res = await app.request(BASE_URL, {
                method: 'GET',
