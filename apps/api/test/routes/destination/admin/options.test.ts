/**
 * Tests for GET /api/v1/admin/destinations/options (SPEC-169 §5.5 / T-018).
 *
...
describe('GET /api/v1/admin/destinations/options (SPEC-169 lookup gate)', () => {
    let app: AppOpenAPI;
    const base = '/api/v1/admin/destinations/options';
    ...
    it('resolves /options before /{id} (does not 404 as a missing destination)', async () => {
        const status = await requestStatus(editorPanelOnlyHeaders);
