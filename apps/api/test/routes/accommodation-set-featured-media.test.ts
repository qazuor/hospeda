/**
 * Unit/integration tests for PUT /api/v1/admin/accommodations/:id/media/:mediaId/featured
 ...
 *
 * @module test/routes/accommodation-set-featured-media
 */
...
const BASE_URL = `/api/v1/admin/accommodations/${ACCOMMODATION_ID}/media/${MEDIA_ID}/featured`;
...
// Tests
// ---------------------------------------------------------------------------
describe('PUT /api/v1/admin/accommodations/:id/media/:mediaId/featured — setFeaturedMedia (SPEC-204 T-020)', () => {
    let app: AppOpenAPI;
    ...
    describe('DB invariant: archived photo cannot be featured', () => {
        it('should return 4xx when service rejects an archived target photo', async () => {
            // Service detects isFeatured=archived state and returns VALIDATION_ERROR.
            mockSetFeaturedMedia.mockResolvedValue({
                data: undefined,
                error: {
            ...
            // Must not succeed — archived photo featuring is an invariant violation.
    ...
    describe('Route registration', () => {
        ...
        it('should not collide with DELETE /:id/media/:mediaId (different method + suffix)', async () => {
            ...
            const deleteUrl = `/api/v1/admin/accommodations/${ACCOMMODATION_ID}/media/featured`;
            const res = await app.request(deleteUrl, {
                method: 'DELETE',
                headers: { Authorization: 'Bearer test-admin-token' }
            });
            // A DELETE on .../media/featured should 400 (UUID validation failure for
            // "featured" as mediaId param) or 404 — never 200.
