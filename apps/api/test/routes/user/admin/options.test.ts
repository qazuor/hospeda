/**
 * Tests for GET /api/v1/admin/users/options (SPEC-169 §5.5 / T-018).
 *
 ...
 * `ACCESS_PANEL_ADMIN` — with NO `USER_READ_ALL` — must NOT be blocked (not 403/404). A
 * guest (no auth) must be rejected (401/403).
 *
 ...
 * (id/label/slug) is covered by the service unit test.
 */
import { beforeAll, describe, expect, it } from 'vitest';
import { initApp } from '../../../../src/app.js';
import type { AppOpenAPI } from '../../../../src/types.js';

describe('GET /api/v1/admin/users/options (SPEC-169 lookup gate)', () => {
    let app: AppOpenAPI;
    const base = '/api/v1/admin/users/options';

    const MOCK_USER_ID = '11111111-1111-4111-8111-111111111111';

    // Editor with ONLY admin-panel access — crucially NO USER_READ_ALL grant.
    ...
    it('accepts an ACCESS_PANEL_ADMIN-only editor (no USER_READ_ALL) at the gate (not 403/404)', async () => {
        const status = await requestStatus(editorPanelOnlyHeaders);
        expect(status).not.toBe(403);
        expect(status).not.toBe(404);
    });

    it('resolves /options before /{id} (does not 404 as a missing user)', async () => {
        const status = await requestStatus(editorPanelOnlyHeaders);
