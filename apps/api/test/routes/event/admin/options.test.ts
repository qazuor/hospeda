/**
 * Tests for GET /api/v1/admin/events/options (SPEC-169 §5.5 / T-018).
 *
...
describe('GET /api/v1/admin/events/options (SPEC-169 lookup gate)', () => {
    let app: AppOpenAPI;
    const base = '/api/v1/admin/events/options';
    ...
    it('resolves /options before /{id} (does not 404 as a missing event)', async () => {
        const status = await requestStatus(editorPanelOnlyHeaders);
