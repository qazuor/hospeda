/**
 * Route tests for the protected post media endpoints (HOS-390).
 *
 ...
 * `packages/service-core/test/services/post/post.media.test.ts`.
 *
 * @module test/routes/post-protected-media
 */
...
vi.mock('@repo/service-core', async () => {
    ...
    return {
        ...actual,
        getPostMedia: mockGetMedia,
        addPostMedia: mockAddMedia,
        removePostMedia: mockRemoveMedia,
        reorderPostMedia: mockReorderMedia,
        setFeaturedPostMedia: mockSetFeaturedMedia
    };
});
...
const BASE = `/api/v1/protected/posts/${POST_ID}/media`;
/**
...
const AUTH = {
    ...
    'x-mock-actor-permissions': JSON.stringify([PermissionEnum.POST_UPDATE_OWN])
};
...
describe('protected post media routes (HOS-390)', () => {
    let app: AppOpenAPI;
        ...
        it('maps a service FORBIDDEN onto a 4xx rather than a 200', async () => {
            mockAddMedia.mockResolvedValue({
                ...
                error: {
                    ...
                    message: 'Forbidden: cannot update post'
                }
            });
    ...
    describe('input validation', () => {
        it('rejects a non-UUID post id before reaching the service', async () => {
            const res = await app.request('/api/v1/protected/posts/not-a-uuid/media', {
                method: 'GET',
                headers: AUTH
            });
