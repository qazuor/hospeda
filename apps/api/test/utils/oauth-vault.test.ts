 ...
 * `env.HOSPEDA_OAUTH_VAULT_MASTER_KEY` on every call, so mutating this between
 * tests is observed immediately.
 */
const mockEnv = vi.hoisted(() => ({
    env: {
        HOSPEDA_OAUTH_VAULT_MASTER_KEY: 'test-vault-master-key-at-least-32-chars-xx' as
            | string
            | undefined
    }
}));
...
import { decryptSecret, encryptSecret } from '../../src/utils/oauth-vault.js';

const VALID_KEY = 'test-vault-master-key-at-least-32-chars-xx';

describe('oauth-vault', () => {
    beforeEach(() => {
        mockEnv.env.HOSPEDA_OAUTH_VAULT_MASTER_KEY = VALID_KEY;
    });

    describe('round-trip', () => {
        it('decrypts back to the original plaintext', () => {
            // Arrange
            const plaintext = 'APP_USR-mercadolibre-refresh-token-123';
            // Act
    ...
    describe('tamper detection', () => {
        ...
        it('throws when decrypted with a different master key', () => {
            const enc = encryptSecret({ plaintext: 'secret' });
            mockEnv.env.HOSPEDA_OAUTH_VAULT_MASTER_KEY = 'a-completely-different-master-key-32xxxx';
    ...
    describe('missing master key', () => {
        it('encrypt throws a clear error', () => {
            mockEnv.env.HOSPEDA_OAUTH_VAULT_MASTER_KEY = undefined;
            expect(() => encryptSecret({ plaintext: 'x' })).toThrow(
                /HOSPEDA_OAUTH_VAULT_MASTER_KEY is not set/
            );
        });
        ...
        it('decrypt throws a clear error', () => {
            const enc = encryptSecret({ plaintext: 'x' });
            mockEnv.env.HOSPEDA_OAUTH_VAULT_MASTER_KEY = undefined;
            expect(() => decryptSecret(enc)).toThrow(/HOSPEDA_OAUTH_VAULT_MASTER_KEY is not set/);
        });
    });
});
