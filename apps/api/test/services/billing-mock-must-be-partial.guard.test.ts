 ...
 * ## Scope
 *
 * `apps/api` only — this file can read only its own package's tests. A sibling
 * guards `packages/service-core`. Known remaining literal mocks outside both,
 * left as documented debt by HOS-702 because they belong to unrelated packages:
 * 4 in `apps/admin`, 2 in `packages/seed`.
 */
...
describe('HOS-702 guard: @repo/billing is never mocked as a whole-module literal', () => {
    it('finds billing mocks to check at all (guard is not vacuously green)', () => {
        ...
        expect(
            total,
            "No vi.mock('@repo/billing', ...) call sites were found under apps/api/test. " +
